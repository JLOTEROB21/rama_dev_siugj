...
include_once("funcionesRendererListadoRegistros.php");
...
function cambiarEtapaFormulario($idFormulario,$idRegistro,$etapa,$comentarios="",$idPerfil=-1,$res1="NULL",$res2="NULL",$idActorProceso="0")
{
	...
	$query100584[$x100584]="insert into 941_bitacoraEtapasFormularios(etapaActual,fechaCambio,idUsuarioCambio,etapaAnterior,idFormulario,idRegistro,comentarios,referencia1,referencia2,actorCambio) 
				values (".$etapa.",'".date('Y-m-d H:i:s')."',".$idUsuario.",".($estadoAnterior==""?0:$estadoAnterior).",".$idFormulario.",".$idRegistro.",'".cv($comentarios)."',".($res1==""?"NULL":$res1).",".($res2==""?"NULL":$res2).",".$idActorProceso.")";
	$x100584++;
	...
	{
		$arrIgnoraCambioEtapa["500"]=1; //Pausado
		$arrIgnoraCambioEtapa["510"]=1; //Abortado
		...
		while($fNotificacion=$con->fetchAssoc($resNotificacionesAnterior))
		{
			
			if(($fNotificacion["marcarAtendidaCambioEtapa"]==1)&&($fNotificacion["confComplementaria"]!="")&&($etapa<>$estadoAnterior)&&(!isset($arrIgnoraCambioEtapa[$etapa])))
			{
				...
				if(($confComplementaria->funcionAplicacion!="")&&($confComplementaria->funcionAplicacion!=-1))
				{
					...
					$cadParametros='{"idFormulario":"'.$idFormulario.'","idReferencia":"'.$idRegistro.'","idProceso":"'.$idProceso.'",
									...
									($idActorProceso==""?0:$idActorProceso).'","iFormulario":"'.$idFormulario.'","iRegistro":"'.$idRegistro.'"}';
				...
				if($aplicarTareaAtendida)
				{
					...
					while($fNotificacionMarca=$con->fetchAssoc($rNotificacion))
					{
						if($afectarNotificionUsuario==1)
						{
		...
		$consulta="SELECT idTableroControl FROM 9060_tablerosControl";
		$rTableros=$con->obtenerFilas($consulta);
		while($fTablero=$con->fetchAssoc($rTableros))
		{
			$nombreTablaTablero="9060_tableroControl_".$fTablero["idTableroControl"];
			$consulta="SELECT tC.* FROM ".$nombreTablaTablero." tC,00014_registrosCreadosElementosMacroProceso r 
					WHERE tC.llaveTarea='".$llaveTarea."' AND tC.idEstado=1 and r.iRegistro=tC.idRegistro and r.nombreTabla='".$nombreTablaTablero."'";
			$rTareas=$con->obtenerFilas($consulta);
			while($fTarea=$con->fetchAssoc($rTareas))
			{
				marcarTareaAtendida($fTarea["idRegistro"],$fTablero["idTableroControl"],$_SESSION["idUsr"],false,true);
			}
		}
		...
		if(esFormularioBase($idFormulario))
		{
			$cadParametros='{"idFormulario":"'.$idFormulario.'","idRegistro":"'.$idRegistro.'","idProceso":"'.$idProceso.
					'","iFormulario":"'.$idFormulario.'","iRegistro":"'.$idRegistro.'","idActorProceso":"'.($idActorProceso==""?0:$idActorProceso).'","campoTablaDestino":"","etapa":"'.$etapa.
					'","idMacroProceso":"","idRegistroProcesoEtapaMacroProceso":"","idElementoEvaluacion":"","tipoElemento":"","idRegistroElemento":"","lblEtiquetaElemento":""}';
			$objParametros=json_decode($cadParametros);
			
			$cuerpo=generarCuerpoExpresion($idProceso,$etapa,$idFormulario,$idRegistro,$idPerfil,$idActorProceso);
			eval ($cuerpo);
			ejecutarListadoNotificaciones($idProceso,$idFormulario,$idRegistro,$etapa,$idPerfil,$idActorProceso);
			ejecutarScriptCambioEtapa($idProceso,$etapa,$idFormulario,$idRegistro,$idActorProceso);
			ejecutarArranqueProcesosAsociados($idProceso,$etapa,$idFormulario,$idRegistro,$idActorProceso);
			ejecutarCambioEtapaProcesosRelacionados($idProceso,$etapa,$idFormulario,$idRegistro,$idActorProceso);
			
			$cAdmonMacroProceso=new cMacroProcesoAdmon($idFormulario,$idRegistro,$objParametros);
			$cAdmonMacroProceso->asentarRegistroMacroProceso();
...
function ejecutarScriptCambioEtapa($idProceso,$etapa,$idFormulario,$idRegistro,$idActorProceso)
{
	global $con;
	$consulta="SELECT * FROM 9083_ejecucionScriptCambioEtapa WHERE idProceso=".$idProceso." AND numEtapa=".$etapa;
	$filaScript=$con->obtenerPrimeraFilaAsoc($consulta);
	if(($filaScript)&&($filaScript["urlScript"]!=""))
	{

		$ejecutarScript=true;
		if(($filaScript["funcionAplicacion"]!=-1)&&($filaScript["funcionAplicacion"]!=0))
		{
			$cacheCalculos=true;
			$cadParametros='{"idFormulario":"'.$idFormulario.'","idRegistro":"'.$idRegistro.'","idProceso":"'.$idProceso.
			...
			$resultadoEvaluacion=removerComillasLimite(resolverExpresionCalculoPHP($filaScript["funcionAplicacion"],$objParametros,$cacheCalculos));
		...
		if($ejecutarScript)
		{
			$_SESSION["funcionCargaProceso"]="ejecutarInvocacionPaginaScript('".$filaScript["urlScript"]."',".($filaScript["imprimirScript"]==1?"true":"false").",".$idFormulario.",".$idRegistro.")";
...
function ejecutarArranqueProcesosAsociados($idProceso,$etapa,$idFormulario,$idRegistro,$idActorProceso)
{
	global $con;
	
	if(isset($_SESSION["aperturaProceso_".$idFormulario."_".$idRegistro."_".removerCerosDerecha($etapa)]))
		unset($_SESSION["aperturaProceso_".$idFormulario."_".$idRegistro."_".removerCerosDerecha($etapa)]);
	$nombreTablaBase=obtenerNombreTabla($idFormulario);
	$consulta="SELECT * FROM ".$nombreTablaBase." WHERE id_".$nombreTablaBase."=".$idRegistro;
	...
	$consulta="SELECT * FROM 9080_arrancadoresProceso WHERE idProcesoOrigen=".$idProceso." AND numEtapaProcesoOrigen=".$etapa;

	$res=$con->obtenerFilas($consulta);
	while($fila=$con->fetchAssoc($res))
	{
		
		$idFormularioBaseDestino=obtenerFormularioBase($fila["idProcesoDestino"]);
		$nombreTabla=obtenerNombreTabla($idFormularioBaseDestino);
		$consulta="SELECT * FROM ".$nombreTabla." WHERE idProcesoPadre=".$idProceso." AND idReferencia=".$idRegistro;
		

		if($con->existeCampo("idReferenciaArrancador",$nombreTabla))
			$consulta.=" and idReferenciaArrancador=".$fila["idRegistroArrancador"];
			
		$filaInstancia=$con->obtenerPrimeraFilaAsoc($consulta);

		$considerarArranqueProceso=true;
		if(($fila["funcionAplicacion"]!="")&&($fila["funcionAplicacion"]!="-1"))
		{
			...
			$resultadoEvaluacion=removerComillasLimite(resolverExpresionCalculoPHP($fila["funcionAplicacion"],$objParametros,$cacheCalculos));
			
			if($resultadoEvaluacion==0)
			{
				$considerarArranqueProceso=false;
			}
			
			
		}

		$ejecutarInsercion=true;
		if($considerarArranqueProceso)
		{
			
			if($filaInstancia)
			{
				if($fila["mostrarVentanaProceso"]==1)
				{
					...
					$actor=obtenerActorIngresoProcesoBusquedaTareas($fila["idProcesoDestino"],$rolActor,$idFormularioBaseDestino,$filaInstancia["id_".$nombreTabla],$fila["numEtapaCambioProcesoDestino"],$_SESSION["idUsr"]);
					$_SESSION["aperturaProceso_".$idFormulario."_".$idRegistro."_".removerCerosDerecha($etapa)]='{"idFormulario":"'.$idFormularioBaseDestino.'","idRegistro":"'.$filaInstancia["id_".$nombreTabla].'","actor":"'.$actor.'"}';
					
				}
				$ejecutarInsercion=false;
			}
			
			if($ejecutarInsercion)
			{
				$arrDocumentos=array();
				$consulta="SELECT * FROM 9074_documentosRegistrosProceso WHERE idFormulario=".$idFormulario." AND idRegistro=".$idRegistro;
				$resDocumentos=$con->obtenerFilas($consulta);
				while($fDocumento=$con->fetchAssoc($resDocumentos))
				{
					array_push($arrDocumentos,$fDocumento["idDocumento"]);
				}
				...
				$arrValores["idProcesoPadre"]=$idProceso;
				if($con->existeCampo("idReferenciaArrancador",$nombreTabla))
				{
					$arrValores["idReferenciaArrancador"]=$fila["idRegistroArrancador"];
				}
				
				
				$consulta="SELECT * FROM 9081_parametrosInicialesArrancadoresProceso WHERE idRegistroArrancador=".$fila["idRegistroArrancador"];
				$rParametros=$con->obtenerFilas($consulta);
				while($fArrancador=$con->fetchAssoc($rParametros))
				{
					
					$cadParametros='{"idFormulario":"'.$idFormulario.'","idRegistro":"'.$idRegistro.'","idProceso":"'.$idProceso.
						'","idActorProceso":"'.($idActorProceso==""?0:$idActorProceso).'","etapa":"'.$etapa.'","campoTablaDestino":"'.cv($fArrancador["campoTablaDestino"]).
						'","iFormulario":"'.$idFormulario.'","iRegistro":"'.$idRegistro.'"}';
					$objParametros=json_decode($cadParametros);
					...
					switch($fArrancador["tipoLlenado"])
					{
						...
						case 6: //Valor de formulario base
							if($valor!="")
							{
								...
								$valorCampo=$fRegistroBase[$campoMysql];
							}
						break;
					...
					if(($fArrancador["campoTablaDestino"]!="idProcesoPadre")||($valorCampo!=""))
						$arrValores[$fArrancador["campoTablaDestino"]]=$valorCampo;
				}
				$idRegistroInstancia=crearInstanciaRegistroFormulario($idFormularioBaseDestino,$idRegistro,$fila["numEtapaCambioProcesoDestino"],$arrValores,$arrDocumentos,-1,0);
				if($fila["mostrarVentanaProceso"]==1)
				{	
					$consulta="SELECT actor FROM 944_actoresProcesoEtapa WHERE idActorProcesoEtapa=".$idActorProceso;
					$rolActor=$con->obtenerValor($consulta);
					$actor=obtenerActorIngresoProcesoBusquedaTareas($fila["idProcesoDestino"],$rolActor,$idFormularioBaseDestino,$idRegistroInstancia,$fila["numEtapaCambioProcesoDestino"],$_SESSION["idUsr"]);
					$_SESSION["aperturaProceso_".$idFormulario."_".$idRegistro."_".removerCerosDerecha($etapa)]='{"idFormulario":"'.$idFormularioBaseDestino.'","idRegistro":"'.$idRegistroInstancia.'","actor":"'.$actor.'"}';
...
function obtenerActorIngresoProcesoBusquedaTareas($idProceso,$rolActor,$idFormulario,$idRegistro,$numEtapa,$idUsuario)
{
	global $con;
	$consulta="SELECT idActorProcesoEtapa FROM 944_actoresProcesoEtapa WHERE idProceso=".$idProceso." AND actor='".$rolActor."' and numEtapa='".$numEtapa."'";
	$actor=$con->obtenerValor($consulta);
	if($actor=="")
	{
		$consulta="SELECT idActorProcesoEtapa FROM 944_actoresProcesoEtapa WHERE idProceso=".$idProceso." AND actor='".$rolActor."'";
		$actor=$con->obtenerValor($consulta);
		if($actor=="")
		{
			...
			$consulta="SELECT idTableroControl FROM 9060_tablerosControl";
			$rTableros=$con->obtenerFilas($consulta);
			while($filaTablero=$con->fetchAssoc($rTableros))
			{
				$nombreTableroControl="9060_tableroControl_".$filaTablero["idTableroControl"];
				$consulta="SELECT  objConfiguracion FROM 9060_tableroControl_5 WHERE iFormulario=".$idFormulario.
						" AND iRegistro=".$idRegistro." and idUsuarioDestinatario=".$idUsuario." and numEtapaRegistro=".$numEtapa;
				$objConfiguracionTarea=$con->obtenerValor($consulta);
				if($objConfiguracionTarea!="")
				{
					$rolActor=$objConfiguracionTarea->actorAccesoProceso;
					$consulta="SELECT idActorProcesoEtapa FROM 944_actoresProcesoEtapa WHERE idProceso=".$idProceso." AND actor='".$rolActor."'";
					$actor=$con->obtenerValor($consulta);
					break;
				}
...
function ejecutarCambioEtapaProcesosRelacionados($idProceso,$etapa,$idFormulario,$idRegistro,$idActorProceso)
{
	global $con;
	
	$consulta="SELECT * FROM 9082_etapaCambioProcesosAsociados WHERE idProcesoOrigen=".$idProceso." AND numEtapaProcesoOrigen=".$etapa;

	$res=$con->obtenerFilas($consulta);
	while($fila=$con->fetchAssoc($res))
	{
		
		$considerarRegistro=true;
		if(($fila["funcionAplicacion"]!="")&&($fila["funcionAplicacion"]!="-1")&&($fila["funcionAplicacion"]!="0"))
		{
			$cacheCalculos=true;
			$cadParametros='{"idFormulario":"'.$idFormulario.'","idRegistro":"'.$idRegistro.'","idProceso":"'.$idProceso.
							'","idActorProceso":"'.($idActorProceso==""?0:$idActorProceso).'","etapa":"'.$etapa.'"}';
			...
			if($resultadoEvaluacion==0)
			{
				$considerarRegistro=false;
			}
			
			
		}
		
		if($considerarRegistro)
		{
			if($fila["tipoRelacion"]==1)
			{
				$idFormularioBaseDestino=obtenerFormularioBase($fila["idProcesoDestino"]);
				$nombreTabla=obtenerNombreTabla($idFormularioBaseDestino);
				
				$consulta="SELECT id_".$nombreTabla." FROM ".$nombreTabla." WHERE idProcesoPadre=".$fila["idProcesoDestino"]." AND idReferencia=".$idRegistro;
				$idRegistroDestino=$con->obtenerValor($consulta);
				if($idRegistroDestino!="")
					cambiarEtapaFormulario($idFormularioBaseDestino,$idRegistroDestino,$fila["numEtapaCambioProcesoDestino"],"",-1,"NULL","NULL",0);
			}
			else
			{
				$nombreTabla=obtenerNombreTabla($idFormulario);
				$consulta="SELECT idReferencia,idProcesoPadre FROM ".$nombreTabla." WHERE id_".$nombreTabla."=".$idRegistro;
				$fRegistroBase=$con->obtenerPrimeraFilaAsoc($consulta);
				if($fRegistroBase["idProcesoPadre"]!=0)
				{
					$idFormularioBaseDestino=obtenerFormularioBase($fRegistroBase["idProcesoPadre"]);
					cambiarEtapaFormulario($idFormularioBaseDestino,$fRegistroBase["idReferencia"],$fila["numEtapaCambioProcesoDestino"],"",-1,$idFormulario,$idRegistro,0);
				}
...
function ejecutarListadoNotificaciones($idProceso,$idFormulario,$idRegistro,$etapa,$idPerfil,$idActorProceso)
{
	...
	$cadParametros='{"idFormulario":"'.$idFormulario.'","idRegistro":"'.$idRegistro.'","idProceso":"'.$idProceso.'","idActorProceso":"",
					...
					'","idUsuarioDestinatario":"","iFormulario":"'.$idFormulario.'","iRegistro":"'.$idRegistro.'"}';
	$objParametros=json_decode($cadParametros);
	...
	while($filaConfiguracion=$con->fetchRow($resConfiguracion))
	{	
		...
		$cadParametros='{"idFormulario":"'.$idFormulario.'","idRegistro":"'.$idRegistro.'","idReferencia":"'.$idReferencia.
					'","idProceso":"'.$idProceso.'","idActorProceso":"'.$idActorProceso.
					'","etapa":"'.$etapa.'","idUsuarioRemitente":"'.$_SESSION["idUsr"].'","nombreUsuarioRemitente":"'.cv($nombreUsuarioRemitente).'",
					...
					'","etiquetaNotificacion":"","iFormulario":"'.$idFormulario.'","iRegistro":"'.$idRegistro.
					'","arrDatosTarea":""}';
		...
		if(($funcionAsignacionDestinatario!="")&&($funcionAsignacionDestinatario!=-1))
		{
			...
			$cadParametros='{"idFormulario":"'.$idFormulario.'","idRegistro":"'.$idRegistro.'","idProceso":"'.$idProceso.
							'","idActorProceso":"'.($idActorProceso==""?0:$idActorProceso).'","etapa":"'.$etapa.
							'","actorDestinatario":"'.$actorDestinatario.'","iFormulario":"'.$idFormulario.'","iRegistro":"'.$idRegistro.'"}';
		...
		else
		{
			if($actorDestinatario!="")		
			{
				...
				while($fDestinatario=$con->fetchRow($resDestinatarios))	
				{
...
function registrarNotificacionTableroControl($idNotificacion,$objParametrosNotificacion)
{
	...
	while($fCampoTablero=$con->fetchRow($rCamposTablero))
	{
		...
		switch($fCampoTablero[1])
		{
			...
			case 7:	
				//funcion de sistema
				$objParametrosNotificacion->arrDatosTarea=bE(serialize($arrCamposValor));
				if($valor!="")
					$valorFinal="'".removerComillasLimite(resolverExpresionCalculoPHP($valor,$objParametrosNotificacion,$cacheCalculos))."'";
	...
	if($fNotificacion["repetible"]==0)
	{
		$consulta="SELECT COUNT(*) FROM ".$nombreTabla." WHERE idUsuarioDestinatario=".$objParametrosNotificacion->idUsuarioDestinatario.
				...
				cv($objParametrosNotificacion->nombreUsuarioDestinatario)."' and llaveTarea='".$llaveTarea."' and idEstado<>0";

		$nReg=$con->obtenerValor($consulta);	
		if($nReg>0)
		{
			$consulta="update ".$nombreTabla." set idEstado=1 WHERE idUsuarioDestinatario=".$objParametrosNotificacion->idUsuarioDestinatario.
				" AND idNotificacion=".$fNotificacion["idNotificacion"]." and iFormulario=".$objParametrosNotificacion->idFormulario.
				" and iRegistro=".$objParametrosNotificacion->idRegistro." and usuarioDestinatario='".
	...
	$tituloNotificacion=$fNotificacion["tituloNotificacion"];

	if(($fNotificacion["funcionRendererNotificacion"]!="")&&($fNotificacion["funcionRendererNotificacion"]!="-1"))
	{
		$objParametrosNotificacion->etiquetaNotificacion=$tituloNotificacion;
		
		$tituloNotificacion=removerComillasLimite(resolverExpresionCalculoPHP($fNotificacion["funcionRendererNotificacion"],$objParametrosNotificacion,$cacheCalculos));
			
	}
	
	if($fNotificacion["funcionAperturaPersonalizada"]!="")
	{
		$objConfiguracion='{"actorAccesoProceso":"'.$objParametrosNotificacion->actorAccesoProceso.'","funcionApertura":"'.$fNotificacion["funcionAperturaPersonalizada"].'"';
	}
	
	if($fNotificacion["funcionAfterVisualizacion"]!="")
	{
		$objConfiguracion.=',"afterVisualizacion":"'.$fNotificacion["funcionAfterVisualizacion"].'"';
	}
	...
	if($nReg==0)
	{
		...
		if(!$agregarSecretaria)
		{
			...
			$consulta="INSERT INTO ".$nombreTabla."(codigoUnidad,fechaAsignacion,idNotificacion,tipoNotificacion,
						...
						cv($tituloNotificacion)."','".cv($objParametrosNotificacion->nombreUsuarioRemitente).
		...
		else
		{
			...
			$consulta="INSERT INTO ".$nombreTabla."(codigoUnidad,fechaAsignacion,idNotificacion,tipoNotificacion,usuarioRemitente,
						...
						iReferencia,objConfiguracion,permiteAbrirProceso,secretaria,numEtapaRegistro,llaveTarea".$listaCampos.")
						values('".$codigoUnidad."','".date("Y-m-d H:i:s")."',".$fNotificacion["idNotificacion"].",'".
						cv($tituloNotificacion)."','".cv($objParametrosNotificacion->nombreUsuarioRemitente).
						"',".$objParametrosNotificacion->idUsuarioRemitente.",'".cv($objParametrosNotificacion->nombreUsuarioDestinatario).
						"',".$objParametrosNotificacion->idUsuarioDestinatario.",1,'".cv($contenidoMensaje)."',".
						...
						",'".$secretaria."',".$numEtapaRegistro.",'".$llaveTarea."'".$listaValores.")";

		}

		if(@$con->ejecutarConsulta($consulta))
		{	
			$idTarea=$con->obtenerUltimoID();
			...
			if($fNotificacion["enviarMail"]==1)
			{
				...
				if($con->existeCampo("notificadoMail",$nombreTabla)=="")
				{
					...
					$consulta="alter table `".$nombreTabla."` add column `notificadoMail` int(11) default 0, add columna `msgErrorMail` text NULL";
					$con->ejecutarConsulta($consulta);
				...
				while($filaMail=$con->fetchRow($resMailDestinatario))
				{
					$m=$filaMail[0];
					$objMail[0]=$m;
					$objMail[1]="";
				...
				if(count($arrDestinatario)>0)
				{
					...
					eval('$resultado=@'.$funcionEnvioCorreoElectronico.'($arrDestinatario,$fNotificacion["tituloNotificacion"],$contenidoMensaje,"","",$arrDocumentos,$arrCCO,$arrCC);');
					if($resultado)
					{
						$consulta="UPDATE ".$nombreTabla." SET notificadoMail=1 WHERE idRegistro=".$idTarea;
					}
					else
					{
						$errorCorreo="";
						if($_SESSION["resultadoEnvioCorreo"]!="")
						{
							$errorCorreo=",msgErrorMail='".cv($_SESSION["resultadoEnvioCorreo"])."'";
						}
						$consulta="UPDATE ".$nombreTabla." SET notificadoMail=2".$errorCorreo." WHERE idRegistro=".$idTarea;
					}
			...
			if($fNotificacion["enviarSMS"]==1)
			{
				$cuerpoMensaje="";
				if($con->existeCampo("numeroCarpetaAdministrativa",$nombreTabla))
				{
					$consulta="SELECT numeroCarpetaAdministrativa FROM ".$nombreTabla." WHERE idRegistro=".$idTarea;
					$expediente=$con->obtenerValor($consulta);
					
					$cuerpoMensaje="Código único de proceso: ".$expediente."\n\n";
				}
				$cuerpoMensaje.="Asunto: ".$tituloNotificacion."\n\n".strip_tags(str_replace("<br>","\n",str_replace("<br />","\n",$contenidoMensaje)));
				
				
				$totalNotificados=0;
				$notificacionesEnviadas=0;
				$consulta="SELECT * FROM 804_telefonos WHERE idUsuario=".$objParametrosNotificacion->idUsuarioDestinatario;
				$res=$con->obtenerFilas($consulta);
				while($fila=$con->fetchAssoc($res))
				{
					if($fila["codArea"]!="")
					{
						$totalNotificados++;
						$consulta="SELECT codigoTelefono FROM 238_paises WHERE idPais=".$fila["codArea"];
						$prefijoCelular=$con->obtenerValor($consulta);
						$resultado=enviarMensajeSMSTwilio($fila["Numero"],$cuerpoMensaje,$prefijoCelular);
				...
				{
					$consulta="UPDATE ".$nombreTabla." SET notificadoSMS=-1 WHERE idRegistro=".$idTarea;
				...
				else
				{
					...
					{
						$consulta="UPDATE ".$nombreTabla." SET notificadoSMS=2 WHERE idRegistro=".$idTarea;
					}
					else
						$consulta="UPDATE ".$nombreTabla." SET notificadoSMS=1 WHERE idRegistro=".$idTarea;
			...
			if($fNotificacion["enviarWhatApp"]==1)
			{
				$arrParametros[0]="No aplica";
				$arrParametros[1]=$tituloNotificacion;
				$arrParametros[2]=trim(strip_tags(str_replace("<br>",". ",str_replace("<br />",". ",$contenidoMensaje))));
				$arrParametros[2]=normalizarEspacios(str_replace("):","): ",$arrParametros[2]));
				$mensajeDestino="*Código único de proceso:* {0}
				...
				*Mensaje:*
				...
				if($con->existeCampo("numeroCarpetaAdministrativa",$nombreTabla))
				{
					...
					$arrParametros[0]=$expediente;
				}
				foreach($arrParametros as $id=>$resto)
				{
					$mensajeDestino=str_replace("{".$id."}",$resto,$mensajeDestino);
				}
				...
				while($fila=$con->fetchAssoc($res))
				{
					if($fila["codArea"]!="")
					{
						...
						$consulta="SELECT codigoTelefono FROM 238_paises WHERE idPais=".$fila["codArea"];
						$prefijoCelular=$con->obtenerValor($consulta);
						$resultado=enviarMensajeWhatsAppTwilio($fila["Numero"],$mensajeDestino,$prefijoCelular);
				...
				if($totalNotificados==0)
				{
					$consulta="UPDATE ".$nombreTabla." SET notificadoWhatsApp=-1 WHERE idRegistro=".$idTarea;
				...
				else
				{
					if($notificacionesEnviadas==0)
					{
						$consulta="UPDATE ".$nombreTabla." SET notificadoWhatsApp=2 WHERE idRegistro=".$idTarea;
					}
					else
						$consulta="UPDATE ".$nombreTabla." SET notificadoWhatsApp=1 WHERE idRegistro=".$idTarea;
	...
	<?php
		...
		if($con->filasAfectadas>0)
		{
			while($filaP=$con->fetchRow($resP))
			{
				...
				while($filaAccion=$con->fetchRow($resPagAccion))
				{
					switch($filaAccion[1])
					{
						case "0"://Nuevo
				...
				$arrEtapas=array();
				while($filaReg=$con->fetchRow($resReg))
				{
				...
				if($relacion=="1")
				{
					...
					$verRegistros=false;
					$idActorAgregar="";
					while($fPermisos=$con->fetchRow($resPermisos))
					{
                ...
                <td>
					<table >
						...
						<td style="padding:7px" colspan="3">
							...
							while($filaCat=$con->fetchRow($resCat))
							{
								$totalCategoria=0;
								...
								if($nEtapas>0)
								{
									while($filasEt=$con->fetchRow($resEt))
									{
    ...
    ?>
                    ...
                    <?php
                        ...
                        while($filaP=$con->fetchRow($resP))
                        {
							...
							$listadoRegistro="-1";
							while($filaActor=$con->fetchRow($resRegistros))
							{
                            ...
                            <td style="padding:7px" colspan="3">
								...
								while($filaCat=$con->fetchRow($resCat))
								{
									...
									<?php	
										...
										while($filasEt=$con->fetchRow($resEtEval))
										{
											$tagA="";
											$tagC="";
											if(isset($arrEtapas[$filasEt[0]]))
...
function obtenerCamposConfiguracion($idFormulario,&$dComp)
{
	...
	$arrConfCampo=array();
	$pos=0;
	while($fcampos=$con->fetchRow($resCampos))
	{
		switch($fcampos[3])
		{
			case 1:
...
function obtenerModulosDisponibles($idProceso,&$investigadores,&$anexos,&$programaTrabajo,&$ademdum,$datosComp,&$invAux)
{
	...
	while($filaEl=$con->fetchRow($resEl))
	{
		switch($filaEl[0])
		{
			case "3":
...
<?php
	if($anexos)
	{
		...
		<tr>
			...
			while($filaArch=$con->fetchRow($resArch))
			{
			?>
				<tr>
				<td>
	...
	if($ademdum)
	{
		...
		<tr>
			...
			$resArch=$con->obtenerFilas($consulta);
			while($fila=$con->fetchRow($resArch))
			{
			...
			<?php
               ...
               <?php		
				...
				else
				{
					...
					$pos=0;
					$arrPosLlave=array();
					while($filaReg=$con->fetchAssoc($resReg))
					{
					...
					for($nFilaReg=$nInicio;$nFilaReg<$nFinFilaReg;$nFilaReg++)
					{
						...
						$filaReg=$con->fetchAssoc($rRegistro);	
					...
					if($nReg>0)
					{
						...
						
						{
							...
							if($investigadores)
							{
								...
								$invPrincipal="<table>";
								while($filaInv=$con->fetchRow($resInv))
								{
											...
											<?php
												...
												if(!$sl)
												{
													if(($relacion=="1")&&($ingresarComoRol))
													{
														...
														if($con->filasAfectadas>0)
														{
															while($filaVAct=$con->fetchRow($resVAct))
															{
										 ...
										 <tr>
											...
											<?php
												...
												$consulta="SELECT tituloAccion,accion,ac.ayuda,icono,funcionValidacion,tipoFuncion,resultadoMuestra,condicionEval,ap.leyenda,ap.ayuda,ac.msgConfirm,ap.msgConfirm,idAccionComplementaria 
													...
													while($filaAccion=$con->fetchRow($resAccionComp))
													{
...
//tipoEventos= 0 considerar todos los tipos;"" No considerar eventos institucionales;>0 el tipo de evnto a considerar separado por ,
{
	...
	if($considerarClases)
	{
		...
		while($filaClase=$con->fetchRow($resSesionClase))
		{
	...
	if($tipoEventos!="")
	{
		...
		while($filaEvento=$con->fetchRow($resEvento))
		{
...
function actualizarDireccion($idUsuarioF,$idUsuarioD,$ignorarUsuario="-1")
{
	...
	if($con->ejecutarConsulta($consulta))		
	{
		...
		if($con->ejecutarConsulta($consulta))
		{
			...
			while($fTel=$con->fetchRow($resTel))
			{
...
function calcularHorasComprometidas($idUsuario,$fIni,$fFin,&$desgloceTiempo)
{
	...
	$res=$con->obtenerFilas($consulta);	
	$totalHoras=0;
	while($filaAct=$con->fetchRow($res))
	{
		switch($filaAct[1])
		{
			case 1:
...
function existeBloque($horario,$arrHorarioDia,$idMateria,$idGrupo,$idDia)
{
	...
	if($validarSituacion)
	{
		...
		{
			while($matBloq=$con->fetchRow($bloquesMateria))
			{
...
function validarHorarioMateria($idMateria,$idPerfil,$idGrupo)
{
	...
	if($tipoHorarioMat==1)
	{
		...
		while($filaG=$con->fetchRow($grupos))
		{
			$arrDias=array();
			...
			while($filaH=$con->fetchRow($bloquesMatGrupo))
			{
        ...
        <td>
            <table >
                ...
                <td style="padding:7px" colspan="3">
					...
					$totalPF=0;
					$totalPM=0;
                    while($filaEdo=$con->fetchRow($resEdo))
                    {
						$tagC1="";
						$tagA1="";
						$tagC2="";
						...
						$listaCredPFisica="";
						while($filaCred=$con->fetchRow($resCred))
						{
							if($listaCredPFisica=="")
...
<?php		
	...
	while($fila=$con->fetchRow($resListado))	
	{
?>
	<table>
    <tr>
...
function generarExpresionPHP($idConsulta,&$arrDependencias)
{
	...
	$cad='';	
	while($fila=$con->fetchRow($resQuery))
	{
		if($fila[3]<0)
		{
			$arrToken=array();
		  ...
		  switch($tipoConsulta)
		  {
			  ...
			  case 3:
				  ...
				  if(isset($dependencia[2]))
				  {
					  ...
					  while($filaParam=$con->fetchRow($resParam))
					  {
...
function realizarCalculosIndividuales(&$obj,$arrCalculosDef,$arrAcumuladores)
{
	...
	while($filaCalculo=$con->fetchRow($resCalculos))
	{
		...
		if($considerar)
		{
			...
			while($filaAcumulador=$con->fetchRow($resAcum))
			{
			...
			$objCuenta=array();
			while($filaAfectCuentas=$con->fetchRow($resAfectCuentas))
			{
...
function obtenerMateriasHijas($idMateria,&$arrMaterias,$considerarContenedores=false)
{
	...
	while($filasMat=$con->fetchRow($resMat))
	{
...
function generarCuerpoExpresion($idProceso,$numEtapa,$idFormulario,$idRegistro,$idPerfil=-1,$idActorProceso=0)
{
	global $con;
	$cadena="";
	...
	$arrDatosProceso["idActorProceso"]=$idActorProceso;
	...
	while($filaCampo=$con->fetchRow($resCampo))
	{
	...
	while($filaCon=$con->fetchRow($resConAux))
	{
					...
					switch($filaParam[1])
					{
						...
						case "16":
							switch($filaParam[0])
							{
								...
								break;
								case "5":
									$valor=$arrDatosProceso["idActorProceso"];
								break;
							}
						break;
	...
	if($idPerfil==-1)
		$consulta="select idConsulta,formatoGuardado from 991_consultasSql where (comp1='".$idProceso."' and (comp3='' or comp3 is null) and (comp2='".$numEtapa."' or cast(comp2 as decimal(11,4))=".$numEtapa."))";
	else
		$consulta="select idConsulta,formatoGuardado from 991_consultasSql where (comp1='".$idProceso."' and comp3='".$idPerfil."' and (comp2='".$numEtapa."' or cast(comp2 as decimal(11,4))=".$numEtapa."))";
	
	$fConsulta=$con->obtenerPrimeraFilaAsoc($consulta);
	
	$idConsulta=$fConsulta["idConsulta"];
	$formatoGuardado=$fConsulta["formatoGuardado"];
	if($idConsulta=="")
		return $cadena;
	...
	{	
		...
		if($formatoGuardado==2)
		{
			$fila[0]=str_replace("?>","",str_replace("<?php","",bD($fila[0])));
		}
	...
	switch($tipo)
	{
		...
		case "21":
			...
			if($objConsulta["ejecutado"]==1)
			{
				...
				{
					$filaRef=$con->fetchRow($objConsulta["resultado"]);
					if($filaRef)
					{
...
function funcValidarHorarioMateriaCurso($horaI,$horaF,$dia,$idMateria,$idGrupo,$idCiclo,$id)
{
	...
	{
		$sumatoriaMinutos=0;
		while($hMat=$con->fetchRow($resHorMat))			
		{
...
function validarModificacionHorarioDisponibleDocenteExtendido($dia,$horaI,$horaF,$idCiclo,$idUsuario,$idPeriodo,$idInstancia)
{
	...
	else
	{
		...
		while($fila=$con->fetchRow($resHorarios))
		{
...
function validarModificacionHorarioProfExtendido($idUsuario,$horaI,$horaF,$dia,$idCiclo,$idMateria,$idGrupo,$formato=1,$fAplicacion=NULL,$objComp=NULL)
{
	...
	else
	{
		...
		while($fila=$con->fetchRow($filas))	
		{
			  ...
			  while($filaMatP=$con->fetchRow($respuesta))
			  {
...
function validarModificacionHorarioAlumnosExtendido($idMateria,$idGrupo,$idCiclo,$dia,$hIni,$hFin,$formato=1,$fAplicacion=NULL)
{
	...
	else
	{
		...
		while($fila=$con->fetchRow($res))
		{
			...
			while($filaMat=$con->fetchRow($resAlum))
			{
				...
				while($filaAlum=$con->fetchRow($resMatAlum))
				{
...
function obtenerProgramasInstitucionalesVigentes($idCiclo,$formato) //1 arreglo php,2 arreglo java
{
	...
	while($fAux=$con->fetchRow($resDef))
	{
		...
		while($fPrograma=$con->fetchRow($resProgramas))
		{
...
function registrarDocumentoServidorRepositorio($idDocumento,$nombreDocumento,$idCategoria=6,$descripcion="")
{
	...
	$consulta="SELECT origenRepositorioDocumentos FROM 903_variablesSistema";
	$origenRepositorioDocumentos=$con->obtenerValor($consulta);
	if($origenRepositorioDocumentos=="")
		$origenRepositorioDocumentos=0;
	...
	procesarDocumentoRepositorioOCRText($idArchivo);
	if(($origenRepositorioDocumentos>0&&(!$guardarArchivosBD)))
	{

		$conexion=generarInstanciaConectorGestor($origenRepositorioDocumentos);
		$rutaDirectorio=$conexion->raizServidor."/repositorioDocumentos/documentosUsuario";//Modificado
		$nombreDocumento=$idArchivo." ".str_replace("_"," ",$binario_nombre);
		$contenidoDocumento=leerContenidoArchivo($directorioInstalacion."/repositorioDocumentos/documento_".$idArchivo);
		$eliminarDocumento=false;
		$resObj=$conexion->crearDocumento($rutaDirectorio,$nombreDocumento,$contenidoDocumento);
		
		if($resObj)
		{
			$idDocumentRepo=$resObj->id;
			$consulta="UPDATE 908_archivos SET documentoRepositorio='".$idDocumentRepo."',repositorioDocumento=".$origenRepositorioDocumentos." WHERE idArchivo=".$idArchivo;
		...
		{
			unlink($directorioInstalacion."/repositorioDocumentos/documento_".$idArchivo);
			/*if(file_exists($directorioInstalacion."/repositorioDocumentos/documento_".$idArchivo.".pkcs7"))
			{
				
				unlink($directorioInstalacion."/repositorioDocumentos/documento_".$idArchivo.".pkcs7");
			}*/
		}
	}
