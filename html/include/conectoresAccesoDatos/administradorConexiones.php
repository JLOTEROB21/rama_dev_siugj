<?php
	...
	function resolverQueries($id,$tipoDataSet,$paramDatos,$pNombreCampoCompleto=false,$vRegistros=false,$ejecutarConsultas=false,$conBD=NULL)
	{
		...
		$tablaBaseName="_".$id."_tablaDinamica";
		...
		$consulta="select nodosFiltro,camposProy,operacion,nTabla,parametros,idDataSet,tipoAlmacen,nombreDataSet,orden,complementario,idConexion,obtenerValoresDistintos,numRegistros from 
				9014_almacenesDatos where idReporte=".$id." and tipoDataSet in(".$tipoDataSet.") and tipoAlmacen<>2 order by idDataSet asc";
				...
				if((!isset($arrResultConsulta[$filaCon[5]]))||(!isset($arrResultConsulta[$filaCon[5]]["tipoAlmacen"])))
				{
							...
							else
							{
								...
								switch($filaParam[1])
								{
										...
										if(($filaParam[0]<$filaCon[5])||(isset($arrResultConsulta[$filaParam[0]])))
										{
											...
											if(isset($arrResultConsulta[$filaParam[0]]))
											{
												if($arrResultConsulta[$filaParam[0]]["ejecutado"]==1)
												{
													...
													{
														$valor=$filaRes[str_replace(".","_",$arrDatos[1])];
													}
												}
												else
													$valor='-100584';
									...
									case "16": //Valores de proceso
										$existeValor=false;
										eval('$existeValor=isset($paramDatos->p16->p'.$filaParam[0].');');
										
										
										if($existeValor)
										{
											eval('$valor=$paramDatos->p16->p'.$filaParam[0].';');
										}
										else
										{
											
											$existeValor=!is_numeric($filaParam[0]);
											if($existeValor)
												eval('$existeValor=isset($paramDatos->p16->'.$filaParam[0].');');
											if($existeValor)
											{
												eval('$valor=$paramDatos->p16->'.$filaParam[0].';');
											}
											else
											{
												switch($filaParam[0])
												{
													case "1":
														$valor=$paramDatos->p17->iFormulario;
													break;
													case "2":
														$valor=$paramDatos->p17->idProceso;
													break;
													case "3":
														$valor=$paramDatos->p17->iRegistro;
													break;
													case "4":
														$valor=$paramDatos->p17->etapa;
													break;
													case "5":
														$valor=$paramDatos->p17->idActorProceso;
													break;
												}
											}
							...
							switch($dToken[0])
							{
										...
										if($filaCon[10]==0)
										{
											switch($dToken[1])
											{
												...
												case 3://Institución de usuario autenticado
													$valor="";
													if($dToken[1]==1)
														$valor=$tablaBaseName.".responsable";
													else
														if($dToken[1]==2)
															$valor=$tablaBaseName.".codigoUnidad";
														else
															$valor=$tablaBaseName.".codigoInstitucion";
													if(strpos($codMysql,' like ')===false)
													{
														$codMysql=str_replace('@'.$filaSesion[1],$valor,$codMysql);
								...
								case 16:  //Valor de proceso
									...
									if((!$vRegistros)||($dToken[1]==1)||($dToken[1]==2))
									{
										...
										$valor="";
										eval('$existeValor=isset($paramDatos->p16->p'.$dToken[1].');');
										
										
										if($existeValor)
										{
											eval('$valor=$paramDatos->p16->p'.$dToken[1].';');
										}
										else
										{
											switch($dToken[1])
					...
					if(($filaCon[8]!="")&&(!$vRegistros))
					{
						...
						foreach($objOrden as $orden)	
						{
							...
							if((strpos($arrCampo[0],"tablaDinamica")!==false) &&isset($arrCamposFrmTablas[$arrCampo[1]]))
					...
					{
						//echo "Espera resolver: ".$filaCon[5]."<br>";
					}
				}
			}
