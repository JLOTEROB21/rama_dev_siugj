...
function registrarAlertaNotificacionSistema($arrValores)
{
	...
	if($recordarDiasAnticipacion>0)
	{
		if($intervaloRecordario==1)
			$fechaRecordatorio=date("Y-m-d H:i:s",strtotime("- ".$recordarDiasAnticipacion." days",strtotime($arrValores["fechaAlerta"])));
		else
			$fechaRecordatorio=date("Y-m-d H:i:s",strtotime("- ".$recordarDiasAnticipacion." hours",strtotime($arrValores["fechaAlerta"])));
