...
class cSendMail
{
	...
	function obtenerCorreosBandeja($bandeja,$criterio="ALL")
	{
		imap_reopen($this->conexion,$bandeja);
		$emails = imap_search($this->conexion,$criterio,SE_UID);
	...
	{
		$mensaje = "";
		$header = imap_header($this->conexion, imap_msgno($this->conexion,$iMail));
		$structure = imap_fetchstructure($this->conexion, $iMail,FT_UID);
		
		if ($this->esEmailMultiparte($structure))
		{

			$mensaje = imap_fetchbody($this->conexion,$iMail,"1.1",FT_UID); ## GET THE BODY OF MULTI-PART MESSAGE
			if(!$mensaje) 
			{
				$mensaje = '';
			}
	...
	function obtenerCuerpoMailRaw($iMail,$escaparEnter=false)
	{
		$mensaje = "";
		$header = imap_header($this->conexion, imap_msgno($this->conexion,$iMail));
		$structure = imap_fetchstructure($this->conexion, $iMail,FT_UID);
	...
	function obtenerRemitente($iMail)
	{
		...
		$asunto=imap_mime_header_decode($cabecera->from);
		$texto="";
		foreach($asunto as $a)
		{
			$texto.=$a->text;
		}
		$arrMail=explode("<",$texto);
		if(sizeof($arrMail)>1)
		{
	...
	{
		$cabecera=$this->obtenerCabeceraMail($iMail);
		$asunto=imap_mime_header_decode($cabecera->to);
		$texto="";
		foreach($asunto as $a)
		{
			$texto.=$a->text;
	...
	{
		$cabecera=$this->obtenerCabeceraMail($iMail);
		$asunto=imap_mime_header_decode($cabecera->subject);
		$texto="";
		foreach($asunto as $a)
		{
			$texto.=$a->text;
