...
//include_once("PdfToText/PdfToText.php");
...
function actualizarGridFigurasJuridicas($idFormulario,$idRegistro)
{
	...
	if($con->existeTabla("_47_chParticipacionJuridica"))
	{
		...
		while($fParticipacion=$con->fetchRow($rParticipaciones))
		{
		...
		while($fPersona=$con->fetchRow($rPersona))
		{
			...
			switch($fFiguras[0])
			{
				...
				case 4:
					...
					while($fDelito=$con->fetchRow($resDelitos))
					{
...
function convertirDocumentoUsuarioDocumentoResultadoProceso($idDocumentoBase,$idFormulario,$idRegistro,$nombreFinal,$tipo,$tipoResultado=2)
{
	global $con;
	global $baseDir;
		...
		if(	$con->ejecutarConsulta($consulta))
		{
			...
			if(($idFormulario!=-1)&&($idRegistro!=-1))
			{
				if($tipoResultado==2)
					...
					registrarDocumentoReferenciaProceso($idFormulario,$idRegistro,$idDocumentoBase);
			}
			return true;
		}
...
function registrarDocumentosConfiguradosSolicitudAudiencia($idFormulario,$idRegistro)
{
	...
	while($fila=$con->fetchAssoc($res))
	{
		...
		else
		{
			...
			while($fFigura=$con->fetchRow($rFigura))
			{
				$considerar=true;		
				if($fila["funcionAplicacion"]!="")
				{
...
function registrarDocumentoResultadoProcesoPadre($idFormulario,$idRegistro)
{
	...
	$res=$con->obtenerFilas($consulta);
	while($fDocumento=$con->fetchRow($res))
	{
...
function registrarSolicitudDefensorPublico($idFormulario,$idRegistro)
{
	...
	if($tipoFormato==24)
	{
		...
		while($fila=$con->fetchRow($resImputados))
		{
...
function registrarSolicitudTrasladoImputado($idFormulario,$idRegistro)
{
	...
	while($fParticipante=$con->fetchRow($resParticipante))
	{
...
function ejecutarAccionAsignacionDocumentoAtencionUsuario($idFormulario,$idRegistro)
{
	...
	if($fDocumento["tipoDocumento"]==141)
	{
		...
		while($fDocumentoAux=$con->fetchRow($resDocumento))
		{
...
function enviarCorreoNotificaciones($idFormulario,$idRegistro)
{
	...
	if(!$prueba)
	{
		...
		while($fMail=$con->fetchRow($rMail))
		{
...
function clonarSolicitudAudiencia($idRegistro,$iFormulario,$iRegistro,$motivoRemision,$arrParametros=NULL)
{
	...
	$res=$con->obtenerFilas($consulta);
	while($fDelito=$con->fetchAssoc($res))
	{	
	...
	while($fParticipante=$con->fetchAssoc($resParticipante))
	{
...
function obtenerFechaAudienciaSolicitudAmparo($idFormulario,$idRegistro,$idReferencia,$tipoAudiencia,$nivel=4)
{
	...
	if((!$fEventoAudiencia)||($fEventoAudiencia[2]==""))
	{
		...
		while($fJueces=$con->fetchRow($rJueces))
		{
			$oJuez=array();
...
function generarFolioCarpetaTribunalEnjuciamiento($idFormulario,$idRegistro)
{
	...
	while(!$encontrado)
	{
		...
		if($con->filasAfectadas>0)
		{
			...
			while($fTribunal=$con->fetchRow($rTribunales))
			{
...
function obtenerResponsablePenitenciaria($idFormulario,$idRegistro,$actorDestinatario)
{
	...
	while($fUsuario=$con->fetchRow($rUsuarios))
	{
...
function generarNotificacionMedidaCautelar($idEvento)
{
	...
	while($fParticipante=$con->fetchRow($resParticipante))
	{
		...
		while($fDelitos=$con->fetchRow($rDelitos))
		{
			$oDelito='{
		...
		while($fAlias=$con->fetchRow($rAlias))
		{
			$a='{"alias":"'.cv($fAlias[0]).'"}';
			if($arrAlias=="")
				$arrAlias=$a;
		...
		while($fTelefono=$con->fetchRow($rTelefono))
		{
			$o='{
					"lada":"'.$fTelefono[1].'", 
					"numero":"'.$fTelefono[2].'", 
		...
		while($fMedidaCautelar=$con->fetchRow($rMedidaCautelar))
		{
			$oMedida="";
	...
	{	
		...
		while($fImputado=$con->fetchRow($rImputadoDefendido))
		{
			$i='{
...
function obtenerCarpetasDerivadas($carpeta,$tipoCarpeta=-1)
{
	...
	while($filaCarpeta=$con->fetchRow($resCarpeta))
	{
...
function buscarCoincidenciasCriterio($tipoCriterio,$valor,$porcentaje,$figuraJuridica=0,$idFormulario=46,$idEstado=1)
{
	...
	switch($tipoCriterio)
	{
		case 1://Por nombre participante
			...
			while($fila=$con->fetchRow($rResultados))
			{
...
function obtenerResolutivosAudienciaHistorial($idEvento)
{
	...
	while($fila=$con->fetchRow($rEvento))
	{
		$tipoValor=$fila[4];
		$oResolutivo=array();
...
function autorizarRecepcionCarpetaEjecucionV2($idFormulario,$idRegistro)
{
	...
	while($fSentencia=$con->fetchRow($rSetenciado))
	{
...
function obtenerCarpetasHijas($cA)
{
	global $con;
	$arrHijos="";
	$consulta="SELECT * FROM 7006_carpetasAdministrativas WHERE carpetaAdministrativaBase='".$cA."'";
	$res=$con->obtenerFilas($consulta);
	while($fila=$con->fetchAssoc($res))
	{
		$arrHijos2="[".obtenerCarpetasHijas($fila["carpetaAdministrativa"])."]";	
		
		$obj='{"cls":"expedienteDerivado",tipoCarpeta:"12","idPerfilAcceso":"'.$fila["idPerfilAcceso"].'",expanded:true,"icon":"../images/s.gif","id":"'.$fila["carpetaAdministrativa"].'","text":"'.$fila["carpetaAdministrativa"].'",children:'.$arrHijos2.',leaf:'.(($arrHijos2=="[]")?"true":"false").'}';
		if($arrHijos=="")
			$arrHijos=$obj;
		else
			$arrHijos.=",".$obj;
...
function generarEnvioCopiaSentencia($idFormulario,$idRegistro)
{
	...
	while($fSentenciado=$con->fetchRow($rSentenciado))
	{
		$arrValores=array();
		...
		while($fCentroDetencion=$con->fetchRow($rCentroDetencion))
		{
...
function registrarAlertaNotificacionSistema($arrValores)
{
	...
	$recordarDiasAnticipacion=0;
	if(isset($arrValores["diasRecordatorios"]))
		$recordarDiasAnticipacion=$arrValores["diasRecordatorios"];
	
	$intervaloRecordario=1;
	if(isset($arrValores["intervaloRecordario"]))
		$intervaloRecordario=$arrValores["intervaloRecordario"];
	
	$x=0;
	$consulta[$x]="begin";
	$x++;
	
	if($con->existeCampo("recordarDiasAnticipacion","7036_alertasNotificaciones"))
	{
		$consulta[$x]="INSERT INTO 7036_alertasNotificaciones(carpetaAdministrativa,situacion,descripcion,valorReferencia1,valorReferencia2,
				fechaRegistro,responsableRegistro,tipoAlerta,fechaAlerta,idTitularAlerta,recordarDiasAnticipacion) values('".$arrValores["carpetaAdministrativa"]."',1,'".cv($arrValores["descripcion"]).
				...
				".$arrValores["tipoAlerta"].",'".$arrValores["fechaAlerta"]."',".$idTitularAlerta.",".$recordarDiasAnticipacion.")";
		$x++;
	}
	else
	{
	...
	if($recordarDiasAnticipacion>0)
	{
		if($intervaloRecordario==1)
		{
			$fechaRecordatorio=date("Y-m-d H:i:s",strtotime("- ".($recordarDiasAnticipacion*1)." days",strtotime($arrValores["fechaAlerta"])));
		}
		else
			$fechaRecordatorio=date("Y-m-d H:i:s",strtotime("- ".($recordarDiasAnticipacion*1)." hours",strtotime($arrValores["fechaAlerta"])));
		$consulta[$x]="INSERT INTO 7037_recordatoriosPreviosNotificacion(fechaRecordatorio,idAlertaNotificacion) VALUES('".$fechaRecordatorio."',@idRegistro)";
		$x++;
	}
...
{
	global $con;
	$consulta="SELECT COUNT(*) FROM _17_tablaDinamica WHERE claveUnidad='".$_SESSION["codigoInstitucion"]."'";
	
	if($con->existeCampo("cmbCategoria","_17_tablaDinamica"))
		$consulta.=" AND cmbCategoria=1";
	return $con->obtenerValor($consulta);
}
...
function aplicarRemisionCarpetaJudicial($idFormulario,$idRegistro)
{
	...
	while($fAudiencias=$con->fetchAssoc($rAudiencias))
	{
...
function determinarRutaIncompetencia($idFormulario,$idRegistro)
{
	...
	while($fila=$con->fetchAssoc($res))
	{
		...
		while($fDocumento=$con->fetchRow($resDocs))
		{
...
function obtenerSiguienteAsignacionObjeto($arrConfiguracion)
{
	...
	$metodoAsignacion=isset($arrConfiguracion["metodoAsignacion"])?$arrConfiguracion["metodoAsignacion"]:"2";	//1 Aleatorio;2 Secuencial
	$ciclos=0;
	$objConf=serialize($arrConfiguracion);
	$noRonda=obtenerNoRondaAsignacionObjeto($tipoAsignacion,$serieRonda,$idObjetoReferencia);

	$idRegistroAsignacion=-1;
	
	
	$cadParametros='{"fechaReferencia":"'.date("Y-m-d").'","codigoUnidad":""';
	if(isset($arrConfiguracion["objParametrosBase"]))
	{
		foreach($arrConfiguracion["objParametrosBase"] as $campo=>$valor)
		{
	...
	while($ciclos<10)
	{
		...
		foreach($aUniverso as $idUnidad)
		{
			...
			$arrUniverso[$idUnidad]["idUnidad"]=$idUnidad;
		...
		foreach($arrUniverso as $idUnidad=>$resto)
		{
			if(($resto["nAdeudos"]>0)&&(($limitePagoRonda==0) ||($resto["nPagadas"]<$limitePagoRonda)))
			{
				...
				{
					if(strpos($funcValidacionPagoDeuda,"[")===false)
					{
					...
					{
						$cache=NULL;
						$objParametros->codigoUnidad=$idUnidad;
						$idFuncion=str_replace("[","",str_replace("]","",$funcValidacionPagoDeuda));
						$resultado=resolverExpresionCalculoPHP($idFuncion,$objParametros,$cache);
						$aplicaPagoDeuda=$resultado[0];
					}
				...
				if($aplicaPagoDeuda)
				{
					$consulta="SELECT  idAsignacion,AES_DECRYPT(UNHEX(noRonda), '".bD($versionLatis)."') as noRonda 
							FROM 7001_asignacionesObjetos WHERE tipoAsignacion=HEX(AES_ENCRYPT('".$tipoAsignacion."', '".bD($versionLatis)."'))  
							AND tipoRonda=HEX(AES_ENCRYPT('".$serieRonda."', '".bD($versionLatis)."')) 
							AND idUnidadReferida=HEX(AES_ENCRYPT('".$idUnidad."', '".bD($versionLatis)."')) 
							 and idObjetoReferido=HEX(AES_ENCRYPT('".$idObjetoReferencia."', '".bD($versionLatis)."')) 
					...
					if($escribirAsignacion)
					{
						...
						{
							//
							$consulta="UPDATE 7001_asignacionesObjetos SET situacion=3,rondaPagada=HEX(AES_ENCRYPT('".$noRonda."', '".bD($versionLatis).
									"')),objParametros=HEX(AES_ENCRYPT('".cv($objConf)."', '".bD($versionLatis)."')) WHERE idAsignacion=".$idAsignacionPagada;
							$con->ejecutarConsulta($consulta);
							$consulta="INSERT INTO 7001_asignacionesObjetos(idFormulario,idRegistro,idObjetoReferido,idUnidadReferida,fechaAsignacion,
									tipoRonda,noRonda,situacion,rondaPagada,comentariosAdicionales,tipoAsignacion,idAsignacionPagada,objParametros) 
									values(".$arrConfiguracion["idFormulario"].",".$arrConfiguracion["idRegistro"].
									",HEX(AES_ENCRYPT('".$idObjetoReferencia."', '".bD($versionLatis)."')),HEX(AES_ENCRYPT('".$idUnidad."', '".bD($versionLatis)."')),'".
									date("Y-m-d H:i:s")."',HEX(AES_ENCRYPT('".$serieRonda."', '".bD($versionLatis)."')),HEX(AES_ENCRYPT('".$noRonda."', '".bD($versionLatis).
									"')),1,NULL,'',HEX(AES_ENCRYPT('".$tipoAsignacion."', '".bD($versionLatis)."')),HEX(AES_ENCRYPT('".$idAsignacionPagada."', '".bD($versionLatis).
									"')),HEX(AES_ENCRYPT('".cv($objConf)."', '".bD($versionLatis)."')))";	
							$con->ejecutarConsulta($consulta);
						...
						else
						{
							...
							$consulta="UPDATE 7001_asignacionesObjetos SET situacion=1,rondaPagada=HEX(AES_ENCRYPT('".$noRonda."', '".bD($versionLatis)."'))".
										",idFormulario=".$arrConfiguracion["idFormulario"].",idRegistro=".$arrConfiguracion["idRegistro"].
										",fechaAsignacion='".date("Y-m-d H:i:s").
										"',comentariosAdicionales='',objParametros=HEX(AES_ENCRYPT('".cv($objConf)."', '".bD($versionLatis).
										"')) WHERE idAsignacion=".$idAsignacionPagada;
							$con->ejecutarConsulta($consulta);
		...
		if($metodoAsignacion==1)//Aleatorio
		{
			$arrUniversoAux=array();
			foreach($arrUniverso as $resto)
			{
				array_push($arrUniversoAux,$resto);
				
			}
			shuffle($arrUniversoAux);
			$arrUniverso=array();
			foreach($arrUniversoAux as $resto)
			{
				$arrUniverso[$resto["idUnidad"]]=$resto;
		...
		foreach($arrUniverso as $idUnidad=>$resto)
		{
			if($resto["nAsignaciones"]==0)
			{
				...
				if($funcValidacionSeleccion!="")
				{
					...
					if(strpos($funcValidacionSeleccion,"[")===false)
					{
					...
					else
					{
						$cache=NULL;
						$objParametros->codigoUnidad=$idUnidad;
						$idFuncion=str_replace("[","",str_replace("]","",$funcValidacionSeleccion));
						$aplicaAsignacion=resolverExpresionCalculoPHP($idFuncion,$objParametros,$cache);
				...
				if($aplicaAsignacion)
				{
					$consulta="SELECT  idAsignacion FROM 7001_asignacionesObjetos WHERE tipoAsignacion=HEX(AES_ENCRYPT('".$tipoAsignacion.
							"', '".bD($versionLatis)."')) AND tipoRonda=HEX(AES_ENCRYPT('".$serieRonda."', '".bD($versionLatis).
							"')) AND idUnidadReferida=HEX(AES_ENCRYPT('".$idUnidad."', '".bD($versionLatis).
							"'))  and idObjetoReferido=HEX(AES_ENCRYPT('".$idObjetoReferencia."', '".bD($versionLatis).
							"'))  and situacion=2 and noRonda=HEX(AES_ENCRYPT('".$noRonda."', '".bD($versionLatis).
							"'))";
					...
					if($escribirAsignacion)
					{
						...
						{
							$consulta="INSERT INTO 7001_asignacionesObjetos(idFormulario,idRegistro,idObjetoReferido,idUnidadReferida,fechaAsignacion,
									tipoRonda,noRonda,situacion,comentariosAdicionales,tipoAsignacion,idAsignacionPagada,objParametros) values(".
									$arrConfiguracion["idFormulario"].",".$arrConfiguracion["idRegistro"].",HEX(AES_ENCRYPT('".$idObjetoReferencia."', '".bD($versionLatis).
									"')),HEX(AES_ENCRYPT('".$idUnidad."', '".bD($versionLatis)."')),'".date("Y-m-d H:i:s")."',
									HEX(AES_ENCRYPT('".$serieRonda."', '".bD($versionLatis)."')),HEX(AES_ENCRYPT('".$noRonda."', '".bD($versionLatis)."')),1,
									HEX(AES_ENCRYPT('".cv($comentariosAdicionales)."', '".bD($versionLatis)."')),HEX(AES_ENCRYPT('".$tipoAsignacion."', '".bD($versionLatis)."'))
									,HEX(AES_ENCRYPT('-1', '".bD($versionLatis)."')),HEX(AES_ENCRYPT('".cv($objConf)."', '".bD($versionLatis)."')))";	
							$con->ejecutarConsulta($consulta);
							$idRegistroAsignacion=$con->obtenerUltimoID();
						}
						else
						{
							...
							if($aplicaPagoDeuda)
							{
								$consulta="UPDATE 7001_asignacionesObjetos SET situacion=1,rondaPagada=HEX(AES_ENCRYPT('".$noRonda."', '".bD($versionLatis).
											"')),idFormulario=".$arrConfiguracion["idFormulario"].",idRegistro=".$arrConfiguracion["idRegistro"].
										",fechaAsignacion='".date("Y-m-d H:i:s")."',comentariosAdicionales=HEX(AES_ENCRYPT('".cv($comentariosAdicionales).
										"', '".bD($versionLatis)."')),objParametros=HEX(AES_ENCRYPT('".cv($objConf)."', '".bD($versionLatis).
										"')) WHERE idAsignacion=".$idAsignacionPagada;
				...
				{	
					$consulta="INSERT INTO 7001_asignacionesObjetos(idFormulario,idRegistro,idObjetoReferido,idUnidadReferida,fechaAsignacion,
									tipoRonda,noRonda,situacion,comentariosAdicionales,tipoAsignacion,idAsignacionPagada,objParametros) values(".
									$arrConfiguracion["idFormulario"].",".$arrConfiguracion["idRegistro"].",HEX(AES_ENCRYPT('".$idObjetoReferencia."', '".bD($versionLatis).
									"')),HEX(AES_ENCRYPT('".$idUnidad."', '".bD($versionLatis)."')),'".date("Y-m-d H:i:s")."',HEX(AES_ENCRYPT('".$serieRonda."', '".bD($versionLatis).
									"')),HEX(AES_ENCRYPT('".$noRonda."', '".bD($versionLatis)."')),2,HEX(AES_ENCRYPT('".cv($comentariosAdicionales)."', '".bD($versionLatis).
									"')),HEX(AES_ENCRYPT('".$tipoAsignacion."', '".bD($versionLatis)."')),HEX(AES_ENCRYPT('-1', '".bD($versionLatis)."')),HEX(AES_ENCRYPT('".cv($objConf)."', '".bD($versionLatis)."')))";	
					$con->ejecutarConsulta($consulta);
...
function obtenerAsignacionesRondaObjeto($idUnidadReferedida,$tipoAsignacion,$tipoRonda,$noRonda,$idObjetoReferencia=-1,$considerarAsigancionesCanceladas=false)
{
	global $con;
	global $versionLatis;
	$consulta="SELECT  COUNT(*) FROM 7001_asignacionesObjetos WHERE tipoAsignacion=HEX(AES_ENCRYPT('".$tipoAsignacion."', '".bD($versionLatis).
			"')) AND tipoRonda=HEX(AES_ENCRYPT('".$tipoRonda."', '".bD($versionLatis)."')) AND idUnidadReferida=HEX(AES_ENCRYPT('".$idUnidadReferedida."', '".bD($versionLatis)."'))
			 and idObjetoReferido=HEX(AES_ENCRYPT('".$idObjetoReferencia."', '".bD($versionLatis)."')) and noRonda=HEX(AES_ENCRYPT('".$noRonda."', '".bD($versionLatis)."'))
			 and situacion in(".($considerarAsigancionesCanceladas?"1,2,10":"1").") and (idAsignacionPagada is null or idAsignacionPagada=HEX(AES_ENCRYPT('-1', '".bD($versionLatis)."')))";
...
function obtenerAsignacionesPendientesObjeto($idUnidadReferedida,$tipoAsignacion,$tipoRonda,$noRonda,$considerarDeudasMismaRonda=true,$idObjetoReferencia=-1)
{
	...
	$consulta="SELECT  COUNT(*) FROM 7001_asignacionesObjetos WHERE tipoAsignacion=HEX(AES_ENCRYPT('".$tipoAsignacion."', '".bD($versionLatis).
			"')) AND tipoRonda=HEX(AES_ENCRYPT('".$tipoRonda."', '".bD($versionLatis)."')) AND idUnidadReferida=HEX(AES_ENCRYPT('".$idUnidadReferedida."', '".bD($versionLatis)."'))
			 and idObjetoReferido=HEX(AES_ENCRYPT('".$idObjetoReferencia."', '".bD($versionLatis)."')) and situacion=2";
...
function obtenerAsignacionesPagadasRondaObjeto($idUnidadReferedida,$tipoAsignacion,$tipoRonda,$noRonda,$idObjetoReferencia=-1)
{
	...
	global $versionLatis;
	$consulta="SELECT  COUNT(*) FROM 7001_asignacionesObjetos WHERE tipoAsignacion=HEX(AES_ENCRYPT('".$tipoAsignacion."', '".bD($versionLatis).
			"')) AND tipoRonda=HEX(AES_ENCRYPT('".$tipoRonda."', '".bD($versionLatis)."')) AND idUnidadReferida=HEX(AES_ENCRYPT('".$idUnidadReferedida.
			"', '".bD($versionLatis)."')) and idObjetoReferido=HEX(AES_ENCRYPT('".$idObjetoReferencia."', '".bD($versionLatis).
			"')) and noRonda=HEX(AES_ENCRYPT('".$noRonda."', '".bD($versionLatis).
			"')) and situacion=1 and idAsignacionPagada is not null and idAsignacionPagada<>HEX(AES_ENCRYPT('-1', '".bD($versionLatis)."'))";
	$nAsignaciones=$con->obtenerValor($consulta);
...
function obtenerUltimoDomicilioFiguraJuridica($idParticipante)
{
	...
	$arrTelefonos="";
	$consulta="SELECT tipoTelefono,lada,numero,extension,idPais,verificado FROM ".$tblTelefono." WHERE idReferencia=".$fila["idRegistro"];

	$res=$con->obtenerFilas($consulta);
	while($f=$con->fetchRow($res))
	{
		$oM='{"tipoTelefono":"'.$f[0].'","lada":"'.$f[1].'","numero":"'.$f[2].'","extension":"'.$f[3].
			'","idPais":"'.$f[4].'","verificado":"'.$f[5].'"}';
		if($arrTelefonos=="")
			$arrTelefonos=$oM;
		else
			$arrTelefonos.=",".$oM;
...
function obtenerUltimoDomicilioUsuario($idUsuario)
{
	global $con;
	
	
	$consulta="SELECT * FROM 803_direcciones WHERE idUsuario=".$idUsuario." AND Tipo=0";
	...
	$consulta="SELECT Mail as mail FROM 805_mails WHERE idUsuario=".$idUsuario;
	$correoElectronico=$con->obtenerFilasJSON($consulta);
	
	
	$consulta="SELECT Tipo2 AS tipoTelefono,codArea AS idPais,Lada AS lada,Numero AS numero,Extension AS extension,verificado FROM 804_telefonos
				WHERE idUsuario=".$idUsuario." and numero<>''";
	$arrTelefonos=$con->obtenerFilasJSON($consulta);
	
	$obj='{"calle":"'.cv($fDireccion["Calle"]).'","noExt":"'.cv($fDireccion["Numero"]).'","noInt":"'.cv($fDireccion["NumeroInt"]).
...
function obtenerCuerpoDocumentoB64($idDocumento,$base64=true)
{
	global $con;
	$consulta = "SELECT nomArchivoOriginal,documento,tipoArchivo,tamano,enBD,repositorioDocumento,documentoRepositorio FROM 908_archivos WHERE idArchivo=".$idDocumento;
	$fila=$con->obtenerPrimeraFila($consulta);
	...
	if($fila[5]==0)
	{
		...
		if($rutaDocumento!="")
		{
			if(strpos($rutaDocumento,"http")!==false)
			{
				...
				if(!$base64)
					 $cuerpoDocumento=bD($cuerpoDocumento);
	...
	{
		$conexion=generarInstanciaConectorGestor($fila[5]);
		if($base64)
			return bE($conexion->obtenerDocumento($fila[6]));
		return $conexion->obtenerDocumento($fila[6]);
		
	}
}

function obtenerDocumentoFisico($idDocumento)
{
	global $baseDir;
	$rutaDestino=$baseDir."/archivosTemporales/".generarNombreArchivoTemporal();
	$cuerpoDocumento=obtenerCuerpoDocumentoB64($idDocumento,false);
	if(escribirContenidoArchivo($rutaDestino,$cuerpoDocumento))
		return true;
...
function buscarParticipanteExhorto($figuraJuridica,$nombreParticipante)
{
	...
	while($fila=$con->fetchRow($resResultado))
	{
		if($listaRegistros=="")
...
//			$resultado["idUnidad"]=33;
		...
		case 3: //Guardia Ley Mujeres
			...
			while($fila=$con->fetchAssoc($resFechas))
			{
				if(colisionaTiempo($fila["fechaInicio"]." ".$fila["horaInicio"],$fila["fechaFin"]." ".$fila["horaFin"],$fRegistroSolicitud[2],$fRegistroSolicitud[2],false))
				{
					...
					$res=$con->obtenerFilas($consulta);
					while($filaJuez=$con->fetchRow($res))
					{
						if($lista=="")
						{
							$lista=$filaJuez[0];
	...
	if($con->ejecutarBloque($consulta))
	{
		...
		$rDocumentos=$con->obtenerFilas($query);
		while($fila=$con->fetchRow($rDocumentos))
		{
...
function mostrarSeccionEdicionDocumentoSeleccionFormatoDocumentoBloqueado($idFormulario,$idRegistro,$idFormularioEvaluacion)
{
	global $con;
