...
function generarDocumentoPDFFormato($idRegistroFormato,$descomponerDocumentoMarcadores=true,$bloquearDocumento=0,$conversorPDF=1,$documentosAnexos="")
{
	...
	$consulta="SELECT cuerpoFormato,cadenaFirma,documentoBloqueado,tipoFormato,idFormulario,idRegistro,
				if(idDocumento is not null,idDocumento,idDocumentoAdjunto) as idDocumentoAdjunto FROM 3000_formatosRegistrados WHERE idRegistroFormato=".$idRegistroFormato;
	...
	if($fDocumento[4]==-2)
	{
		$consulta="SELECT datosParametros FROM 7035_informacionDocumentos WHERE idRegistro=".$fDocumento[5];
		$datosParametros=$con->obtenerValor($consulta);
		
		if($datosParametros!="")
		{
			$datosParametros=json_decode($datosParametros);
			if(isset($datosParametros->office365)&&(($fDocumento[6]=="")||($fDocumento[6]=="-1")))
				$esOffice365=true;
		}
		else
			$datosParametros=NULL;
	...
	$cuerpoFormato=removerCaracteresNoImprimiblesPlantilla($cuerpoFormato);
	/*$x;
	for($x=0;$x<strlen($cuerpoFormato);$x++)
	{
		echo ord($cuerpoFormato[$x])."=".$cuerpoFormato[$x]."<br>";
	}
	return;*/
	$firma=$fDocumento[1];
	...
	if($esOffice365)
	{
		$nombreArchivo=$datosParametros->nombreDocumento;
		$archivoTemporal=$baseDir."/archivosTemporales/".$datosParametros->nombreDocumento.".pdf";
		
		$infoComp["idConexion"]=$datosParametros->idConexion;
		$cDrive=new cOneDrive("","","","",$infoComp);
		$cDrive->conectar();
		$cuerpoFormato=$cDrive->obtenerDocumentoPDF("/".$nombreArchivo,false);
		if(!$cuerpoFormato)
			return false;
	}
	else
	...
	if(($fDocumento[6]=="")&&(escribirContenidoArchivo($archivoTemporal,$cuerpoFormato)))
	{
		...
		if(file_exists($directorioDestino."/".$nombreArchivo.".pdf"))
		{
			...
			{
				descomponerDocumentoMarcadores($idRegistroFormato,$cuerpoFormato);
			}
			...
			if($actualizarFormato)
			{
				...
				$consulta="update 3000_formatosRegistrados set formatoPDF=1,documentoBloqueado=".$bloquearDocumento.",idDocumento=".$idDocumento.
						",fechaBloqueo=".($bloquearDocumento==1?("'".date("Y-m-d H:i:s")."'"):"NULL")." where idRegistroFormato=".$idRegistroFormato;
	...
	else
	{
		if($fDocumento[6]!="")
		{
			...
			$consulta="SELECT LOWER(nomArchivoOriginal) FROM 908_archivos WHERE idArchivo=".$fDocumento[6];
			...
			{
				$directorioDestino=$baseDir."/archivosTemporales/";
				$archivoOrigen=obtenerRutaDocumento($fDocumento[6]);
				$cuerpoDocumentoSinFirma=bD(obtenerCuerpoDocumentoB64($fDocumento[6]));
				...
				if(file_exists($archivoDestino))
				{
					...
					reemplazarArchivoRepositorio($fDocumento[6],$cuerpoDocumento);	
					unlink($archivoDestino);
					
					$consulta="SELECT nomArchivoOriginal FROM 908_archivos WHERE idArchivo=".$fDocumento[6];
					...
					$consulta="update 908_archivos set  nomArchivoOriginal='".$nomArchivoOriginal."'  WHERE idArchivo=".$fDocumento[6];
					$con->ejecutarConsulta($consulta);
					$carpetaAdministrativa=obtenerCarpetaAdministrativaProceso($fDocumento[4],$fDocumento[5]);
					...
					{	
						if($carpetaAdministrativa!="")
						{
							registrarDocumentoCarpetaAdministrativa($carpetaAdministrativa,$fDocumento[6],$fDocumento[4],$fDocumento[5]);
						}
						registrarDocumentoResultadoProceso($fDocumento[4],$fDocumento[5],$fDocumento[6]);
					}
					
					$consulta="update 3000_formatosRegistrados set formatoPDF=1,documentoBloqueado=".$bloquearDocumento.",idDocumento=".$fDocumento[6].",fechaBloqueo='".date("Y-m-d H:i:s")."' where idRegistroFormato=".$idRegistroFormato;
					$con->ejecutarConsulta($consulta);
					if($esOffice365)
					{
						$cDrive->removerRecurso("/".$datosParametros->nombreDocumento);
...
function registrarDocumentoCarpetaAdministrativa($carpetaAdministrativa,$idDocumento,$idFormulario=-1,$idRegistro=-1,$idCarpetaAdministrativa=-1)
{
	...
	if(($idCarpetaAdministrativa!=-1)&&($idCarpetaAdministrativa!=""))
	{
	...
	else
	{
		...
		if($tipoCarpetaAdministrativa!=0)
			$idCarpetaAdministrativa=-1;
	}
	...
	if($nReg>0)
		return true;
	$consulta="SELECT sha512,upper(nomArchivoOriginal) as nomArchivoOriginal,nomArchivoOriginal as nombreOriginal,fechaCreacion,tamano,categoriaDocumentos FROM 908_archivos WHERE idArchivo=".$idDocumento;
	$fFDatosDocumento=$con->obtenerPrimeraFilaAsoc($consulta);
	
	
	
	$sha512=$fFDatosDocumento["sha512"];
	$nombreArchivo=$fFDatosDocumento["nomArchivoOriginal"];
	

	
	
	if($sha512!="") //Modificado
	{
	...
	$cacheCalculos=NULL;
	$consulta="SELECT * FROM 908_configuracionExtensionesContabilizacionIndiceElectronico WHERE UPPER(extension)='".$extension."'";
	$fConfExtension=$con->obtenerPrimeraFilaAsoc($consulta);
	if($fConfExtension)
	{
		if($fConfExtension["tipoContabilizacion"]==1)
			$noPaginas=$fConfExtension["valorContabilizacion"];	
		else
		{
			$cadParametros='{"idDocumento":"'.$idDocumento.'"}';
			$objParametros=json_decode($cadParametros);
			$noPaginas=removerComillasLimite(resolverExpresionCalculoPHP($fConfExtension["valorContabilizacion"],$objParametros,$cacheCalculos));
		}
	}
	...
	$carpetaAdministrativaRaiz="";
	$idCarpetaAdministrativaRaiz="";
	
	if($tipoCarpetaAdministrativa!=0)
	{
		$carpetaAdministrativaRaiz=	$carpetaAdministrativa;
		$idCarpetaAdministrativaRaiz=$idCarpetaAdministrativa;
		if($idCarpetaAdministrativaRaiz==-1)
		{
			$consulta="SELECT idCarpeta FROM 7006_carpetasAdministrativas WHERE carpetaAdministrativa='".$carpetaAdministrativaRaiz."'";
			$idCarpetaAdministrativaRaiz=$con->obtenerValor($consulta);
		}
	}
	else
	{
		$arrDatosCarpeta=obtenerCarpetaRaiz($carpetaAdministrativa,$idCarpetaAdministrativa);
		$carpetaAdministrativaRaiz=$arrDatosCarpeta["carpetaAdministrativa"];
		$idCarpetaAdministrativaRaiz=$arrDatosCarpeta["idCarpeta"];
	}
	
	$paginaInicio="";
	$ordenDocumento="";
	
	$consulta="SELECT paginaFin,ordenDocumento FROM 7007_contenidosCarpetaAdministrativa WHERE 
				carpetaAdministrativaRaiz='".$carpetaAdministrativaRaiz."' AND idCarpetaAdministrativaRaiz=".$idCarpetaAdministrativaRaiz.
				" AND tipoContenido=1 and copia=0 order by paginaFin desc";

	$fUltimoElemento=$con->obtenerPrimeraFilaAsoc($consulta);
	if($fUltimoElemento)
	{
		$paginaInicio=$fUltimoElemento["paginaFin"];
		$ordenDocumento=$fUltimoElemento["ordenDocumento"];
	...
	$paginaFin=$paginaInicio+($noPaginas-1);	
	...
	$consulta="INSERT INTO 7007_contenidosCarpetaAdministrativa(carpetaAdministrativa,fechaRegistro,responsableRegistro,tipoContenido,descripcionContenido,
				idRegistroContenidoReferencia,idFormulario,idRegistro,etapaProcesal,idCarpetaAdministrativa,ordenDocumento,noPaginas,paginaInicio,paginaFin,
				observaciones,origen,extension,carpetaAdministrativaRaiz,idCarpetaAdministrativaRaiz)
				VALUES('".$carpetaAdministrativa."','".date("Y-m-d H:i:s")."',".$_SESSION["idUsr"].",1,'',".$idDocumento.",".$iFormulario.",".$iRegistro.
				",".$etapaProcesal.",".$idCarpetaAdministrativa.",".$ordenDocumento.",".$noPaginas.",".$paginaInicio.",".$paginaFin.",'".cv($observaciones).
				"',".$origen.",'".cv($extension)."','".$carpetaAdministrativaRaiz."',".$idCarpetaAdministrativaRaiz.")";

	if($con->ejecutarConsulta($consulta))
	{
		$nombreDocumento=normalizarNombreDocumento($carpetaAdministrativa,$idCarpetaAdministrativa,$idDocumento);
		$consulta="UPDATE 908_archivos SET nomArchivoOriginal='".$nombreDocumento."' WHERE idArchivo=".$idDocumento;
		$con->ejecutarConsulta($consulta);
		
		registrarMovimientoCarpetaAdministrativa($carpetaAdministrativa,$idCarpetaAdministrativa,"Se agrega documento \"".$nombreDocumento."\" en carpeta \"".$carpetaAdministrativa."\".",4);
			
		$consulta="SELECT nombreCategoria FROM  908_categoriasDocumentos WHERE idCategoria=".$fFDatosDocumento["categoriaDocumentos"];			
		$tipoDocumental=$con->obtenerValor($consulta);
		$lblOrigen=$origen==1?"Físico":"Digital";
		
		$comentarioAdicionalSQL="Registro de foliado electr&oacute;nico con la siguiente informaci&oacute;n:<br><b>Código del documento:</b> ".
								cv($fFDatosDocumento["nombreOriginal"]).",<b>Tipo documental:</b> ".cv($tipoDocumental).",<b>Fecha de creaci&oacute;:</b> ".$fFDatosDocumento["fechaCreacion"].
								",<b>Fecha de incorporación:</b> ".date("Y-m-d H:i:s").", <b>N&uacute;mero de p&aacute;ginas:</b> ".$noPaginas.", <b>P&aacute;gina inicial:</b> ".$paginaInicio.
								", <b>P&aacute;gina final:</b> ".$paginaFin.", <b>Formato:</b> PDF/A, <b>Tama&ntilde;o (bytes):</b> ".$fFDatosDocumento["tamano"].
								",<b>Origen:</b> ".$lblOrigen.", <b>Observaciones: </b>, <b>Funcion hash:</b> SHA-512, <b>Hash del documento: </b>".$sha512;
		guardarRegistroBitacoraSistema("../gestorDocumental/funcionesDocumentos.php",$idDocumento,19,("No. Expediente: ".$carpetaAdministrativa.".- ".$comentarioAdicionalSQL));
		...
		if($fDatosArchivo["repositorioDocumento"]!=0)
		{
			...
			$rutaDirectorio="repositorioDocumentos/".$carpetaAdministrativa;////Modificado
			
			if($fDatosArchivo["documentoRepositorio"]!=$rutaDirectorio)
			{
			
				$conexion->moverDocumento($fDatosArchivo["documentoRepositorio"],$rutaDirectorio);
			}
		}
		else
		{
			$rutaDocumento=obtenerRutaDocumento($idDocumento);
			$pathDestino=$baseDir."/repositorioDocumentos/documento_".$idDocumento;
			if($rutaDocumento!=$pathDestino)
				rename($rutaDocumento, $pathDestino);
...
function registrarDocumentoResultadoProceso($idFormulario,$idRegistro,$idDocumento)
{
	...
	{
		$carpetaAdministrativa=obtenerCarpetaAdministrativaProceso($idFormulario,$idRegistro);
		if(($carpetaAdministrativa=="")||($carpetaAdministrativa=="N/E"))
			return true;
...
function registrarDocumentoReferenciaProceso($idFormulario,$idRegistro,$idDocumento,$asociarExpediente=true) //Modificado
{
	global $con;
	...
	if( $con->ejecutarConsulta($consulta))
	{
		...
		if(($carpetaAdministrativa=="")||(!$asociarExpediente))
			return true;
...
function llenarFormato_214($idActa)
{
	...
	$res=$con->obtenerFilas($consulta);
	while($fFila=$con->fetchAssoc($res))
	{
		...
		while($fMedio=$con->fetchAssoc($rMedio))
		{
		...
		while($fMedioFundameto=$con->fetchAssoc($rMedioFundamento))
		{
	...
	$fFila="";
	$nPos=1;
	while($fActor=$con->fetchRow($rActores))
	{
		if($nPos==1)
			$fFila='<tr>';
...
{	
	...
	while($fEventoAudiencia=$con->fetchRow($res))
	{
...
function obtenerPermisosActor($actor,$idDocumento,$tipoDocumento,$iFormulario,$iRegistro)
{
	...
	if(($idPerfilEvaluacion!="-1")&&($actor!="0_0"))
	{
		...
		while($fOpciones=$con->fetchRow($resOpciones))
		{
		...
		while($fOpciones=$con->fetchRow($rOpcionesTurno))
		{
...
function cambiarSituacionDocumento($iRegistro,$actor,$etapaCambio,$comentarios,$rolActual,$resultadoEvaluacion,$idUsuarioDestinatario=0)
{
	...
	if( $con->ejecutarBloque($query))
	{
		...
		while($fFuncion=$con->fetchRow($rFunciones))
		{
			...
			if($fDocumentoComp["idFormulario"]==460)
			{
				...
				if($fAmparoPromocion["resolverAmparoTransitorio"]==1)
				{
					...
					if($fResolucionAmparo["existeJuezConoce"]==1)
					{
						...
						{
							$arrJuecesAux=array();
							while($filaJuez=$con->fetchRow($rJuezAmparo))
							{
				...
				if($r1 && $r2)
				{
					...
					if($actualizarFormato)
					{
						...
						$consulta="update 3000_formatosRegistrados set formatoPDF=1,documentoBloqueado=".$bloquearDocumento.
									",idDocumento=".$idDocumento.",idDocumentoAdjunto=".$idDocumento.",fechaBloqueo=".($bloquearDocumento==1?("'".date("Y-m-d H:i:s")."'"):"NULL")." where idRegistroFormato=".$idRegistroFormato;
						$con->ejecutarConsulta($consulta);
