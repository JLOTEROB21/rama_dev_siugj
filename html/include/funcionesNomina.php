<?php include_once("funcionesEnvioMensajes.php");
include_once("conectoresAccesoDatos/administradorConexiones.php");
include_once("cLectorFormatoImportacion.php"); 
include_once("funcionesNominaModeloLita.php"); 
...
function resolverExpresionCalculoPHP($idConsulta,$objParametros,&$cacheCalculos)
{
	...
	if(($cacheCalculos!==NULL)&&(isset($objParametros->marcaParametro)))
	{
...
function generarExpresionCalculoPHP($idConsulta,$objParametros,&$cacheCalculo,&$arrQueries)
{
	...
	$arrQueries=resolverQueries($idConsulta,"2,3",$objParam,true);
	$consulta="SELECT formatoGuardado FROM 991_consultasSql WHERE idConsulta=".$idConsulta;

	$formatoGuardado=$con->obtenerValor($consulta);
	//echo $formatoGuardado;
	if($formatoGuardado==2)                                                                                                                           
	{
		$consulta="SELECT tokenMysql FROM 992_tokensConsulta WHERE idConsulta=".$idConsulta;
		$txtCuerpoPHP=$con->obtenerValor($consulta);
		$txtCuerpoPHP=str_replace("return ",'$resultadoFinal=',bD($txtCuerpoPHP));
		return $txtCuerpoPHP;
	}
	...
	while($fToken=$con->fetchRow($resTokens))
	{
	...
	while($fila=$con->fetchRow($resQuery))
	{
...

		{
			...
			if($fila[2]!="<br />")
			{
				...
				if(isset($arrQuitarComillas[$valor]))
				{
...

{
	...
	switch($tipoToken)
	{
		...
		case 16:
			...
			{
				case "1":
					$valor=$objParametros->iFormulario;
				break;
				case "2":
					$valor=$objParametros->idProceso;
				break;
				case "3":
					$valor=$objParametros->iRegistro;
				break;
				case "4":
					$valor=$objParametros->etapa;
				break;
				case "5":
					$valor=$objParametros->idActorProceso;
				break;
...

{
...

	{
...

		{
			...
			while($fPlaza=$con->fetchRow($resTabulacion))
...

{
	...
	while($filaPuesto=$con->fetchRow($resConsulta))
	...
	while($filaAcum=$con->fetchRow($resAcumuladores))
...

	{
		...
		while($fNomina=$con->fetchRow($resTbl))
