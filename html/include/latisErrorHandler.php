...
function latisErrorHandler($errno, $errstr, $errfile, $errline) 
{
	...
	$consulta="SELECT reportarErroresAdministrador,correoErroresAdministrador FROM 903_variablesSistema";
	$fConfiguracion=$con->obtenerPrimeraFilaAsoc($consulta);
	if($fConfiguracion["reportarErroresAdministrador"]==1)
	{
		$arrParam["msgError"]=base64_decode($parametros["reporte"]);
		$arrParam["mailDestinatario"]=trim($fConfiguracion["correoErroresAdministrador"]);
		
		if(@enviarMensajeEnvio(32,$arrParam,$funcionEnvioCorreoElectronico))
		{
//
		}
	}
