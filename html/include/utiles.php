<?php 	
	//include_once("PdfToText/PDF2Text.php");
	require("class.phpmailer.php");
	include_once("SIUGJ/libreriasFuncionesDespachos.php");
	include_once("libreriaFuncionesAdministracionEventosAudiencia.php");
...
//echo "Rol: ".$rol." ".$consulta."<br>";
	...
	$nombreRol=preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $nombreRol);
	return $nombreRol; 
}
...
function obtenerNombreUsuario($idUsuario,$prefijo=false)
{
	...
	$consulta="";
	if($prefijo)
		$consulta="select concat(if(Prefijo is null,'',concat(Prefijo,' ')),if(Nom is null,'',Nom),' ',if(Paterno is null,'',Paterno),' ',if(Materno is null,'',Materno)) from 802_identifica where idUsuario=".$idUsuario;
	else
		$consulta="select Nombre from 800_usuarios where idUsuario=".$idUsuario;

	$usuario=$con->obtenerValor($consulta);

	if(isset($_SESSION["suplantado"])&&($_SESSION["suplantado"]==1))
	{
		$usuario.=" (Suplantado)";
	}
	//$usuario=preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $usuario);
	return limpiarCadenaSerializacion($usuario);
}
...
{
	global $con;
	if(isset($_SESSION) &&($_SESSION["configuracionBitacora"]["0"]==1)&&(isset($_SESSION["idUsr"])))
	{	
		$fechaActual=date("Y-m-d");
		$horaActual=date("H:i:s");
		$fechaEvento=$fechaActual." ".$horaActual;
		$consulta="insert into 8000_logSistema(fecha,hora,idUsuario,pagina,parametros,tipo,dirIP,fechaEvento) values ('".$fechaActual."','".$horaActual."',".$_SESSION["idUsr"].",'".$rutaNomPagina.
					"','".cv($parametros)."',0,'".obtenerIP()."','".$fechaEvento."')";
...
function guardarBitacoraConsultaBD($consulta)
{
	...
	global $con;
	global $versionLatis;
	if(isset($_SESSION) && isset($_SESSION["configuracionBitacora"]) && isset($_SESSION["configuracionBitacora"][1]))
	{
		if(($_SESSION["configuracionBitacora"]["1"]==1)&&(isset($_SESSION["idUsr"])))
		{
			$fechaActual=date("Y-m-d");
			$horaActual=date("H:i:s");
			$fechaEvento=$fechaActual." ".$horaActual;
			
			$consulta="insert into 8000_logSistema(fecha,hora,idUsuario,consultaSql,tipo,dirIP,fechaEvento) values ('".$fechaActual."','".$horaActual."',".$_SESSION["idUsr"].
						",select HEX(AES_ENCRYPT('".cv($consulta)."', '".bD($versionLatis)."')),1,'".obtenerIP()."','".$fechaEvento."')";
			$con->ejecutarConsultaLog($consulta);
		}
	}
}
...
function guardarBitacoraModificacionBD($consulta)
{
	...
	$tipoEvento=0;
	if(strpos(mb_strtoupper($consulta),"INSERT")!==false)
	{
		$tipoEvento=6;
	}
	if(strpos(mb_strtoupper($consulta),"UPDATE")!==false)
	{
		$tipoEvento=2;
	}
	if(strpos(mb_strtoupper($consulta),"DELETE")!==false)
	{
		$tipoEvento=7;
	}
	...
	if($tipoEvento!=0)
	{
		if($tipoEvento==6)
		{
			if(strpos($consulta,"8000_logSistema")!==false)
			{
				return true;
			}
		}
		
		if(($_SESSION["configuracionBitacora"][$tipoEvento]==1)&&(isset($_SESSION["idUsr"])))
		{
			$fechaActual=date("Y-m-d");
			$horaActual=date("H:i:s");
			$fechaEvento=$fechaActual." ".$horaActual;
			...
			$consulta="insert into 8000_logSistema(fecha,hora,idUsuario,consultaSql,tipo,dirIP,fechaEvento) values ('".$fechaActual."','".$horaActual."',".$_SESSION["idUsr"].
			",(select HEX(AES_ENCRYPT('".cv($consulta)."', '".bD($versionLatis)."'))),".$tipoEvento.",'".obtenerIP()."','".$fechaEvento."')";
...
function guardarBitacoraInicioSesion()
{
	...
	if(isset($_SESSION) &&($_SESSION["configuracionBitacora"]["3"]==1)&&(isset($_SESSION["idUsr"])))
	{
		$fechaActual=date("Y-m-d");
		$horaActual=date("H:i:s");
		$fechaEvento=$fechaActual." ".$horaActual;
		$consulta="insert into 8000_logSistema(fecha,hora,idUsuario,pagina,tipo,dirIP,fechaEvento) values ('".
				$fechaActual."','".$horaActual."',".$_SESSION["idUsr"].",'./',3,'".obtenerIP()."','".$fechaEvento."')";
		$con->ejecutarConsultaLog($consulta);
	}
}
...
function guardarBitacoraInicioSesionFallida($login)
{
	global $logInicioSesion;
	global $con;
	$consulta="SELECT intentosFallidosBloquea FROM 903_variablesSistema";
	$intentosFallidosBloquea=$con->obtenerValor($consulta);
	
	
	if(!isset($_SESSION) || !isset($_SESSION["configuracionBitacora"]) || (!isset($_SESSION["configuracionBitacora"]["5"])))
	{
		...
		while($fBitacora=$con->fetchAssoc($resBitacora))
		{
			$_SESSION["configuracionBitacora"][$fBitacora["idRegistroBitacora"]]=$fBitacora["valor"];
		}
	}
	
	if(isset($_SESSION) && ($_SESSION["configuracionBitacora"]["5"]==1))
	{
		...
		$consulta="SELECT idUsuario FROM 800_usuarios WHERE Login='".$login."'";

		$idUsuario=$con->obtenerValor($consulta);
		if($idUsuario=="")
			$idUsuario="NULL";
		$consulta="insert into 8000_logSistema(fecha,hora,pagina,parametros,tipo,dirIP,idUsuario,fechaEvento) values ('".
				$fechaActual."','".$horaActual."','./','".$login."',5,'".obtenerIP()."',".$idUsuario.",'".$fechaEvento."')";
		$con->ejecutarConsultaLog($consulta);
		
		if(($idUsuario!="")&&($intentosFallidosBloquea>0))
		{
			$consulta="SELECT fechaEvento FROM 8000_logSistema WHERE idUsuario=".$idUsuario." AND tipo=3 ORDER BY fechaEvento DESC";
		
			$fUltimoLogeoCorrecto=$con->obtenerPrimeraFilaAsoc($consulta);
			if($fUltimoLogeoCorrecto)
				$consulta="SELECT COUNT(*) FROM 8000_logSistema WHERE idUsuario=".$idUsuario." AND fechaEvento>='".$fUltimoLogeoCorrecto["fechaEvento"]."'  AND tipo=5";
			else
				$consulta="SELECT COUNT(*) FROM 8000_logSistema WHERE idUsuario=".$idUsuario."  AND tipo=5";
			
			$numImtentosFallidos=$con->obtenerValor($consulta);
			if($intentosFallidosBloquea<=$numImtentosFallidos)
			{
				bloquearCuentaIntentosFallidos($idUsuario,$numImtentosFallidos);
			}
		}
	}
}

function bloquearCuentaIntentosFallidos($idUsuario,$noIntentos)
{
	global $con;
	$consulta="UPDATE 800_usuarios SET cuentaActiva=100 WHERE idUsuario=".$idUsuario;
	$con->ejecutarConsulta($consulta);
	if($_SESSION["configuracionBitacora"]["9"]==1)
	{
		$fechaActual=date("Y-m-d");
		$horaActual=date("H:i:s");
		$fechaEvento=$fechaActual." ".$horaActual;
		
		$consulta="insert into 8000_logSistema(fecha,hora,pagina,parametros,tipo,dirIP,idUsuario,consultaSql,fechaEvento) values ('".$fechaActual."','".$horaActual."','./','',9,'".
		obtenerIP()."',".$idUsuario.",'".cv("Bloqueo de cuenta por superar numero de intentos fallidos (".$noIntentos." intentos)")."','".$fechaEvento."')";
		$con->ejecutarConsultaLog($consulta);
	}
...
function guardarBitacoraFinSesion()
{
	...
	if(($_SESSION["configuracionBitacora"]["4"]==1)&&(isset($_SESSION["idUsr"])))
	{
		$fechaActual=date("Y-m-d");
		$horaActual=date("H:i:s");
		$fechaEvento=$fechaActual." ".$horaActual;
		$consulta="insert into 8000_logSistema(fecha,hora,idUsuario,pagina,parametros,tipo,dirIP,consultaSql,fechaEvento) values ('".$fechaActual."','".$horaActual."',".
				$_SESSION["idUsr"].",'./','".$_SESSION["login"]."',4,'".obtenerIP()."','Cerrado por Usuario','".$fechaEvento."')";
		$con->ejecutarConsultaLog($consulta);
	}
}

function guardarRegistroBitacoraSistema($pagina,$parametros,$tipoEvento,$comentarioAdicionalSQL)
{
	global $con;
	$fechaActual=date("Y-m-d");
	$horaActual=date("H:i:s");
	$fechaEvento=date($fechaActual." ".$horaActual);
	
	$consulta="insert into 8000_logSistema(fecha,hora,idUsuario,pagina,parametros,tipo,dirIP,consultaSql,fechaEvento,nivel) values ('".$fechaActual."','".$horaActual."',".
				$_SESSION["idUsr"].",'".cv($pagina)."','".cv($parametros)."',".$tipoEvento.",'".obtenerIP()."','".cv($comentarioAdicionalSQL)."','".$fechaEvento."',0)";
	$con->ejecutarConsultaLog($consulta);
...
function obtenerDiferenciaDiasHabiles($fechaI,$fechaF)
{
	$fecha1=strtotime($fechaI);
	$fecha2=strtotime($fechaF);
	$ct=0;
	...
	{
		$fecha1=strtotime("+1 days",$fecha1);
		if(esDiaHabilInstitucion(date("Y-m-d",$fecha1)))
			$ct++;
	}
	return $ct;
}
...
function sumarDiasHabiles($fechaI,$totalDias)
{
	$fecha1=strtotime($fechaI);
	$ct=0;
	while($ct<$totalDias)
...
function validarAsignacionMateria($tipoMateria,$dia,$hInicio,$hFin,$idGrupo,$idBloque)
{
	...
	{
		while($filaMateria=$con->fetchRow($resMaterias))
		{
...
function obtenerMapaCurricularHerenciaMateria($idMateria,$idGrado)
{
	...
	while($filaMateria=$con->fetchRow($resMateria))
	{
...
function buscarMateria($idMateriaPadre,$idMateriaBusqueda,&$arrMapaMateria)
{
	...
	while($filaMatHija=$con->fetchRow($resMatHijas))
	{
...
function comprobarHorarioAlumno($idUsuario,$idMateria,$idGrupo)
{
	...
	else
	{
		...
		{
			  while($filaMatA=$con->fetchRow($res))
			  {
				  ...
				  while($filaHmatA=$con->fetchRow($res1))	
				  {
						$con->dataSeek($res3,0);
						while($filaHmat=$con->fetchRow($res3))
						{
...
function obtenerCandidatosArmarGrupos($idMateria,$idMapaCurricular)
{
	...
	$cadenaCandidatos="";
	while($filaC=$con->fetchRow($resCandidatos))
	{
		if($cadenaCandidatos=="")
...
function obtenerCandidatosValidos($idPadre,$maximoMat,$bandera)
{
	...
	while($filaA=$con->fetchRow($resAspirantes))
	{
...
function crearGridDinamico($consulta,$configuracion,$divDestino,$anchoGrid=700,$altoGrid=450,$tamPagina="25",$condExtra="")
{
	...
	foreach($confBotones as $boton)
	{
		...
		switch($boton->tipo)
		{
			case "R":
				...
				$icono="../principalPortal/imagesSIUGJ/salir.png";
			...
			case "A":
				...
				$icono="../principalPortal/imagesSIUGJ/add.png";
				if(!isset($boton->leyenda))
					$tituloBtn="Agregar";
				else
					$tituloBtn=$boton->leyenda;
			...
			break;
			case "E":
				$icono="../principalPortal/imagesSIUGJ/delete.png";
				if(!isset($boton->leyenda))
					$tituloBtn="Eliminar";
				else
					$tituloBtn=$boton->leyenda;
			...
			break;
			case "M":
				$icono="../principalPortal/imagesSIUGJ/pencil.png";
				if(!isset($boton->leyenda))
					$tituloBtn="Modificar";
				else
					$tituloBtn=$boton->leyenda;
			...
			break;	
			case "V":
				$icono="../principalPortal/imagesSIUGJ/magnifier.png";
				if(!isset($boton->leyenda))
					$tituloBtn="Ver ficha";
				else
					$tituloBtn=$boton->leyenda;
			...
			break;	
			case "C":
				$icono="../principalPortal/imagesSIUGJ/icon_code.png";
	...
	$agregarCheckBox=true;
					
	if(isset($conf->ocultarCheckBox) && ($conf->ocultarCheckBox==1))
		$agregarCheckBox=false;
	
	if($agregarCheckBox)			
		$cadFunGrid.="var chkRow=new Ext.grid.CheckboxSelectionModel({singleSelect:".$multiSel.",checkOnly :true,width:40});";


	$cadFunGrid.="var camposJava=new Array( ".$camposJava.");
				  var columnasJava=new Array(new  Ext.grid.RowNumberer({width:60}),".($agregarCheckBox?"chkRow,":"").$arrCM.");";
       ...
       var gridRegistros=	new Ext.grid.EditorGridPanel	(
																  {
																	  ...
																	  frame:false,
																	  border:".$border.",
																	  cm: modelColumn,".($agregarCheckBox?"sm:chkRow,":"")."
																	  height:".$altoGrid.",
																	  width:".$anchoGrid.",
																	  ".$renderTo."
...
function generarXMLTabla($nTabla,$titulo,$cond="")
{
	...
	if(strpos($nTabla,'tablaDinamica')===false)
	{
		...
		while($filaO=$con->fetchRow($resTablaO))
		{
			$objCampo=array();
		...
		$listCampos="";
		while($filaColunm=$con->fetchRow($resColunm))
		{
			$campo='`'.$filaColunm[0].'`';
	...
	else
	{
		...
		$arrElementos=array();
		while($filaElemento=$con->fetchRow($resElementoFormulario))
		{
			$obj=array();
...
function respaldarProceso($idProceso,$idRegistro)
{
	...
	$xmlProceso.=$xmlBase;
	while($fila=$con->fetchRow($resTablas))
	{
		if($fila[3]=="0")
...
function generarFormatoOpcionesQuery($objParam) //1 array javascript; 2 opciones select;
{
	...
	else
	{
		...
		else
		{
			...
			$cadFinal="";
			while($fila=$con->fetchAssoc($resultado))
			{
				$textoProy="";
				foreach($objParam->arrCamposProy as $campo)
				{
...
function generarDocumentoPDF($nombreArchivo,$descarga=false,$borrarPDF=false,$borrarOrigen=false,$nombreFinal="",$comando="libreoffice4.2",$directorioDestino="")
{
	...
	switch($comando)
	{
		...
		default:
			...
			$comando=$comandoLibreOffice."  --headless --convert-to pdf ".$archivoOrigen." --outdir ".$directorioDestino." 2>&1";
...
function convertirWordToPDFServidorConversion($archivoOrigen,$archivoDestino)
{
	...
	//$documentBody=removerCaracteresNoImprimiblesPlantilla($documentBody);
...
function crearBaseUsuario($apPaterno,$apMaterno,$nombre,$email,$adscripcion="",$departamento="",$roles="",$loginUsuario="",$passwordUsuario="",$idUsuarioSugerido=-1,$enviarMailCuentaCreada=false)
{
	global $con;
	global $versionLatis;
	...
	//if($login=="")
	$aMail=str_replace("'","",$email);
	$arrMail=explode(",",$aMail);
	$login=$arrMail[0];
	...
	if($idUsuarioSugerido==-1)
	{
		$consulta[$x]="insert into 800_usuarios (Login,Password,Nombre,FechaActualiza,cambiarDatosUsr,cuentaActiva)VALUES('".
					cv($login)."',HEX(AES_ENCRYPT('".$passwd."', '".bD($versionLatis)."')),'".cv($nombreCompleto)."','".cv($fechaActual)."',2,1)";
		$x++;
	...
	if($con->ejecutarBloque($consulta))
	{
		...
		if($idUsuario=="")
			$idUsuario=0;
		if($enviarMailCuentaCreada)	
		{
			foreach($arrMail as $m)
			{
				$arrParam["login"]=$login;
				$arrParam["password"]=$passwd;
				$arrParam["nombreUsuario"]=$nombre." ".$apPaterno." ".$apMaterno;
				$arrParam["idUsuario"]=$idUsuario;
				$arrParam["idActivacion"]=bE("cuenta:".$idUsuario);
				$arrParam["mailDestinatario"]=trim($m);
...
function removerComillasSimples($valor)
{
...
function formatearTituloPagina($titulo,$mostrarObligatorio=false,$idValor=0,$alineacion="left",$mostrarTituloModulo=true)
{
	...
	switch($idValor)
	{
		...
		break;	
		default:
			$complementario=" (Modificaci&oacute;n)";
		break;
	}
	
	
	$tabla="<br><table width='100%'>";
	if($cadRuta!="")
	{
		$tabla.="<tr><td align='left'><span class='leyendaPathHistorial'>".$cadRuta."<div class='tituloPathHistorial'>".($titulo.' '.$complementario)."</div></span><br><br></td></tr>";
	}
	if($mostrarTituloModulo)
		$tabla.="<tr><td align='left'><span class='tituloSIUGJ'>".($titulo.' '.$complementario)."</span><br><br></td></tr>";
	if($mostrarObligatorio)
	{
		$tabla.="<tr><td align='left'><div class='leyendaFormulario'>Los datos marcados con <font color=\"#9B2027\">*</font>&nbsp; son obligatorios</div></td></tr>";
...
function generarRutaAcceso($nConfiguracion)
{
	...
	$nReg=count($arrRuta)-1;
	for($x=$nReg;$x>=0;$x--)
	{
		$oLink='<a href="javascript:irRuta(\''.bE($arrRuta[$x]["configuracion"]).'\',\''.bE($arrRuta[$x]["url"]).'\')">'.$arrRuta[$x]["titulo"].'</a>';
		if($cadenaRuta=="")
			$cadenaRuta=$oLink.'<div class="separadorPath"></div>';
		else
			$cadenaRuta.=$oLink.'<div class="separadorPath"></div>';
	}
...
function esDiaHabilInstitucion($fecha,$claveInstitucion="")
{
	global $con;
	$fechaTime=strtotime($fecha);
	...
	if(($dia>=1)&&($dia<=5))
	{
		...
		if($claveInstitucion=="")
		{
			$consulta="SELECT COUNT(*) FROM 7022_diasNOHabiles dH  WHERE ambitoGeneral=1 AND situacion=1
		...
		{
			$consulta="SELECT p.idRegistro FROM 7022_ambitoAplicacionFechaInhabil a,7022_diasNOHabiles p WHERE tipoAmbito=4 AND  cveElemento='".$claveInstitucion."'
				and p.idRegistro=a.idFechaReferencia and p.situacion=1 and '".$fecha."'>=p.fechaInicio AND '".$fecha."'<=p.fechaTermino";
			...
			$municipio=obtenerMunicipioDespacho($claveInstitucion);
			if($municipio!="")
			{
	
				
				$consulta="SELECT p.idRegistro FROM 7022_ambitoAplicacionFechaInhabil a,7022_diasNOHabiles p WHERE tipoAmbito=3 AND  cveElemento='".$municipio."'
						and p.idRegistro=a.idFechaReferencia and p.situacion=1 and '".$fecha."'>=p.fechaInicio AND '".$fecha."'<=p.fechaTermino";
				$idPerfil=$con->obtenerValor($consulta);
			...
			$circuito=obtenerCircuitoDespacho($claveInstitucion);
			if($circuito!="")
			{
				
				$consulta="SELECT p.idRegistro FROM 7022_ambitoAplicacionFechaInhabil a,7022_diasNOHabiles p WHERE tipoAmbito=2 AND  cveElemento='".$circuito."'
						and p.idRegistro=a.idFechaReferencia and p.situacion=1 and '".$fecha."'>=p.fechaInicio AND '".$fecha."'<=p.fechaTermino";
				$idPerfil=$con->obtenerValor($consulta);
			...
			$distrito=obtenerDistritoJuicialDespacho($claveInstitucion);
			if($distrito!="")
			{
				$consulta="SELECT p.idRegistro FROM 7022_ambitoAplicacionFechaInhabil a,7022_diasNOHabiles p WHERE tipoAmbito=1 AND  cveElemento='".$distrito."'
						and p.idRegistro=a.idFechaReferencia and p.situacion=1 and '".$fecha."'>=p.fechaInicio AND '".$fecha."'<=p.fechaTermino";
				$idPerfil=$con->obtenerValor($consulta);
				if($idPerfil!="")
				{
			...
			$consulta="SELECT idRegistro FROM 7022_diasNOHabiles dH  WHERE ambitoGeneral=1 AND situacion=1
						AND '".$fecha."'>=fechaInicio AND '".$fecha."'<=fechaTermino";
			$idPerfil=$con->obtenerValor($consulta);
			
			return $idPerfil==""?true:false;
		}
...
function obtenerSiguienteDiaHabil($fecha,$claveInstitucion="")
{
	if(esDiaHabilInstitucion($fecha,$claveInstitucion))
		return $fecha;
	while(true)
	{
...
function obtenerAnteriorDiaHabil($fecha,$claveInstitucion="")
{
	if(esDiaHabilInstitucion($fecha,$claveInstitucion))
		return $fecha;
	while(true)
...
function removerCaracteresNoImprimiblesPlantilla($plantilla)
{
	$arrCaracteres[chr(194)]=1;
	$arrCaracteres[chr(160)]=1;
	$arrCaracteres[chr(226)]=1;
	$arrCaracteres[chr(128)]=1;
	$arrCaracteres[chr(139)]=1;
	$arrCaracteres[chr(9)]=1;
	$arrCaracteres[chr(10)]=1;
	
	foreach($arrCaracteres as $campo=>$valor)
		$plantilla=str_replace($campo,"",$plantilla);
	
	return $plantilla;
}

function aplicarOCRSobreDocumento($rutaDocumento,$idioma="spa")
{
	return true;
	global $comandoOCRMYPDF;
	$arrSalida=array();
	$resultado="";
	$archivoDestino=$rutaDocumento."_ocr";
	$comando=$comandoOCRMYPDF." -l spa ".$rutaDocumento." ".$archivoDestino." --skip-text  2>&1";//
	exec($comando,$arrSalida,$resultado);	
	if(file_exists($archivoDestino))
	{
		unlink($rutaDocumento);
		rename($archivoDestino,$rutaDocumento);		
		return true;
	}
	return false;
}

function extraerTextoDocumento($rutaDocumento)
{
	global $comandoPDFTOTXT;
	$arrSalida=array();
	$archivoDestino=$rutaDocumento."_txt";
	$comando=$comandoPDFTOTXT." -o ".$archivoDestino." ".$rutaDocumento." 2>&1";//
	exec($comando,$arrSalida,$resultado);	
	if(file_exists($archivoDestino))
	{
		$contenidoDocumento=leerContenidoArchivo($archivoDestino);
...
function procesarDocumentoRepositorioOCRText($idDocumento,$idioma="spa")
{
	global $con;
	...
	if($rutaDocumento!="")
	{
		...
		if(aplicarOCRSobreDocumento($rutaDocumento,$idioma))
		{
			
			$cuerpoDocumento=extraerTextoDocumento($rutaDocumento);
			$sha512=strtoupper(hash_file ( "sha512" , $rutaDocumento,false ));
			$consulta="UPDATE 908_archivos SET aplicadoOCR=1,cuerpoDocumento='".cv($cuerpoDocumento)."',sha512='".$sha512."' WHERE idArchivo=".$idDocumento;
...
function convertirParticipanteProcesoCuentaSistema($idParticipante,$adscripcion,$roles)
{
	global $con;
	
	
	$consulta="SELECT idCuentaAcceso FROM 7005_relacionFigurasJuridicasSolicitud WHERE idParticipante=".$idParticipante." AND idCuentaAcceso IS NOT NULL";
	$idCuentaAcceso=$con->obtenerValor($consulta);
	if($idCuentaAcceso!="")
	{
		return $idCuentaAcceso;
	}
	$consulta="SELECT * FROM _47_tablaDinamica WHERE id__47_tablaDinamica=".$idParticipante;
	$fParticipante=$con->obtenerPrimeraFilaAsoc($consulta);
	
	
	$consulta="SELECT  idUsuario FROM 802_identifica WHERE tipoIdentificacion=".$fParticipante["tipoIdentificacion"]." AND noIdentificacion='".$fParticipante["folioIdentificacion"]."'";
	$idCuentaAcceso=$con->obtenerValor($consulta);
	if($idCuentaAcceso!="")
	{
		$consulta="UPDATE 7005_relacionFigurasJuridicasSolicitud SET idCuentaAcceso=".$idCuentaAcceso." WHERE idParticipante=".$idParticipante;
		if($con->ejecutarConsulta($consulta))
		{
			return $idCuentaAcceso;
		}
	...
	$consulta="SELECT idRegistro FROM 7025_datosContactoParticipante WHERE idParticipante=".$fParticipante["id__47_tablaDinamica"]." ORDER BY idRegistro DESC";
	$idContacto=$con->obtenerValor($consulta);
	$consulta="SELECT correo FROM 7025_correosElectronico WHERE idReferencia=".$idContacto;
	$email=$con->obtenerListaValores($consulta,"'");
	if($email=="")	
		$email=-1;
	$consulta="SELECT idUsuario FROM 805_mails WHERE Mail IN(".$email.") order by idUsuario";
	...
	if($email!=-1)
	{
		if($idUsuario=="")
		{
			$idUsuario=crearBaseUsuario($fParticipante["apellidoPaterno"],$fParticipante["apellidoMaterno"],$fParticipante["nombre"],$email,$adscripcion,"",$roles,"", "",-1,true);
			$consulta="UPDATE 800_usuarios SET tipoCreacion=2 WHERE idUsuario=".$idUsuario;
			$con->ejecutarConsulta($consulta);
			$consulta="INSERT INTO 804_telefonos(Tipo2,Lada,Numero,Extension,codArea,idUsuario)
					SELECT tipoTelefono,lada,numero,extension,idPais,'".$idUsuario."' AS idUsuario FROM 7025_telefonos WHERE idReferencia=".$idContacto;
			$con->ejecutarConsulta($consulta);
		}
		$consulta="UPDATE 7005_relacionFigurasJuridicasSolicitud SET idCuentaAcceso=".$idUsuario." WHERE idParticipante=".$idParticipante;
		if($con->ejecutarConsulta($consulta))
		{
			return $idUsuario;
		}
...
function aperturarSesionUsuario($idUsuario)
{
	global $con;
	...
	$_SESSION["codigoUnidad"]=$institucion;
	$_SESSION["codigoInstitucion"]=$institucion;
	$_SESSION["login"]="";	
}

function generarFuentesLetras($ruta="")
{
	global $extensionesFuentesPermitidas;
	global $baseDir;

	if($ruta=="")
		$ruta=$baseDir."/fuentes";	
	if (is_dir($ruta)) 
	{
		if ($dh = opendir($ruta)) 
		{
			while (($file = readdir($dh)) !== false) 
			{
				...
				if (is_dir($ruta ."/". $file) && $file!="." && $file!="..")
				{
					...
					generarFuentesLetras($ruta ."/". $file);
				}
				else
				{
					if($file!="." && $file!="..")
					{
						...
						$rutaAbsoluta=$ruta."/".$file;
						$arrInfo=pathinfo($rutaAbsoluta);
						
						if(isset($extensionesFuentesPermitidas[strtolower($arrInfo["extension"])]))
						{

							$rutaRelativa=str_replace($baseDir,"..",$rutaAbsoluta);
							echo "
									
									@font-face
									{
									  font-family:'".$arrInfo['filename']."';
									  src: url('".$rutaRelativa."');
...
function limpiarCadenaSerializacion($cadena)
{
	$cadena=trim($cadena);
...
function encryptString($psSource, $psKey)
{
	$psSource = bE($psSource);
	$sDestination = '';
	$iSourceLen = strlen($psSource);
	$iKeyLen = strlen($psKey);
	

	for ($iSource=0; $iSource<$iSourceLen; $iSource++)
	{
		$cSource = ord($psSource[$iSource]);
		$cKey = ord($psKey[$iSource % $iKeyLen]);
		$cDestination = $cSource ^ $cKey;
		$sDestination .= chr($cDestination);
	}
	return bE($sDestination);
}

function decryptString($psSource, $psKey)
{
	$psSource = bD($psSource);
	$sDestination = '';
	$iSourceLen = strlen($psSource);
	$iKeyLen = strlen($psKey);
	...
	return bD($sDestination);
}

function enviarMensajeSMSTwilio($numeroDestino,$MensajeDestino,$prefijoCelular="521",$numeroOrigen="")
{
	global $con;

	$consulta="SELECT * FROM _1242_tablaDinamica WHERE codigo='00001/2023'";

	$fRegistro=$con->obtenerPrimeraFilaAsoc($consulta);
	
	$arrParametros=array();
	$consulta="SELECT * FROM _1242_gParametrosAdicionales WHERE idReferencia=".(isset($fRegistro["id__1242_tablaDinamica"])?$fRegistro["id__1242_tablaDinamica"]:-1);
	$res=$con->obtenerFilas($consulta);
	...
	if($numeroOrigen=="")
		$numeroOrigen=$arrParametros["PhoneNumber"]["valor"];
	...
	try
	{
		...
		$UrlConexion=$fRegistro["urlConexion"];
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $UrlConexion);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS,$parametros);	
		curl_setopt($ch, CURLOPT_USERPWD, $arrParametros["AccountSID"]["valor"].':'.$arrParametros["AuthToken"]["valor"]);
		...
		{
			$tipoNotificacion=14;
			$idRegistroBitacora=registrarBitacoraConsumoServicioConexiones($tipoNotificacion,$numeroDestino);
			actualizarBitacoraConsumoServicioConexiones($idRegistroBitacora,curl_error($ch),4,$result,$UrlConexion);
			return json_decode('{"resultado":"4","mensajeError":"'.curl_error($ch).'","idMensaje":"","responseString":""}');
		...
		if(isset($r->sid))
		{
			return json_decode('{"resultado":"1","mensajeError":"'.cv(isset($r->message)?$r->message:'').'","idMensaje":"'.(isset($r->sid)?$r->sid:""). '","responseString":"'.bE($result).'"}');
		}
		else
		{	
			$tipoNotificacion=14;
			...
			actualizarBitacoraConsumoServicioConexiones($idRegistroBitacora,isset($r->message)?$r->message:'',4,$result,$UrlConexion);
			
			return json_decode('{"resultado":"4","mensajeError":"'.cv(isset($r->message)?$r->message:'').'","idMensaje":"'.(isset($r->sid)?$r->sid:""). '","responseString":"'.bE($result).'"}');
		}
	...
	catch(Exception $e)
	{
		...
		$tipoNotificacion=14;
		//$numeroDestino.":<br>".$MensajeDestino
		$idRegistroBitacora=registrarBitacoraConsumoServicioConexiones($tipoNotificacion,$numeroDestino);
		actualizarBitacoraConsumoServicioConexiones($idRegistroBitacora,$e->getMessage(),4,$result,$UrlConexion);
		return json_decode('{"resultado":"4","mensajeError":"'.$e->getMessage().'","idMensaje":"","responseString":""}');
	}
}

function enviarMensajeWhatsAppTwilio($numeroDestino,$MensajeDestino,$prefijoCelular="521",$numeroOrigen="16474927546")
{
	try
	{
		
		$ch = curl_init();
		$MensajeDestino=str_replace("\t","",$MensajeDestino);
		/*for($x=0;$x<strlen($MensajeDestino);$x++)
		{
			echo $MensajeDestino[$x].".- ".ord($MensajeDestino[$x])."<br>";
		}*/
		$parametros="To=".urlencode("whatsapp:+".$prefijoCelular.$numeroDestino)."&From=".urlencode("whatsapp:+".$numeroOrigen)."&Body=".urlencode(str_replace("<br>",". ",(($MensajeDestino))));
		...
		if (curl_errno($ch)) 
		{
			return json_decode('{"resultado":"0","mensajeError":"'.curl_error($ch).'","idMensaje":"","responseString":""}');

		}
		curl_close($ch);
	
		return json_decode('{"resultado":"'.(isset($r->sid)?"1":"0").'","mensajeError":"'.cv($r->error_message).'","idMensaje":"'.(isset($r->sid)?"1":"0").'","responseString":"'.bE($result).'"}');
	...
	catch(Exception $e)
	{
		...
		return json_decode('{"resultado":"0","mensajeError":"'.$e->getMessage().'","idMensaje":"","responseString":""}');
	}
}


function obtenerTRDExpediente($unidadGestion,$fecha,$tipoProceso)
{
	global $con;	
	...
	if($tipoProceso=="")	
		$tipoProceso=-1;
	$consulta="SELECT idTablaRetencion FROM 908_despachosTablaRetencion WHERE despacho='".$unidadGestion."'";

	$listaTablas=$con->obtenerListaValores($consulta);
	if($listaTablas=="")
		$listaTablas=-1;
	
	$consulta="SELECT DISTINCT r.idTablaRetencion,r.version,s.idSerie FROM 908_seriesTiposProcesos s,
				908_registrosTablasRetencionDocumental r,908_versionesTablasRetencionDocumental v
				WHERE s.tipoProceso=".$tipoProceso." AND r.idRegistro=s.idSerie AND r.idTablaRetencion IN(".$listaTablas.
				") AND v.version=r.version AND v.situacionActual=2
				AND v.fechaAplicacion<='".$fecha."' ORDER BY v.fechaAplicacion DESC";

	$fTRD=$con->obtenerPrimeraFilaAsoc($consulta);
	
	$arrResultado=array();
	if($fTRD)
	{
		$arrResultado["idTablaRetencion"]=$fTRD["idTablaRetencion"];
		$arrResultado["version"]=$fTRD["version"];
		
		$consulta="SELECT * FROM 908_registrosTablasRetencionDocumental WHERE idRegistro=".$fTRD["idSerie"];
		$fRegistro=$con->obtenerPrimeraFilaAsoc($consulta);
		switch($fRegistro["tipoElemento"])
		{
			case 1:
				$arrResultado["idSerie"]=$fTRD["idSerie"];
				$arrResultado["idSubserie"]=-1;
			break;
			case 2:
				$arrResultado["idSerie"]=$fRegistro["idSerie"];
				$arrResultado["idSubserie"]=$fTRD["idSerie"];
			break;
		}
	}
...
function validarTokenServicio($token,$servicio,$archivo,$formato="JSON")
{
	global $con;
	$arrResp=array();
	$consulta="SELECT COUNT(*) FROM 8000_tokensAccesoWS WHERE token='".$token.
					"' AND nombreServicio='".$servicio."' AND archivoServicio='".$archivo."'";
	...
	if($numFila==0)
	{
		...
		{
			case "JSON":
				$arrResp[1]='{"resultado":"0","error":"El token ingresado no es v&aacute;lido para este servicio","codigoError":"404"}';
			break;
			case "XML":
				$arrResp[1]='<resultado>0</resultado><error>El token ingresado no es v&aacute;lido para este servicio</error><codigoError>404</codigoError>';
			break;
		}
...
function reemplazarArchivoRepositorio($idDocumento,$cuerpoDocumento,$nomArchivoOriginal="")
{
	global $con;
	$consulta="SELECT repositorioDocumento,documentoRepositorio FROM 908_archivos WHERE idArchivo=".$idDocumento;
	$fRegistro=$con->obtenerPrimeraFilaAsoc($consulta);
	if($fRegistro["repositorioDocumento"]==0)
	{
	
		$rutaDocumento=obtenerRutaDocumento($idDocumento);
		escribirContenidoArchivo($rutaDocumento,$cuerpoDocumento);
		$tamano=filesize($rutaDocumento);
		$sha512=strtoupper(hash_file( "sha512" , $rutaDocumento,false ));
		$consulta="update 908_archivos set folioQR=NULL, ".($nomArchivoOriginal!=""?"nomArchivoOriginal='".$nomArchivoOriginal."',":"")."tamano='".$tamano.
					"',sha512='".$sha512."'  WHERE idArchivo=".$idDocumento;
		return $con->ejecutarConsulta($consulta);
	}
	else
	{
		$conexion=generarInstanciaConectorGestor($fRegistro["repositorioDocumento"]);
		$rutaDirectorio="";
		$arrRuta=explode("/",$fRegistro["documentoRepositorio"]);
		$nombreDocumento=$arrRuta[count($arrRuta)-1];
		for($x=0;$x<count($arrRuta)-1;$x++)
		{
			if($arrRuta[$x]!="")
			{
				if($rutaDirectorio=="")
					$rutaDirectorio=$arrRuta[$x];
				else
					$rutaDirectorio.="/".$arrRuta[$x];
			}
		}

		return $conexion->crearDocumento($rutaDirectorio,$nombreDocumento,$cuerpoDocumento);
	}
	
}

function registrarRespaldoArchivoRepositorio($idDocumento,$cuerpoDocumento,$nomArchivoOriginal)
{
	global $con;
	global $baseDir;
	...
	if($fRegistro["repositorioDocumento"]==0)
	{
		...
		$pathDocumento=$baseDir."/repositorioDocumentosRespaldo/".$nomArchivoOriginal;
		return escribirContenidoArchivo($pathDocumento,$cuerpoDocumento);
		
	}
	else
	{
		$conexion=generarInstanciaConectorGestor($fRegistro["repositorioDocumento"]);
		$rutaDirectorio=$conexion->raizServidor."/repositorioDocumentos/repositorioDocumentosRespaldo";
		return $conexion->crearDocumento($rutaDirectorio,$nomArchivoOriginal,$cuerpoDocumento);
	}
...
{
	global $con;
	$consulta="SELECT apellidoPaterno,apellidoMaterno,nombre FROM _47_tablaDinamica WHERE id__47_tablaDinamica='".$idParticipante."'";
	$res=$con->obtenerPrimeraFila($consulta);
	$nombre=trim($res[2])." ".trim($res[0])." ".trim($res[1]);
	return limpiarCadenaSerializacion($nombre);
}


function obtenerPrefijoSitio()
{
	global $urlSitio;

	$uSitio=str_replace("https://","",$urlSitio);
	$uSitio=str_replace("http://","",$uSitio);
	
	$prefijoSitio=substr($uSitio,0,4);
	$prefijoSitio=mb_strtoupper(str_replace("-","",$prefijoSitio));
	return $prefijoSitio;
}


function registrarBitacoraConsumoServicioConexiones($tipoOperacion,$idReferencia1="",$idReferencia2="")
{
	global $con;
	
	$consulta="INSERT INTO 3017_solicitudesWebServicesOperadores(fechaSolicitud,tipoOperacion,idReferencia1,idreferencia2) 
			VALUES('".date("Y-m-d H:i:s")."',".$tipoOperacion.",'".$idReferencia1."','".$idReferencia2."')";
	if($con->ejecutarConsulta($consulta))
	{
...
function actualizarBitacoraConsumoServicioConexiones($idRegistroBitacora,$cuerpoSolicitud,$resultado,$mensajeRespuesta,$urlConexion)
{
	global $con;
	$consulta="UPDATE 3017_solicitudesWebServicesOperadores SET cuerpoSolicitud='".bE($cuerpoSolicitud)."',resultado=".cv($resultado).
			",mensajeRespuesta='".bE($mensajeRespuesta)."',urlConexion='".$urlConexion."' WHERE idRegistroSolicitud=".$idRegistroBitacora;
	return $con->ejecutarConsulta($consulta);
}


function normalizarValor($valor,$soloNumeros=false)
{
	$arrCaracteres["0"]=1;
	$arrCaracteres["1"]=1;
	$arrCaracteres["2"]=1;
    ...
    for($x=0;$x<strlen($valor);$x++)
	{
		...
		if(isset($arrCaracteres[$caracter]))
			$cadAux.=$caracter;
	}
	return $cadAux;
}

function obtenerPerfilDiaInhabil($fecha,$claveInstitucion="")
{
	global $con;
	if($claveInstitucion=="")
	{
		$consulta="SELECT dH.idRegistro FROM 7022_diasNOHabiles dH  WHERE ambitoGeneral=1 AND situacion=1
					AND '".$fecha."'>=fechaInicio AND '".$fecha."'<=fechaTermino";
		$idPerfil=$con->obtenerValor($consulta);
...
function realizarEjecucionPagina($URL,$parametros,$metodo="GET")
{
	$urlWebServices=$URL;
	...
	if(($queryString!="")&&($metodo=="GET"))
	{
		$urlWebServices.="?".$queryString;
	}
