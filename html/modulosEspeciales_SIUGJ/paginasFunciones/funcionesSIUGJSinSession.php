<?php 
	...
	function obtenerEventosFechaSala()
	{
		...
		$consulta="SELECT if(horaInicioReal is null,horaInicioEvento,horaInicioReal),if(horaTerminoReal is null,horaFinEvento,horaTerminoReal),
					(SELECT tipoAudiencia FROM _4_tablaDinamica WHERE id__4_tablaDinamica=a.tipoAudiencia) as tipoAudiencia,idRegistroEvento,idCentroGestion,idCentroGestion
					 FROM 7000_eventosAudiencia a
		...
		while($fila=mysql_fetch_row($res))
		{
			...
			$consulta="SELECT nombreUnidad FROM _17_tablaDinamica WHERE id__17_tablaDinamica=".$fila[4];
			...
			$titulo=cv("ID ".$fila[3]."__".$fila[2]."__[".$cAdministrativa."]__".$lblDespacho);
			
			$e='{"id":"e_'.$fila[3].'","editable":false,"title":"'.$titulo.'","start":"'.date("Y-m-d\TH:i:s",strtotime($fila[0])).'","end":"'.date("Y-m-d\TH:i:s",strtotime($fila[1])).'","color":"#F1F7FF"}';	
			if($arrEventos=="")
				$arrEventos=$e;
			else
				$arrEventos.=",".$e;
