...
<script type="text/javascript" src="../modulosEspeciales_SGJP/Scripts/tblNotificacionConsejo.js.php"></script>
				  ...
				  <!-- InstanceBeginEditable name="content2" --> 
                   <tr>
                        ...
                        <?php
							...
							$consulta="SELECT datosCJF,carpetaAdministrativa,idReferencia FROM _460_tablaDinamica WHERE id__460_tablaDinamica=".$idRegistro;
							$fPromocion=$con->obtenerPrimeraFila($consulta);
							$datosCJF=$fPromocion[0];
							$arrRegistros="";
							if($datosCJF=="")
							{
								$consulta="SELECT id__26_tablaDinamica,j.clave,cTJ.tipoJuez FROM _346_juecesAmparo je,_26_tablaDinamica j,
										_18_tablaDinamica cTJ,_26_tipoJuez tj WHERE j.id__26_tablaDinamica=je.idOpcion AND je.idPadre=".$fPromocion[2]."
										AND tj.idPadre=id__26_tablaDinamica AND cTJ.id__18_tablaDinamica=tj.idOpcion ORDER BY cTJ.tipoJuez,j.clave";
							}
							else
							{
								$consulta="SELECT idOpcion FROM _460_juezReferido WHERE idPadre=".$idRegistro;
								$juezRefiere=$con->obtenerValor($consulta);
								$consulta="SELECT id__26_tablaDinamica,j.clave,cTJ.tipoJuez FROM _26_tablaDinamica j,
										...
										cTJ.id__18_tablaDinamica=tj.idOpcion and j.id__26_tablaDinamica=".$juezRefiere.
							...
							while($fila=mysql_fetch_row($res))
							{
								...
								if($con->filasAfectadas==1)
									$notifica="true";
								$o="[".$notifica.",'".$fila[0]."','Juez ".cv($fila[1])." (".cv($fila[2]).")']";
								if($arrRegistros=="")
									$arrRegistros=$o;
								else
									$arrRegistros.=",".$o;
                        ...
                        ?>
                        	<input type="hidden" id="iFormulario" value="<?php echo $idFormulario?>" />
                            <input type="hidden" id="iRegistro" value="<?php echo $idRegistro ?> " />
                            <input type="hidden" id="datosCJF" value="<?php echo $datosCJF ?> " />
                            <input type="hidden" id="carpetaAdministrativa" value="<?php echo $fPromocion[1] ?> " />
                            <input type="hidden" id="idCarpetaAdministrativa" value="-1" />
                             <input type="hidden" id="arrJuecesNotifica" value="<?php echo bE($arrRegistros)?>" />
                        </td>
                   </tr>
				  <!-- InstanceEndEditable -->
				  <tr>
