...
<script src="../modulosEspeciales_SGJP/Scripts/editorFormatosFirmaInmediata.js.php" ></script>
<?php
				  ...
				  <!-- InstanceBeginEditable name="content2" --> 
                   <tr>
                        ...
                        <?php
							...
							$tipoFormato=-1;							
							$consulta="SELECT tipoFormato,idFormulario,idRegistro,idReferencia,cuerpoFormato,idRegistroFormato,documentoBloqueado FROM 3000_formatosRegistrados 
										WHERE idFormulario=".$idFormulario." and idReferencia=".$idRegistro." and idFormularioProceso=".$idFormularioProceso;
							...
							$consulta="SELECT etapaCambioFirma FROM 
											3003_configuracionModuloFormatos WHERE idProceso=".$idProceso." AND idPerfil=".$idPerfil.
											" and idFormularioProceso=".$idFormularioProceso;
							$etapaCambioFirma=$con->obtenerValor($consulta);
							if(!$fFormato)
							{
								
								$consulta="SELECT tipoDefinicionFormato,idFormato,funcionDefinicionFormato FROM 
											3003_configuracionModuloFormatos WHERE idProceso=".$idProceso." AND idPerfil=".$idPerfil.
											" and idFormularioProceso=".$idFormularioProceso;
								...
								if($fConfiguracion)
								{
									switch($fConfiguracion[0])
									{
										case 1:  //Seleccion directa
											$tipoFormato=$fConfiguracion[1];
										break;
										case 2:
											if($fConfiguracion[2]!="")
											{
												$cacheCalculos=NULL;
												$cadParametros='{"idFormulario":"'.$idFormulario.'","idRegistro":"'.$idRegistro.'","actor":"'.$actor.'"}';
												
												$objParametros=json_decode($cadParametros);
												$resultadoEvaluacion=removerComillasLimite(resolverExpresionCalculoPHP($fConfiguracion[2],$objParametros,$cacheCalculos));
												if($resultadoEvaluacion!="")
												{
													$tipoFormato=$resultadoEvaluacion;
												}
											}
										break;
									}
								...
								{
									$cache=NULL;
									$oParametro='{"accion":"0","idDocumento":"'.$tipoFormato.'","idReferencia":"'.$idReferencia.'","idRegistro":"'.$idRegistro.'","idFormulario":"'.$idFormulario.'"}';
									$objParametros=json_decode($oParametro);
                        ...
                        <input type="hidden" id="etapaCambioFirma"  value="<?php echo $etapaCambioFirma?>"/>
