...
?>
var uploadControl;
var costoPalabra=1.5;
var funcionEjecucionFirmaPHP='';
var esFirmaFinal=false;
var objGlobal;
...
function inicializar()
{
    ...
    if((gE('idDocumentoAdjunto').value=='-1')||((gE('permiteSeleccionPlantilla').value=='0')&&(gE('permiteEdicionTextoEnriquecido').value=='0')))
    {
    	oVisor=	{
                    region:'center',
                    html:'<br />'+
                    		...
                    		'<table >'+
                                '<tr>'+
                                '<td align="center"><textarea style="width:450px" id="txtDocumento"></textarea></td><td width="40"></td><td width="250" valign="top" align="left">'+
                                   '<table width="200" style="background-color: #F8FAE4;border-color: #CCC;border-style: solid;border-width: 1px;padding: 10px;">'+
                                   '<tr><td width="140"><span class="TSJDF_Etiqueta">Total de palabras:</span></td><td width="60" align="right"><span class="TSJDF_Control" id="spTotalPalabras">0</span></td></tr>'+
                                   '<tr><td><span class="TSJDF_Etiqueta">Costo por palabra:</span></td><td align="right"><span class="TSJDF_Control" id="spCostoPalabra">'+Ext.util.Format.usMoney(costoPalabra)+'</span></td></tr>'+
                                   '<tr><td><span class="TSJDF_Etiqueta">No. publicaciones:</span></td><td align="right" style="border-bottom-style:solid; border-bottom-width:1px; border-bottom-color:#000"><span class="TSJDF_Control" id="spNoPublicaciones">'+Ext.util.Format.number(gE('noPublicaciones').value,'0')+'</span></td></tr>'+
                                   '<tr><td><span class="TSJDF_Etiqueta">Costo total:</span></td><td align="right"><span class="vino12NegritasSub" id="spCostoTotal">$ 0.00</span></td></tr>'+
                                   '</table>'+
                               '</td>'+
                               '</tr>'+
                           '</table>'+
                                ...
                                items: [
                                            {
                                                ...
                                                tbar:	[
                                                            ...
                                                            },'-',
                                                            {
                                                                ...
                                                                cls:'x-btn-text-icon',
                                                                id:'btnWord',
                                                                hidden:(gE('idDocumentoAdjunto').value!='-1')||(gE('permiteSubirWord').value=='0'),
													...
													show : {
																...
																fn : function() 
																{
                                                                    ...
                                                                    // Backend settings
                                                                                            ...
                                                                                            upload_success_handler : subidaCorrectaV2
                                                                                        };
...
function subidaCorrectaV2(file, serverData) 
{
...
function firmarDocumentoExecFormato()
{
    ...
    if((gE('sL').value=='1')||(gE('idDocumentoAdjunto').value!='-1'))
    {
		...
		mostrarVentanaFirmaElectronicaFormato(cadConf);
    }
    else
    {
    	guardarDocumento(
...
function mostrarVentanaFirmaElectronicaFormato(cadConf)
{
													...
													show : {
																...
																fn : function() 
																{
                                                                    ...
                                                                    // Backend settings
                                                                                            ...
                                                                                            upload_success_handler : subidaCorrectaFinalFormato
                                                                                        };   
...
function subidaCorrectaFinalFormato(file, serverData) 
{
		...
		else 
		{
            ...
            objResultado.funcionEjecucion=bE(funcionEjecucionFirmaPHP);
...
function contarPalabras()
{
	...
	var contenido=CKEDITOR.instances.txtDocumento.getData();
    var arrContenido=contenido.split('<contenidoedicto>');
	arrContenido=arrContenido[1].split('</contenidoedicto>');
    arrContenido[0]=arrContenido[0].replace(/&nbsp;/gi,' ');

    contenido=Ext.util.Format.stripTags(arrContenido[0].trim());
    ...
    if(contenido.trim()!='')
    {
        arrContenido = contenido.trim().split (' ');
	    var x;
        for(x=0;x<arrContenido.length;x++)
        {
        	if(arrContenido[x].trim()!='')
	        	numeroPalabras++;
        }
        
    }
    gE('spTotalPalabras').innerHTML=numeroPalabras;
...
{
	var totalPalabras=parseFloat(gE('spTotalPalabras').innerHTML);
    var costoPalabras=parseFloat(normalizarValor(gE('spCostoPalabra').innerHTML));
    var noPublicaciones=parseFloat(gE('spNoPublicaciones').innerHTML);
    var CostoTotal=totalPalabras*costoPalabras*noPublicaciones;
	gE('spCostoTotal').innerHTML=Ext.util.Format.usMoney(CostoTotal);
