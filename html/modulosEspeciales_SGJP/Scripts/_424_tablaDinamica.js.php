<?php
	...
	if($idRegistro!=-1)
	{
		$consulta="SELECT defensor FROM _424_tablaDinamica WHERE id__424_tablaDinamica=".$idRegistro;	
...
function inyeccionCodigo()
{
    ...
    {
       asignarEvento(gE('_defensorvch'),'change',buscarDomilicio)
	}
    
    gE('sp_6743').innerHTML='';
...
function participanteAgregado(iParticipante,nombre)
{
    ...
    opt.text=nombre;
	gE('_defensorvch').options[gE('_defensorvch').options.length]=opt;
    gE('_defensorvch').selectedIndex=gE('_defensorvch').options.length-1;
    buscarDomilicio();
}
