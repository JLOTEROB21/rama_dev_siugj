...
<script type="text/javascript" src="../modulosEspeciales_SGJP/Scripts/cParticipante.js.php"></script>
<script type="text/javascript" src="../modulosEspeciales_SGJP/Scripts/cDireccionContacto.js.php"></script>
...
<script type="text/javascript" src="../principalPortal/Scripts/cNotificadores.js.php"></script>
				  ...
				  <!-- InstanceBeginEditable name="content2" --> 
                   <tr>
                        ...
                        <?php
							$sL=0;
							$idOrden=1;
							if(isset($objParametros->idOrden))
								$idOrden=$objParametros->idOrden;
							$idDiligencia=-1;
							$selPersona="";
							if(isset($objParametros->idDiligencia))
							{
								$idDiligencia=$objParametros->idDiligencia;
								$consulta="SELECT idOrden,idNombreParteProcesal,idParteProcesal FROM 7029_diligenciaActaNotificacion WHERE idRegistro=".$objParametros->idDiligencia;
								$fDiligencia=$con->obtenerPrimeraFila($consulta);
								$idOrden=$fDiligencia[0];
								$selPersona="p_".$fDiligencia[1]."_".$fDiligencia[2];
							}
							
							$vTAudiencia=0;
							if(isset($objParametros->vTAudiencia))
								$vTAudiencia=$objParametros->vTAudiencia;
							
							$consulta="SELECT carpetaJudicial,idCarpeta,idEventoDeriva,nombreDeterminacion,situacion 
										FROM 7042_ordenesNotificacion WHERE idOrden=".$idOrden;
							$fCarpeta=$con->obtenerPrimeraFila($consulta);
							if($fCarpeta[1]==-1)
							{
								$consulta="SELECT idCarpeta FROM 7006_carpetasAdministrativas WHERE carpetaAdministrativa='".$fCarpeta[0]."'";
								$fCarpeta[1]=$con->obtenerValor($consulta);
							}
							...
							$consulta="SELECT idRegistroEvento,
									...
									WHERE c.carpetaAdministrativa='".$fCarpeta[0]."' and c.idCarpetaAdministrativa in (-1,".$fCarpeta[1].
							...
							$consulta="SELECT idRegistroEvento FROM 7000_eventosAudiencia e,7007_contenidosCarpetaAdministrativa c 
									WHERE c.carpetaAdministrativa='".$fCarpeta[0]."' and c.idCarpetaAdministrativa in (-1,".$fCarpeta[1].
									") AND c.tipoContenido=3 AND c.idRegistroContenidoReferencia=e.idRegistroEvento 
									AND e.situacion IN(0,1,2,4,5) AND e.fechaEvento<='".date("Y-m-d").
									"' ORDER BY e.fechaEvento DESC LIMIT 0,1";
							$idEventoUltimo=$con->obtenerValor($consulta);
							
							$consulta="SELECT COUNT(*) FROM 805_configuracionMailSMTP WHERE idUsuario=".$_SESSION["idUsr"];
							$configuradoSMTP=$con->obtenerValor($consulta);
							$consulta="SELECT a.idArchivo,nomArchivoOriginal FROM 7043_documentosNotificacion d,908_archivos a 
									WHERE idOrden=".$idOrden." AND a.idArchivo=d.idDocumento ORDER BY nomArchivoOriginal";
							$arrDocumentosOrdenNotificacion=$con->obtenerFilasArreglo($consulta);
							
                        ?>
                        	<script type="text/javascript" src="../modulosEspeciales_SGJP/Scripts/tblOrdenNotificacionAtencion.js.php?cA=<?php echo $fCarpeta[0]?>&iC=<?php echo $fCarpeta[1]?>"></script>
                        	<input type="hidden" id="idOrden" value="<?php echo $idOrden ?>" />
                            <input type="hidden" id="carpetaJudicial" value="<?php echo $fCarpeta[0] ?>" />
                            <input type="hidden" id="idCarpeta" value="<?php echo $fCarpeta[1] ?>" />
                            
                            <input type="hidden" id="carpetaAdministrativa" value="<?php echo $fCarpeta[0] ?>" />
                            <input type="hidden" id="idCarpetaAdministrativa" value="<?php echo $fCarpeta[1] ?>" />
                            
                            <input type="hidden" id="arrAudiencias" value="<?php echo bE($arrAudiencias) ?>" />
                            <input type="hidden" id="idEventoDeriva" value="<?php echo $fCarpeta[2] ?>" />
                            <input type="hidden" id="idEventoUltimo" value="<?php echo $idEventoUltimo ?>" />
                            <input type="hidden" id="idDiligencia" value="<?php echo $idDiligencia ?>" />
                            <input type="hidden" id="selPersona" value="<?php echo $selPersona ?>" />
                            <input type="hidden" id="nombreDeterminacion" value="<?php echo bE($fCarpeta[3]) ?>" />
                            
                            <input type="hidden" id="idRegistroFormato" value="" />
                            <input type="hidden" id="tipoFormato" value="" />
                            <input type="hidden" id="idFormulario" value="" />
                            <input type="hidden" id="idRegistro" value="" />
                            <input type="hidden" id="idReferencia" value="" />
                            <input type="hidden" id="sL" value="<?php echo $sL?>" />
                            <input type="hidden" id="vTAudiencia" value="<?php echo $vTAudiencia?>" />
                            <input type="hidden" id="configuradoSMTP" value="<?php echo $configuradoSMTP?>" />
                            <input type="hidden" id="arrDocumentosOrdenNotificacion" value="<?php echo bE($arrDocumentosOrdenNotificacion)?>" />
