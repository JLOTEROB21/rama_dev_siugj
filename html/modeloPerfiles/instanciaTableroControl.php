...
<?php
	$consulta="SELECT DISTINCT f.archivoJS FROM 9061_camposTableroControl c,9033_funcionesScriptsSistema f WHERE 
				f.idFuncion=c.funcionRenderer";
				  ...
				  <!-- InstanceBeginEditable name="content2" --> 
                   <tr>
                        ...
                        <?php
							...
							$desactivarTemporizador=0;
							if(isset($objParametros->desactivarTemporizador))
							{
								$desactivarTemporizador=$objParametros->desactivarTemporizador;
							}
							
							$consulta="SELECT paginasScripts FROM 9060_tablerosControl WHERE idTableroControl=".$idTableroControl;
							$fDatosTablero=$con->obtenerPrimeraFila($consulta);
							$arrScripts=explode(",",$fDatosTablero[0]);
							foreach($arrScripts as $s)
							{
								if(trim($s)!="")
									echo '<script type="text/javascript" src="'.$s.'?iT='.bE($idTableroControl).'"></script>';
							}
							
                        ?>
                        <script type="text/javascript" src="../modeloPerfiles/Scripts/instanciaTableroControl.js.php?iT=<?php echo bE($idTableroControl)?>"></script>
                        <input type="hidden" id="desactivarTemporizador" value="<?php echo $desactivarTemporizador?>" />
                        <input type="hidden" id="idTableroControl" value="<?php echo $idTableroControl?>" />
                        </td>
                   </tr>
				  <!-- InstanceEndEditable -->
				  <tr>
