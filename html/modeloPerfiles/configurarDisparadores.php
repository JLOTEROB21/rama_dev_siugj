...
<script src="../Scripts/aceMaster/src/ace.js" type="text/javascript" charset="utf-8"></script>
                        ...
                        <td>
                        	...
                        	<?php
								...
								if(isset($objParametros->idPerfil))
									$idPerfil=bD($objParametros->idPerfil);
								$idProceso="-1";
								if(isset($objParametros->idProceso))
									$idProceso=bD($objParametros->idProceso);
								$numEtapa="-1";
								if(isset($objParametros->numEtapa))
									$numEtapa=bD($objParametros->numEtapa);	
								$consulta="select nombre from 4001_procesos where idProceso=".$idProceso;
								$nProceso=$con->obtenerValor($consulta);
								...
								if(($idPerfil!="")&&($idPerfil!=-1))
									$consulta="select idConsulta from 991_consultasSql where comp1='".$idProceso."' and comp2='".$numEtapa."' and comp3='".$idPerfil."' and tipoConsulta=10";
								else
									$consulta="select idConsulta from 991_consultasSql where comp1='".$idProceso."' and comp2='".$numEtapa."' and tipoConsulta=10";
									
								$idConsultaExp=$con->obtenerValor($consulta);
								if($idConsultaExp=="")
									$idConsultaExp="-1";
								
								$consulta="select nombreTabla from 900_formularios where idProceso=".$idProceso." and formularioBase=1";
								$nTablaBase=$con->obtenerValor($consulta);
								$txtCuerpoPHP="";
								
								
								$consulta="SELECT formatoGuardado FROM 991_consultasSql WHERE idConsulta=".$idConsultaExp;
								$formatoGuardado=$con->obtenerValor($consulta);
								if($formatoGuardado=="")
									$formatoGuardado=1;
								if($formatoGuardado==2)
								{
									$consulta="SELECT tokenMysql FROM 992_tokensConsulta WHERE idConsulta=".$idConsultaExp;
									$txtCuerpoPHP=$con->obtenerValor($consulta);
								}
                            ...
                            <script type="text/javascript" src="../Scripts/dataSetAPI.js.php"></script>
                            <script type="text/javascript" src="../Scripts/dataEstructuraAPI.js.php"></script>
                            <script type="text/javascript" src="Scripts/configurarDisparadores.js.php?idConsulta=<?php echo bE($idConsultaExp)?>"></script>
                        	<input type="hidden" id="idConsultaExp" value="<?php echo $idConsultaExp?>" />
                        	<input type="hidden" id="idProceso" value="<?php echo $idProceso?>" />
                            <input type="hidden" id="numEtapa" value="<?php echo $numEtapa?>" />
                            <input type="hidden" id="nTablaBase" value="<?php echo $nTablaBase?>" />
                            <input type="hidden" id="idPerfil" value="<?php echo $idPerfil?>" />
                            <input type="hidden" id="txtCuerpoPHP" value="<?php echo $txtCuerpoPHP?>" />
                            <input type="hidden" id="formatoGuardado" value="<?php echo $formatoGuardado?>" />
                        </td>
                   </tr>
				  <!-- InstanceEndEditable -->
				  <tr>
