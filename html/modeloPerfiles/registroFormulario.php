...
function crearElementosFormulario()
{
		...
		while($filas=mysql_fetch_row($res))
		{
			...
			switch($filas[2])
			{
				...
				case "30":
					...
					{
						if(gettype($arrQueries[$filaE[2]]["resultado"])!='resource')
						{
					...
					else
					{
						...
						if((sizeof($arrQueries[$filaE[2]]["arrParamControl"])>0)&&($idRegistro!=-1))
						{
					...
					$etiqueta="<div funcRenderer='".$filaE[19]."' class='".$filaE[13]."' id='".$nombreControl."' idAlmacen='".$filaE[2]."' enlace='".$filaE[5]."'>".$HRef.'<span id="sp_'.$filas[0].'">'.convertirEnterToBR($valor)."</span>".$cHRef."</div>";
					if($filaE[19]!="")
					{
						$funcionesJava.="asignarValorFormatoRenderer('".$filaE[19]."',".$filas[2].",'".$nombreControl."',".$filas[0].");";
					}
				break;	
		...
		if($tipoProceso!=1)
			$query="select idGrupoElemento,nombreCampo,tipoElemento,obligatorio,posX,posY,orden,visible,habilitado from 901_elementosFormulario where tipoElemento not in (-2,1,13,20,30".$compCtrl.") and idFormulario=".$idFormulario." order by orden";
		else
			$query="select idGrupoElemento,nombreCampo,tipoElemento,obligatorio,posX,posY,orden,visible,habilitado from 901_elementosFormulario where tipoElemento not in (1,13,20,30".$compCtrl.") and idFormulario=".$idFormulario." order by orden";
		$res=$con->obtenerFilas($query);
		$ctParamFunciones=0;
		...
		while($fElemento=mysql_fetch_row($res))
		{
			...
			if($nFilasAccion>0)
			{
				$funcionAccion="function funcionAccion_".$fElemento[0]."(objCtrl,registro)
								...
								";
				$arrAcciones=array();
				while($filaCtrl=mysql_fetch_row($resCtrlAccion))
				{
			...
			if(!elementoBloquedo($fElemento[0],$numEtapaPadre))
			{
			  ...
			  break;
					...
					case 2: //pregunta cerrada-Opciones Manuales
						...
						$etiqueta="	
									<table>
										<tr>
											<td>
												<select ".$estiloAncho." funcRenderer='".$filaE[19]."' val='".$val."' ".$habilitado." name='".$nombreControl."' id='".$nombreControl."' ><option value='-1' valDefault='1'>Elija una opción</option>";
						...
						if($filaE[19]!="")
						{
							$funcionesJava.="asignarValorFormatoRenderer('".$filaE[19]."',".$fElemento[2].",'".$nombreControl."',".$fElemento[0].");";
						}
					break;
					case 3: //pregunta cerrada-Opciones intervalo
						...
						if($idRegistro==-1)
							$valorCelda=$vDefault;
						$etiqueta= "	<select ".$estiloAncho." funcRenderer='".$filaE[19]."' val='".$val."' ".$habilitado." name='".$nombreControl."' id='".$nombreControl."' ><option value='-1' valDefault='1'>Elija una opción</option>";
					...
					case 4: //pregunta cerrada-Opciones tabla
						...
						$tipoPagina=$filaE[17];
						$valorPagina=$filaE[18];
						$btnAgregarElemento="";
						if($filaE[14]=="1")
							$btnAgregarElemento='<a href="javascript:agregarElemento(\''.bE($fElemento[0]).'\',\''.bE($nombreControl).'\')"><img src="../images/add.png" title="Agregar elemento" alt="Agregar elemento"></a>';
						$consulta="select tokenMysql from 913_consultasFiltroElemento where idGrupoElemento=".$fElemento[0];
						$resFiltro=$con->obtenerFilas($consulta);
						$condWhere="";
						while($filaFiltro=mysql_fetch_row($resFiltro))
							...
							else
							{
								...
								else
								{
									...
									else
									{
										...
										if((sizeof($arrQueries[$tablaOD]["arrParamControl"])>0)&&($idRegistro!=-1))
										{
						...
						else
						{
							...
							else
							{
								...
								if($arrQueries[$tablaOD]["ejecutado"]==1)
								{
									...
									{
										$cadObj='{"conector":null,"resQuery":null,"idAlmacen":"","arrCamposProy":[],"formato":"4","imprimir":"0","query":"'.$arrQueries[$tablaOD]["query"].'","campoID":"'.$campoId.'","itemSelect":"'.$valorCelda.'"}';
										$obj=json_decode($cadObj);
										$obj->resQuery=$resQuery;
										$obj->idAlmacen=$tablaOD;
										$obj->arrCamposProy=$arrCamposProy;
									...
									<table>
										<tr>
											<td>
													...
													<input type='hidden' name='".$nombreControl."' id='".$nombreControl."' value='".$valorCelda."' val='".$val."'>
											</td>
											<td>
												&nbsp;<span id='spAgregar_".$fElemento[0]."'>".$btnAgregarElemento."</span>
											</td>
							...
							$funcionesJavaInicio.="	function antesCargaExt_".$nombreControl."(dSet)
												{
													...
													dSet.baseParams.qy='".bE($consulta)."';
												};
					...
					case 5: //Texto Corto
						...
						$etiqueta= "<input funcRenderer='".$filaE[19]."' type='text' ".$habilitado." name='".$nombreControl."' id='".$nombreControl."' value='".str_replace("'","&#39;",$valorCelda)."' maxlength='".$maxLogitud."' size='".$filaE[2]."' class='".$filaE[13]."' val='".$val."'  >";
					...
					case 6: //Número entero
						...
						$consulta="select * from 904_configuracionElemFormulario where idElemFormulario=".$fElemento[0];
						$filaE=$con->obtenerPrimeraFila($consulta);	
						$etiqueta= "<input type='text' ".$habilitado." name='".$nombreControl."' id='".$nombreControl."' value='".$valorCelda."' size='".$filaE[2]."' class='".$filaE[13]."' onkeypress='return soloNumero(event,false,false)' val='".$val."' >";
						//$idCtrl=$nombreControl;	
					break;
					case 7: //Número decimal
						...
						$consulta="select * from 904_configuracionElemFormulario where idElemFormulario=".$fElemento[0];
						$filaE=$con->obtenerPrimeraFila($consulta);	
						$etiqueta= "<input type='text' ".$habilitado." name='".$nombreControl."' id='".$nombreControl."' value='".$valorCelda."' size='".$filaE[2]."' class='".$filaE[13]."' onkeypress='return soloNumero(event,true,false,this)' val='".$val."' >";
						//$idCtrl=$nombreControl;		
					break;
					case 8: //Fecha
						if($valorCelda!="")
						...
						$funcionesJava.="crearCampoFecha('sp".$nombreControl."','".$nombreControl."','".$fechaMinima."','".$fechaMaxima."').setDisabledDays([".$diasSel."]);".$compHabilitado;
						//$idCtrl=$nombreControl;
					break;
					case 9://Texto Largo 
						$consulta="select * from 904_configuracionElemFormulario where idElemFormulario=".$fElemento[0];
						...
						$etiqueta="<textarea funcRenderer='".$filaE[19]."' name='".$nombreControl."' ".$habilitado." id='".$nombreControl."' style='width:".$filaE[2]."px; height:".$filaE[3]."px ' class='".$filaE[13]."' val='".$val."' >".$valorCelda."</textarea>";
						//$idCtrl=$nombreControl;
					break;
					case 10: //Texto Enriquecido
						...
						$funcionesJava.="crearTextoEnriquecido('".$nombreControl."','txtEnriquecido_".$fElemento[0]."',".$filaE[2].",".$filaE[3].",'".$valorCeldaFinal."');arrTextRich.push('".$nombreControl."');gE('".$nombreControl."').setAttribute('val','".$val."');gE('".$nombreControl."').setAttribute('richText','".$nombreControl."');";
					...
					case 11: //Correo Electrónico
						...
						$etiqueta="<input type='text' ".$habilitado." name='".$nombreControl."' id='".$nombreControl."' value='".$valorCelda."' maxlength='".$filaE[3]."' size='".$filaE[2]."' class='".$filaE[13]."' val='".$val."' >";
						//$idCtrl=$nombreControl;
					break;
					case 12: //Archivo
						...
						$controlFile="<input type=\"file\" ".$habilitado."  name='".$nombreControlFile."' id='".$nombreControlFile."' val='".$val."' />";
					...
					case 14:
						...
						while($fRes=mysql_fetch_row($resOpt))
						{
							...
							if($valorCelda==$fRes[0])
							{
								...
								<input type="radio" '.$habilitado.' id="opt'.$nombreControl.'_'.$fRes[0].'" name="opt'.$nombreControl.'" value="'.$fRes[0].'"  onclick="selOpcion(this)" checked="checked" >&nbsp;<span name="et'.$nombreControl.'" >'.$fRes[1].'</span></td>';
								$lanzadorEvento.='lanzarEvento(gE(\'opt'.$nombreControl.'_'.$fRes[0].'\'),\'click\');';
							}
							else
							{
								$etiqueta.='<td  width="'.$anchoCelda.'" class="'.$fConfCampo[13].'">
								<input type="radio" '.$habilitado.' id="opt'.$nombreControl.'_'.$fRes[0].'" name="opt'.$nombreControl.'" value="'.$fRes[0].'"  onclick="selOpcion(this)" >&nbsp;<span name="et'.$nombreControl.'">'.$fRes[1].'</span></td>';
							}
							$radio="['".$fRes[0]."','".$fRes[1]."']";
							if($arrRadios=='')
								$arrRadios=$radio;
						...
						$etiqueta.="</table></span><input type='hidden' funcRenderer='".$filaE[19]."' name='".$nombreControl."' id='".$nombreControl."' val='".$val."' value='".$valorCelda."' controlF='opt".$nombreControl."_1' ><input type='hidden' id='lista".$nombreControl."' value=\"[".$arrRadios."]\">";
						//$idCtrl=$nombreControl;
						if($filaE[19]!="")
						{
							$funcionesJava.="asignarValorFormatoRenderer('".$filaE[19]."',".$fElemento[2].",'".$nombreControl."',".$fElemento[0].");";
					...
					case 15:
						...
						if($filaE[2]<$filaE[3])
						{
							for($x=$filaE[2];$x<=$filaE[3];$x+=$filaE[4])
							{
								...
								if($valorCelda==$x)
								{
									$etiqueta.='<td  width="'.$anchoCelda.'" class="'.$filaE[13].'"><input type="radio" '.$habilitado.' id="opt'.$nombreControl.'_'.$x.'" name="opt'.$nombreControl.'" value="'.$x.'" onclick="selOpcion(this)" checked="checked">&nbsp;<span name="et'.$nombreControl.'">'.$x.'</span></td>';
									$lanzadorEvento.='lanzarEvento(gE(\'opt'.$nombreControl.'_'.$x.'\'),\'click\');';
								}
								else
									$etiqueta.='<td  width="'.$anchoCelda.'" class="'.$filaE[13].'"><input type="radio" '.$habilitado.' id="opt'.$nombreControl.'_'.$x.'" name="opt'.$nombreControl.'" value="'.$x.'" onclick="selOpcion(this)">&nbsp;<span name="et'.$nombreControl.'">'.$x.'</span></td>';
								$nCol++;
								$radio="['".$x."','".$x."']";
								if($arrRadios=='')
									$arrRadios=$radio;
					...
					case 16:
						...
						if($obtenerOpciones)
						{
							...
							while($fRes=mysql_fetch_row($resOpt))
							{
								...
								if($valorCelda==$fRes[0])
								{
									$etiqueta.='<td  width="'.$anchoCelda.'" class="'.$filaE[13].'"><input type="radio" '.$habilitado.' id="opt'.$nombreControl.'_'.$fRes[0].'" name="opt'.$nombreControl.'" value="'.$fRes[0].'" onclick="selOpcion(this)" checked="checked">&nbsp;<span class="'.$filaE[13].'" name="et'.$nombreControl.'">'.$fRes[1].'</span></td>';
									$lanzadorEvento.='lanzarEvento(gE(\'opt'.$nombreControl.'_'.$e[0].'\'),\'click\');';
								}
								else
									$etiqueta.='<td  width="'.$anchoCelda.'" class="'.$filaE[13].'"><input type="radio" '.$habilitado.' id="opt'.$nombreControl.'_'.$fRes[0].'" name="opt'.$nombreControl.'" value="'.$fRes[0].'" onclick="selOpcion(this)">&nbsp;<span class="'.$filaE[13].'" name="et'.$nombreControl.'">'.$fRes[1].'</span></td>';						
								$radio="['".$fRes[0]."','".$fRes[1]."']";
								if($arrRadios=='')
									$arrRadios=$radio;
								else
						...
						else
						{
							if($arrElemento!=NULL)	
							{
								foreach($arrElemento as $e)
								{
									...
									if($valorCelda==$e[0])
									{
										$etiqueta.='<td  width="'.$anchoCelda.'" class="'.$filaE[13].'"><input type="radio" '.$habilitado.' id="opt'.$nombreControl.'_'.$e[0].'" name="opt'.$nombreControl.'" value="'.$e[0].'" onclick="selOpcion(this)" checked="checked">&nbsp;<span name="et'.$nombreControl.'">'.$e[1].'</span></td>';
										$lanzadorEvento.='lanzarEvento(gE(\'opt'.$nombreControl.'_'.$e[0].'\'),\'click\');';
									}
									else
										$etiqueta.='<td  width="'.$anchoCelda.'" class="'.$filaE[13].'"><input type="radio" '.$habilitado.' id="opt'.$nombreControl.'_'.$e[0].'" name="opt'.$nombreControl.'" value="'.$e[0].'" onclick="selOpcion(this)">&nbsp;<span name="et'.$nombreControl.'">'.$e[1].'</span></td>';						
									$radio="['".$e[0]."','".$e[1]."']";
									if($arrRadios=='')
										$arrRadios=$radio;
									else
					...
					case 17:
						...
						while($fRes=mysql_fetch_row($resOpt))
						{
							...
							$etiqueta.='<td  width="'.$anchoCelda.'" class="'.$fConfCampo[13].'"><input type="checkbox" '.$habilitado.' id="opt'.$nombreControl.'_'.$fRes[0].'" name="opt_'.$nombreControl.'[]" value="'.$fRes[0].'" '.$checado.' onclick="selCheck(this)"  >&nbsp;<span name="et'.$nombreControl.'">'.$fRes[1].'</span></td>';
						...
						$etiqueta.="</table></span><input   type='hidden' value='".$ctNumSel."' id='numSel".$nombreControl."'> <input type='hidden' funcRenderer='".$filaE[19]."' id='".$nombreControl."' val='min' name='".$nombreControl."' tipo='checkBox'  minSel='".$fConfCampo[10]."'  value='_".$arrNomTabla[1]."_".$nControlSinSuf."' controlF='".$primerElemento."'><input type='hidden' id='lista".$nombreControl."' value=\"[".$arrRadios."]\">";
						//$idCtrl=$nombreControl;
						if($filaE[19]!="")
						{
							$funcionesJava.="asignarValorFormatoRenderer('".$filaE[19]."',".$fElemento[2].",'".$nombreControl."',".$fElemento[0].");";
					...
					case 18:
						...
						if($filaE[2]<$filaE[3])
						{
							for($x=$filaE[2];$x<=$filaE[3];$x+=$filaE[4])
							{
								...
								$etiqueta.='<td  width="'.$anchoCelda.'" class="'.$filaE[13].'"><input type="checkbox" '.$habilitado.' id="opt'.$nombreControl.'_'.$x.'" name="opt_'.$nombreControl.'[]" value="'.$x.'" '.$checado.' onclick="selCheck(this)"  >&nbsp;<span name="et'.$nombreControl.'">'.$x.'</span></td>';
						...
						$arrNomTabla=explode('_',$nombreTabla);
						$nControlSinSuf= substr( $nombreControl,1,sizeof($nombreControl)-4);
						$etiqueta.="</table></span><input type='hidden' value='".$ctNumSel."' id='numSel".$nombreControl."'><input type='hidden' funcRenderer='".$filaE[19]."' id='".$nombreControl."' name='".$nombreControl."' val='min' value='_".$arrNomTabla[1]."_".$nControlSinSuf."' tipo='checkBox'  minSel='".$filaE[10]."' controlF='".$primerElemento."'><input type='hidden' id='lista".$nombreControl."' value=\"[".$arrRadios."]\">  ";
						//$idCtrl=$nombreControl;
						if($filaE[19]!="")
						{
							$funcionesJava.="asignarValorFormatoRenderer('".$filaE[19]."',".$fElemento[2].",'".$nombreControl."',".$fElemento[0].");";
					...
					case 19:
						...
						if($obtenerOpciones)
						{
							...
							while($fRes=mysql_fetch_row($resOpt))
							{
								...
								$etiqueta.='<td  width="'.$anchoCelda.'" class="'.$filaE[13].'" ><input type="checkbox" '.$habilitado.' id="opt'.$nombreControl.'_'.$fRes[0].'" name="opt_'.$nombreControl.'[]" value="'.$fRes[0].'" '.$checado.' onclick="selCheck(this)" >&nbsp;<span class="'.$filaE[13].'" name="et'.$nombreControl.'">'.$fRes[1].'</span></td>';
								$radio="['".$fRes[0]."','".$fRes[1]."']";
								if($arrRadios=='')
									$arrRadios=$radio;
								else
						...
						else
						{
							if($arrElemento!=NULL)	
							{
								foreach($arrElemento as $e)
								{
									...
									$etiqueta.='<td  width="'.$anchoCelda.'"  class="'.$filaE[13].'"><input type="checkbox" '.$habilitado.' id="opt'.$nombreControl.'_'.$e[0].'" name="opt_'.$nombreControl.'[]" value="'.$e[0].'" '.$checado.' onclick="selCheck(this)" >&nbsp;<span name="et'.$nombreControl.'">'.$e[1].'</span></td>';
									$radio="['".$e[0]."','".$e[1]."']";
									if($arrRadios=='')
										$arrRadios=$radio;
									else
					...
					case 21: //Hora
						...
						$funcionesJava.="crearCampoHora('sp".$nombreControl."','".$nombreControl."',".$fMin.",".$fMax.",".$intervalo.");".$compHabilitado;
						//$idCtrl=$nombreControl;
					break;
					...
					case 24: //Moneda
						...
						$etiqueta= "<input type='text' ".$habilitado." name='".$nombreControl."' id='".$nombreControl."' value='".$valorCelda."' size='".$filaE[7]."' class='".$filaE[13]."' onkeypress='return soloNumero(event,true,false,this)' val='".$val."' >";
						//$idCtrl=$nombreControl;		
					break;
					case 29://Grid
						$funcionesJava.="arrFuncionesAfterEdit['func_".$fElemento[0]."']=new Array();";
						$funcAfterEdit="";
						$funcAdd="";
						$consulta="select * from 904_configuracionElemFormulario where idElemFormulario=".$fElemento[0];
						...
						$cadConf=$filaE[8];
						$funcionesJava.="var funcAux2=function(grid,obj){";
						...
						$funcionesJava.=$cuerpoValidacionGrid." };";
						...
						while($filaConf=mysql_fetch_row($resConf))
						{
							...
							switch($tipoCampo)
							{
								...
								break;
								case 3:  //texto
									$editorColumn="new Ext.form.TextField({id:'editor_".$filaConf[3]."_".$fElemento[0]."'})";
								...
								case 4:  //Vinculado a tabla
									...
									$editorColumn="crearComboExt('editor_".$filaConf[3]."',".$arrOpciones.",0,0,null,{transform:false})";
									$rendererCtrl="function (val,meta,registro,fila,columna)
													{
								...
								break;
								case 6:	//Fecha
									$editorColumn="new Ext.form.DateField({id:'editor_".$filaConf[3]."_".$fElemento[0]."',format:'d/m/Y'})";
								...
								break;
								case 7:	//Hora
									$editorColumn="crearCampoHoraExt('editor_".$filaConf[3]."_".$fElemento[0]."','00:00','23:59',5,true)";
									//$editorColumn="null";
								...
								case 8:  //Calculo
									...
									$funcionesJava.="var funcObj=function(obj,registro,pos,idGrid)
																{
																	var cadObj=".$objValor.";
									...
									$funcionesJava.="arrFuncionesAfterEdit['func_".$fElemento[0]."'].push(funcObj);";																																
								break;	
								...
								case 12: //Archivo
									...
									$editorColumn="{xtype:'textoBotonField',tamanoMaxArchivo:'".$oColum->tamMaxArchivo." ".$oColum->unidadTamMaximo."',extensionesPermitidas:'".$oColum->extensionesPermitidas."'}";
								...
								case 14:	//Area de texto
									$editorColumn="new Ext.form.TextArea({id:'editor_".$filaConf[3]."',height:80})";
									$rendererCtrl="function (val,meta,registro,fila,columna)
													{
								...
								switch($objConfOD->tipoOrigenDatos)
								{
									case 1:
										if((isset($arrQueries[$objConfOD->idOrigenDatos]))&&($arrQueries[$objConfOD->idOrigenDatos]["ejecutado"]==1))
										{
											...
											if(mysql_num_rows($resOrigen))
												mysql_data_seek($resOrigen,0);
											while($fOrigen=mysql_fetch_assoc($resOrigen))	
											{
						...
						$funcionesJava.="var funcAux=function (datos)
													{
														".$cuerpoFuncionDeposito."
													};
						...
						if(($objConf!=null)&&isset($objConf->funcionAgregar))
							$funcionesJava.="objConf.funcionAgregar=".$objConf->funcionAgregar.";";
						$funcionesJava.="arrFuncionesGridDeposito['funcionDepositoPie_".$fElemento[0]."']=funcAux;";
						$funcionesJava.="arrFuncionesValidacionEdit['funcionValidacionGrid_".$fElemento[0]."']=funcAux2;";
					...
					case 31:
						...
						$etiqueta="<span class='".$filaE[13]."' id='_".$fElemento[1]."vch' enlace='".$filaE[5]."'>".$HRef.'<span id="sp_'.$filas[0].'">'.convertirEnterToBR($valor)."</span>".$cHRef."</span>
									<input type='hidden' name='".$nombreControl."' value='".$valor."'>";
					break;
					case 32: //Color
						$etiqueta= "<input type='text' ".$habilitado." name='".$nombreControl."' id='".$nombreControl."' value='".$valorCelda."' maxlength='6' size='8'  val='".$val."'>";
					...
					case 33: //galeria Imagenes
						...
						while($fGaleria=mysql_fetch_row($resGaleria))
						{
						...
						$etiqueta="<span name='galeriaDocumento' idCtrl='".$fElemento[0]."' id='_lbl".$fElemento[0]."'><span id='sp_".$fElemento[0]."' ancho='".$filaE[2]."' alto='".$filaE[3]."' arrElementos='".bE("[".$arregloGaleria."]")."'></span><span style='position:relative;top:-40px;left:".($filaE[2]-20)."px;z-index:10;'><a href='javascript:abrirAdmonGaleria(\"".bE($fElemento[0])."\")'><img src='../images/add.png'></a></span></span>";
			...
			else
			{
				...
				switch($fElemento[2])					
				{
					...
					case 9://Texto Largo 
						$consulta="select * from 904_configuracionElemFormulario where idElemFormulario=".$fElemento[0];
						$filaE=$con->obtenerPrimeraFila($consulta);
						$etiqueta= "<div class='letraFichaRespuesta' style='width:".$filaE[2]."px;height:".$filaE[3]."px;overflow: auto;'>".$HRef.$valorCelda.$cHRef."</div>";
					break;
					case 10: //Texto Enriquecido
						$consulta="select * from 904_configuracionElemFormulario where idElemFormulario=".$fElemento[0];
						$filaE=$con->obtenerPrimeraFila($consulta);
					...
					case 17:
						...
						while($fRes=mysql_fetch_row($resOpt))
						{
							...
							if(existeRegistro($fRes[0],$resTabla))
							{
								...
								$etiqueta.='<td width="'.$anchoCelda.'"><span class="'.$claseRespuesta.'">'.$fRes[1].'</span></td>';
								$nCol++;
							}
							if($nCol==$numColumnas)
							{
						...
						if($filaE[2]<$filaE[3])
						{
							for($x=$filaE[2];$x<=$filaE[3];$x+=$filaE[4])
							{
								...
								if(existeRegistro($x,$resTabla))
								{
									...
									$etiqueta.='<td width="'.$anchoCelda.'"><span class="'.$claseRespuesta.'">'.removerCerosDerecha($x).'</span></td>';
									$nCol++;
								}
								if($nCol==$numColumnas)
								{
							...
							if($arrElemento!=NULL)	
							{
								foreach($arrElemento as $e)
								{
									...
									if(existeRegistro($e[0],$resTabla))
									{
										...
										$etiqueta.='<td width="'.$anchoCelda.'"><span class="'.$claseRespuesta.'">'.$e[1].'</span></td>';
										$nCol++;
									}
									if($nCol==$numColumnas)
									{
...
<link rel="stylesheet" href="../Scripts/Carousel/thickbox.css" type="text/css" />
				  ...
				  <!-- InstanceBeginEditable name="content2" --> 
                   <tr>
                       ...
                       <?php
							...
							{
								if($tipoProceso!=1000)
									echo formatearTituloPagina($tituloModulo,true,$idRegistro);
								else
									echo formatearTituloPagina($tituloModulo,true,0);
							}
					   		...
					   		if($configuracionFormulario!="")
							{
								...
								if(isset($objConf->paginasScripts))
								{
									...
									if(sizeof($arrPagina)>0)
									{
										...
										{
											echo '<script type="text/javascript" src="'.$p.'?iF='.bE($idFormulario).'&m=1"></script>';
										}
									}
								}
							}
					   ?>
                       <script type="text/javascript" src="../modeloPerfiles/Scripts/registroFormulario.js.php?iF=<?php echo bE($idFormulario)?>"></script>
                    ...
                    <?php
						...
						?>
                                ...
                                <form method="post" action="<?php echo $paginaEnvioDatos?>" id="frmEnvio" enctype="multipart/form-data">
								<?php
                               ...
                               if($idRegistroG=='-1')
                               {
								   ...
								   if(($idProcesoP!="")&&($idProcesoP!=-1))
									...
									if($funcPHPEjecutarNuevo!="")
										echo "<input type='hidden' name='funcPHPEjecutarNuevo' value='".$funcPHPEjecutarNuevo."'>";
                               ...
                               else
                               {
									...
									if($funcPHPEjecutarModif!="")
										echo "<input type='hidden' name='funcPHPEjecutarModif' value='".$funcPHPEjecutarModif."'>";
                               }
                                 ...
                                 while($fila=mysql_fetch_row($res))
                                 {
									...
									$cad="var matrizControles=new Array();var objMatrizCtrl;";
                            ...
                            <td  >
                               ...
                               <?php
								...
								?>
									...
									/*console.log(queriesControles);
									console.log(diccionarioCtrl);
									*/
