...
var totalTareas=0;
var cargaAutomaticaFormulario=true;
...
Ext.onReady(inicializarFormulario);
...
function calibrar()
{
    ...
    cargaAutomaticaFormulario=false;
...
function inicializarFormulario()
{
	...
	mostrarMensajeProcesando('Preparando, espere un momento...');
	ejecutarFuncionesInicio();
	cargaAutomaticaFormulario=true;
    		...
    		funcionValidacion2=function()
            					{
                                    ...
                                    function funcAjax()
                                    {
                                        ...
                                        if(arrResp[0]=='1')
                                        {
                                        	...
                                        	var x;
                                           	return prepararFormularioEnvio('frmEnvio');
    ...
    var ocultarBotonesCtrl=false;
    
    if(gE('ocultarBotonesCtrl')&&(gE('ocultarBotonesCtrl').value=='1'))
    {
    	oE('sticky');
    }
...
function ponerFoco()
{
	if((typeof(ctrlEnfocar)=='undefined')||(ctrlEnfocar==''))
    	return;
    ...
    if(controlI !=null)
    {
        ...
        switch(parseInt(tipoCtrl))
        {
            ...
            case 10:
                ...
                {
                	CKEDITOR.instances['txt'+nomCtrl].focus();
                }
            break;
        }
	}
...
function validarFrm(idContenedor)
{
    ...
    {
    	if(Ext.getCmp(arrTextRich[x]))
	    	gE(arrTextRich[x]).value=Ext.getCmp(arrTextRich[x]).getValue();
        else
        	gE(arrTextRich[x]).value=obtenerValorTextEnriquecido('txt'+arrTextRich[x]);
    }    
	...
	if(validarFormularios(idContenedor))
    {
            ...
            for (ct=0;ct<aCamposGrid.length;ct++)
            {
                ...
                if(!gridCtrl.disabled)
                {
                	...
                	if(gridCtrl.enEdicion)
                    {
                    	...
                    	msgBox('El campo <b>'+bD(span.getAttribute('msgCampo'))+'</b> se encuentra en edici&oacute;n, primero debe guardar el registro',respAux);
                        return false;
                    }
                    ...
                    for(nAux=0;nAux<almacen.getCount();nAux++)
                    {
                        ...
                        for(nItem=0;nItem<camposColeccion.getCount();nItem++)
                        {
                            ...
                            if(objRegistro=='')
                                objRegistro='"'+camposColeccion.itemAt(nItem).name+'":"'+cv(valor,false,true)+'"';
                            else
                                objRegistro+=',"'+camposColeccion.itemAt(nItem).name+'":"'+cv(valor,false,true)+'"';
    	...
    	if(existeTabla==1)
        {
        	if(funcionValidacion2==null)
            {
                ...
                return prepararFormularioEnvio(idContenedor);
            }
            else
            {
                return funcionValidacion2();
...
function crearTextoEnriquecido(id,idDivDestino,ancho,alto,valor,conf,habilitado,correccionOrtografica)
{
	var valor=bD(valor);
	valor=valor.replace(/\n/gi, '');
    ...
    var texto=crearRichTextV2(id,idDivDestino,ancho,alto,conf,valor,habilitado,correccionOrtografica);
...
function obtenerValorCampo(campo)
{
	...
	var sufijo=campo.substr(campo.length-3);
	
	switch(sufijo)
    {
    	case 'dte':
        	var ctrl=gEx('f_sp'+campo);
			if(ctrl.getValue()!='')
            	return ctrl.getValue().format('Y-m-d');
            return '';
        
        break;
        default:
        	var control=gE(campo.trim());
            ...
            if(tipo=='SPAN')
            {
            	var arrControles=gEN(campo.trim());
                if(arrControles.length>0)
                {
                	control=arrControles[0];
                    tipo='INPUT';
                }
            }
            try
...
function limpiarControl(nControlAux,tipo)
{
	switch(tipo)
    {
        ...
        case 10:
            ...
            {
                establecerValorTextEnriquecido('txt'+nControlAux,'');
                gE(nControlAux).value='';
            }
        break;
        case 8:
        case 21:
...
function desHabilitarControl(nControlAux,tipo,idCtrl)
{
	...
	switch(tipo)
    {
        ...
        case 10:  //ok
            ...
            {	
            	deshabilitarTextoEnriquecido('txt'+nControlAux);
                var ctrlAux=gE(nControlAux);
                var attrVal=ctrlAux.getAttribute('val');
                if(attrVal!=null)
                {
...
function habilitarControl(nControlAux,tipo,idCtrl)
{
	switch(tipo)
    {
        ...
        case 10:
            ...
            {
            	habilitarTextoEnriquecido('txt'+nControlAux);
                var ctrlAux=gE(nControlAux);
                var attrVal=ctrlAux.getAttribute('valAux');
                if(attrVal!=null)
                {
...
function ocultarControl(nControlAux,tipo,nControl,idCtrl)
{
	...
	switch(tipo)
    {
        ...
        case 4:
            ...
            if(!ctrlAux)
            {
            	ctrlAux=gE(nControlAux.substr(0,nControlAux.length-3)+'arr');
                if(ctrlAux)
                {
                	nControlAux=nControlAux.substr(0,nControlAux.length-3)+'arr';
                	arrOpciones=true;
                }
			}
...
function mostrarControl(nControlAux,tipo,nControl,idCtrl)
{
	...
	switch(tipo)
    {
        ...
        case 29:
            ...
            gEx('grid_'+idCtrl).getView().refresh();
        break;
...
function funcionEventoCambio(ctrl,registro)
{
    ...
    if(registro)
    {
    	var sufijo=ctrl.id.substr(ctrl.id.length-3);

    	switch(sufijo)
        {
        	case 'dte':
            	nControl=ctrl.id.replace('f_sp','');
            break;
        	default:
    ...
    if(posControl!=-1)
    {
        ...
        for(x=0;x<aQueries[x];x++) 
		{
            ...
            if(posAux!=-1)
            {
				...
				if((arrControles!='')&(existeValorArreglo(aQueryTmp1,aQueries[x])==-1))
                {
                    if(arrQueriesResueltas=='')
                        arrQueriesResueltas=objAux;
                    else
                        arrQueriesResueltas+=','+objAux;
                    aQueryTmp1.push(aQueries[x]);
                }

                	
            }
       	}        
        arrQueriesResueltas='{"idFormulario":"'+gE('idFormulario').value+'","idRegistro":"'+gE('idRegistroG').value+'","cargaAutomaticaFormulario":"'+(cargaAutomaticaFormulario?1:0)+'","aQueries":['+arrQueriesResueltas+']}';
       
        function funcAjaxRegistroFormularioV2(peticion_http)
        {
            var resp=peticion_http.responseText;
                    ...
                    switch(o.tipoCtrl)
                    {
                    	case '4':
							...
							if(tieneRenderer)
                            {
                                ...
                                {
                                	o.valor[ctAux][1]=funcRenderer(o.valor[ctAux][1],controlFrm);
                                }
                            }
                            ...
                            if(o.valorCampo!="")
                            {
                            	selElemCombo(controlFrm,o.valorCampo,true);
                        ...
                        case '30':
                            ...
                            {
                                if(tieneRenderer)
                                    ctrlDestino.innerHTML=funcRenderer(o.valor,controlFrm);
                                else
        ...
        obtenerDatosWebSincrono('../paginasFunciones/funcionesFormulario.php',funcAjaxRegistroFormularioV2, 'POST','funcion=72&nQuery='+nQuery+'&aQ='+arrQueriesResueltas,true);
    }
}
...
function textoBotonRenderer(val)
{
	if(val!='')
    {
    	...
    	return '<a href="javascript:window.parent.visualizarDocumentoAdjuntoB64(\''+bE(idDocumento)+'\',\''+bE(extension)+'\')"><img width="13" height="13" src="../images/magnifier.png">&nbsp;&nbsp;'+arrDatos[0]+'</a>';
    }
    return '';
}
...
function esRegistroFormulario()
{
	if(gE('esRegistroFormulario'))
    	return true;
    return false;
}
...
function abrirTextoCompleto(i,r)
{
	if(bD(r)!='-1')
    {
        ...
        obj.ancho='100%';
        obj.alto='100%';
        obj.url='../modeloPerfiles/verDetalleCampoTexto.php';
        obj.params=	[['iC',i],['iR',r]];
...
{
	if(esRegistroFormulario())
		return validarFrm('frmEnvio')
}

function resolverConsultaQuery(query,arrParams)
{
	var x;
    var nCtrl;
    var valor;
    ...
    for(x=0;x<arrParams.length;x++)
    {
        ...
        valor=obtenerValorControl(gE(nCtrl));
        patron=new RegExp('\\['+nCtrl+']', "gi");
        query=query.replace(patron,valor);
...
function selElementoCheckBoxMultiple(check, value)
{
	var arrID=check.id.split('__');
    var nombreControl=  '_'+arrID[1]; 
    gE(nombreControl).value=value;
