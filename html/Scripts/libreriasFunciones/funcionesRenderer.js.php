...
{
	var number=number+'';
    var billions_final_string='';
  	var millions_final_string='';
    var milesMillions_final_string='';
    ...
    var billones;
   	...
   	if (number == 0 || number == "") 
   	{ 
   		centenas_final_string=" cero "; // amount is zero (cero). handle it externally, to function breakdown 
  	} 
   	else 
   	{ 
   
   		billones=ObtenerParteEntDiv(number, 1000000000000); // first, send the millions to the string 
        ...
        billions_final_string=string_literal_conversion(billones)+descriptor+' '; 
   		number = mod(number, 1000000000000); 

   		milesMillones=ObtenerParteEntDiv(number, 1000000000); // first, send the millions to the string 
		if (milesMillones != 0)
     	{                      
        ...
        milesMillions_final_string = string_literal_conversion(milesMillones)+descriptor+' '; 
	    ...
	    number = mod(number, 1000000);          
     	...
     	if ((millions != 0)||(milesMillones!=0))
     	{                      
      
        	if ((millions == 1) &&(milesMillones==0))
         	{              
            	descriptor= " millon ";  
         	} 
         	else 
      	...
      	millions_final_string = milesMillions_final_string + string_literal_conversion(millions)+descriptor; 
   	...
   	cad = billions_final_string+millions_final_string+thousands_final_string+centenas_final_string; 
...
function formatearCampoFormularioReparto(val)
{

	var valor=removerComillasContenedorasCadena(val);
	if((valor=='')||(valor=='N/E'))
    	valor='PENDIENTE POR REPARTO';
    
    return valor;
}


function formatearCampoFormularioFechaReparto(val)
{
	var valor=removerComillasContenedorasCadena(val);
	if((valor=='')||(valor=='N/E'))
    	return 'PENDIENTE POR REPARTO';
