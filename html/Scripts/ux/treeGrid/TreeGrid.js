...
Ext.ux.tree.TreeGrid = Ext.extend(Ext.tree.TreePanel, {
	...
	getCheckNodes:function()
				{
					this.arrNodosChecados=new Array();
					this.obtenerNodosChecados(this.root);
					return this.arrNodosChecados;
				},
	
	obtenerNodosChecados:function(nodo)
						{
							var x;
							for(x=0;x<nodo.childNodes.length;x++)
							{
								
								if(nodo.childNodes[x].getUI().isChecked)
								{
									if(nodo.childNodes[x].getUI().isChecked())
									{
										this.arrNodosChecados.push(nodo.childNodes[x]);
									}
								}
								this.obtenerNodosChecados(nodo.childNodes[x]);
							}
						},
	
	
	checkNode:function(id,valor)
			{
				var x;
				var nodo=this.getNode(id);

				if(nodo)	
				{
					nodo.getUI().toggleCheck(valor);
				}
			},
	
	checkAllNodes:function(valor)
					{
						this.cambiarValorNodos(this.root,valor);
					},
		
		
	cambiarValorNodos:function(nodo,valor)
						{
							var x;
							for(x=0;x<nodo.childNodes.length;x++)
							{
								...
								this.cambiarValorNodos(nodo.childNodes[x],valor);
							}
						},
			
		
	getNode:function(id,nodo)
			{
				var nodoInicio=this.root;
				if(nodo)
					nodoInicio=nodo;
				var nodoEnc=this.buscarNodo(nodoInicio,id);
				return nodoEnc;
			},
	
	buscarNodo:function(nodo,id)
				{
					var x;
					var nodoEnc=null;
					for(x=0;x<nodo.childNodes.length;x++)
					{
						if(nodo.childNodes[x].id==id)
						{
							nodoEnc=nodo.childNodes[x];
						}
						else
						{
							nodoEnc=this.buscarNodo(nodo.childNodes[x],id);
						}
						if(nodoEnc!=null)
							return nodoEnc;
					}	
