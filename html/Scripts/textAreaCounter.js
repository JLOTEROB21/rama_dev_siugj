...
(function($){  
	...
	{   
		...
		function getCleanedWordString(content)
		{
			...
			var left_trimmedStr = limpiarEspacioDuplicados(fullStr);//.replace(initial_whitespace_rExp, "_");
		...
		function limpiarEspacioDuplicados(cadena)
		{
			var count=0;
		...
		function cortarPalabras(content)
		{
			...
			var fullStr = content + " ";
			var left_trimmedStr = limpiarEspacioDuplicados(fullStr);
			//var initial_whitespace_rExp = /^[^A-Za-z0-9áéíóúÁÉÍÓÚÑñ]+/gi;
				...
				if(non_alphanumerics_rExp.test(caracter))
				{
					if(palabra.trim()!='')
					{
						...
						if(nPalabras>maxWords)
						{
							...
							msgBox('Ha excedido el n&uacute;mero m&aacute;ximo de palabras: '+maxWords+' palabras',resp);
							break;	
						}
					}
					cadenaFinal+=palabra;
