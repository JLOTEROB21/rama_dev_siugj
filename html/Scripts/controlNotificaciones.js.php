...
function verificarNotificaciones(iT)
{
	...
	function funcAjax(peticion_http)
    {
        ...
        if(arrResp[0]=='1')
        {
            ...
            gE('total_'+iT).innerHTML='<div>2-'+obj.registrosPendientes+'</div>';
            ...
            gE('divPendientes').innerHTML='('+obj.registrosPendientes+')';  
            gE('divVencidas').innerHTML='('+obj.registrosVencidos+')';  
            gE('divPorVencer').innerHTML='('+obj.registrosPorVencer+')';  
            gE('divRealizadas').innerHTML='('+obj.totalTareasAtendidas+')';    
            
            gE('total_tareas').innerHTML='<div>'+obj.registrosPendientes+'</div>';
            
            if(parseInt(obj.registrosVencidos)>0)
            {
            	gE('total_tareas').style.background='#9b2027';   
            }
            else
            {
            	if(parseInt(obj.registrosPorVencer)>0)
                {
                	gE('total_tareas').style.background='#f3af3d';  
                }
                else
                {
                	gE('total_tareas').style.background='#4caf50';  
                }
            }
	...
	function funcAjaxAlerta(peticion_http)
    {
        ...
        if(arrResp[0]=='1')
        {
            ...
            if(obj.totalAlertas!='0')
            {
            		...
            		var totalNotificaciones=obj.arrRegistros.length;
            		var x;
                    for(x=0;x<totalNotificaciones;x++)
                    {	
                        arrNotificaciones+=           	'<tr>'+
                                                          '<td align="center" class="tdDropdown tdDropdownHover">'+
                                                              '<table width="100%">'+
                                                                  '<tr>'+
                                                                      '<td width="30" valign="top" align="left" style="padding-left:10px"><img src="../principalPortal/imagesSIUGJ/bulletTask_yellow.png">'+
                                                                      '</td>'+
                                                                      '<td  align="left">'+
                                                                          '<a href="javascript:cerrarSesionPrincipal()">'+obj.arrRegistros[x].descripcion+
                                                                          '</a><br>'+
                                                                      '<div style="font-weight: 300; font-size: 10px;">'+Date.parseDate(obj.arrRegistros[x].fechaRecordatorio,'Y-m-d H:i:s').format('d/m/Y')+
                                                                      '<br>CUP: '+obj.arrRegistros[x].carpetaAdministrativa+'</div>'+
                                                                      '</td>'+
                                                                  '</tr>'+
                                                              '</table>'+
                                                          '</td>'+
                ...
                gE('total_notificaciones').innerHTML='<div>'+obj.totalAlertas+'</div>';
                
                gE('dropNotificaciones').innerHTML='<table width="100%">'+
                                                        arrNotificaciones+
                                                        '<tr>'+
                                                           ' <td align="center" class="tdDropdownHover">'+
                                                                '<table width="100%">'+
                                                                    '<tr>'+
                                                                        ...
                                                                        '<td  colspan="3" align="center"><a href="javascript:mostrarTableroAlertasNotificaciones()">Ver todas</a></td>'+
                                                                    '</tr>'+
                                                                '</table>'+
                                                            '</td>'+
           ...
           {
           		gE('total_alertas').innerHTML='<div>1-'+obj.totalAlertas+'</div>';
                gE('total_notificaciones').innerHTML='<div>'+obj.totalAlertas+'</div>';
                gE('dropNotificaciones').innerHTML='<table width="100%">'+
                                                        '<tr>'+
                                                           '<td align="center" class="tdDropdownHover">'+
                                                                '<table width="100%">'+
                                                                    '<tr>'+
                                                                        '<td  colspan="3" align="center"><div>No existen alertas/notificaciones del d&iacute;a</div></td>'+
                                                                    '</tr>'+
                                                                '</table>'+
                                                            '</td>'+
...
{
	gE('total_'+iT).innerHTML='<div>3-'+total+'</div>';
}