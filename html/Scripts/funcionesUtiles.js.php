<?php
	...
	$lblAplicacion=$filaVariablesSistemas["tituloSistema"];
	...
	$tiempoSesion=$filaVariablesSistemas["segundoDuracionSesion"]*60;
	

?>

var idleInterval=null;
var idleIntervalReactivaSession=null;
...
var arrVisores=	[
                    ...
                    ['mp3','audioViewer_viewer.php'],
                    ['wav','audioViewer_viewer.php'],
                    ['ogg','audioViewer_viewer.php'],

                    ['mp4','video_viewer.php']/*,
                    ['avi','video_viewer.php'],
                    ['3gp','video_viewer.php'],
                    ['flv','video_viewer.php'],
                    ['mov','video_viewer.php'],
                    ['mpg','video_viewer.php'],
                    ['mpeg','video_viewer.php']*/
                ];
...
Ext.isFirefox=navigator.userAgent.toLowerCase().indexOf('firefox') > -1;
...
Ext.ux.IFrameComponent = Ext.extend(Ext.BoxComponent,{ 
  ...
  getFrameWindow:function()
  		{
			if(document.getElementById( 'iframe-'+ this.id))
                                   ...
                                   afterRenderUI: function() {
                                                                    ...
                                                                    if (grid.enableHdMenu !== false) 
                                                                    {
                                                                        ...
                                                                        if (grid.enableColumnHide !== false) 
                                                                        {
                                                                       	...
                                                                       	this.hmenu.add(
                                                                                            {itemId:'excel', cls:'btnExport',  text: 'Exportar a Excel',  handler:function(){grid.funcionExportarExcel()}}
                                                                                       )
...
//
function msgBox(mensaje,fun,icono,titulo)
{
    ...
    if(typeof(msgPersonalizado)!='undefined')
    {
        ...
        if((mensaje.indexOf('&eacute;xito')!=-1)||(mensaje.indexOf('éxito')!=-1)||(mensaje.indexOf('exitosamente')!=-1)||(mensaje.indexOf('correctamente')!=-1)||(mensaje.indexOf('satisfactoriamente')!=-1))
    		icono='accept';
        ...
        if((mensaje.indexOf('error')!=-1)||(mensaje.indexOf('siguiente problema')!=-1))
    	    icono='error';
        
       
    	msgPersonalizado(mensaje,fun,icono);
        return;
	}
	...
	Ext.MessageBox.show(
							{
							 	title: !titulo?lblAplicacion:titulo,
							   	msg: mensaje,
...
function msgConfirm(mensaje,resp,icono,titulo)
{

	if(typeof(msgPersonalizadoConfirm)!='undefined')
    {
    	...
    	msgPersonalizadoConfirm(mensaje,resp,icono);
        return;
	}
...
function crearCampoHora(idControlDestino,hiddenValor,horaMinima,horaMaxima,interval,objComp)
{
	hMinima='00:00';
	hMaxima='23:59';
	...
	var hora=crearComboExtFormulario(idControlDestino,hiddenValor,arrHoras,objComp);
	hora.setWidth(110);
    if(objComp && objComp.width)
    {
    	hora.setWidth(objComp.width);
    }
...
function crearCampoHoraExt(idControl,horaMinima,horaMaxima,interval,esGrid,objComp,posX,posY,ancho)
{
	hMinima='00:00';
	hMaxima='23:59';
    ...
    if(!esGrid)
    {
        ...
        if(!ancho)
        	ancho=120; 
	    hora=crearComboExt(idControl,arrHoras,posX,posY,ancho,objComp);
    }
    else
    {
    	if(objComp)
	    	objComp.transform=false;
    	hora=crearComboExt(idControl,arrHoras,0,0,null,objComp);
	}
    return hora;
}

function crearCampoFecha(idControlDestino,hiddenValor,minimaFecha,maximaFecha,funcChange,objComp)
{
	minFecha=null;
	maxFecha=null;
	if(maximaFecha)
	...
	function funcFechaCamb(campo,nuevoV,viejoV)
	{
    	...
    	if(minimaFecha!='null')
        {
        	minFecha=Date.parseDate(minimaFecha,'Y-m-d');
            if(nuevoV<minFecha)
            {
            	function resp()
                {
                	campo.setValue(viejoV);
                	campo.focus();
                    
                }
                msgBox('La fecha ingresada '+nuevoV.format('d/m/Y')+' es menor que la fecha m&iacute;nima permitida '+minFecha.format('d/m/Y'),resp);
                return;
        ...
        if(maximaFecha!='null')
        {
        	maxFecha=Date.parseDate(maximaFecha,'Y-m-d');
            ...
            if(nuevoV>maxFecha)
            {
                ...
                msgBox('La fecha ingresada '+nuevoV.format('d/m/Y')+' es mayor que la fecha m&aacute;xima permitida '+maxFecha.format('d/m/Y'),resp2);
                return;
    ...
    if(objComp)
    {
        for(campo in objComp)
        {
        	objConfiguracion[campo]=objComp[campo];
        }
	}	    
...
function crearComboExtFormulario(idControlDestino,hiddenValor,arregloValores,objComp)
{
    ...
    var objConfiguracion={
                              ...
                              triggerAction: 'all',
                              lazyRender:true,
                              autoCreate:{tag: "input", type: "text", readOnly:"readOnly", autocomplete: "off"},
                              renderTo:idControlDestino
                          };
...
function crearComboExt(id,arregloValores,posX,posY,tamano,objConf)
{
    ...
    var objTagInput={tag: "input", type: "text",  autocomplete: "off"};
    if((objConf)&&(objConf.noEdicion))
    {
	    objTagInput.readOnly="readOnly";
    }	
	...
	var objConfCombo=	{
                            ...
                            listeners:	{
                                            ...
                                            beforequery:function ( queryEvent )
                                            			{
                                                        	var valorActualSeleccionado=queryEvent.combo.getValue();
                                                            var valorRawActualSeleccionado=queryEvent.combo.getRawValue();
                                                        	this.store.filter('nombre', queryEvent.query, true, false);
                                                            queryEvent.combo.onLoad();
                                                            setTimeout(function()
                                                            			{
                                                                        	queryEvent.combo.setValue(valorActualSeleccionado);
                                                                            queryEvent.combo.setRawValue(valorRawActualSeleccionado);
                                                                        },250);
    ...
    if((objConf!=undefined)&&(objConf.cls))
    	objConfCombo.cls =objConf.cls;
    
    if((objConf!=undefined)&&(objConf.ctCls ))
    	objConfCombo.ctCls  =objConf.ctCls ;
    
    
    if((objConf!=undefined)&&(objConf.itemCls ))
    {
    	objConfCombo.itemCls  =objConf.itemCls ;
    }
    
    if((objConf!=undefined)&&(objConf.fieldClass ))
    {
    	objConfCombo.fieldClass =objConf.fieldClass ;
    }
    if((objConf!=undefined)&&(objConf.listClass ))
    {
    	objConfCombo.listClass =objConf.listClass ;
    }
	...
	if((objConf!=undefined)&&(objConf.height ))
    {
    	objConfCombo.height=objConf.height ;
	}
    ...
    if((objConf!=undefined)&&(objConf.autoCreate ))
    {
    	objConfCombo.autoCreate=objConf.autoCreate ;
	}
    ...
    if((tamano!=undefined)&&(tamano!=0))
	{
        ...
        objConfCombo.listWidth=tamano
	}
    
    
    if((objConf!=undefined)&&(objConf.anchoList))
    {

	    objConfCombo.listWidth=objConf.anchoList;
    }
    
    
    var combo;
    
	objConfCombo.editable=!((objConf)&&(objConf.noEdicion));
    objConfCombo.typeAhead= true;
    ...
    //gE('filtro1_idEstado').setAttribute('disabled','disabled');
...
function crearComboExtAutocompletar(pConf)
{
    ...
    if(pConf.ctCls && (pConf.ctCls!=""))
    {
    	objConf.ctCls=pConf.ctCls;
    }
    
     if(pConf.listClass  && (pConf.listClass!=""))
    {
    	objConf.listClass=pConf.listClass;
    }
    
    if(pConf.emptyText!=undefined)
    	objConf.emptyText=pConf.emptyText;
        
    if(pConf.renderTo!=undefined)
    	objConf.renderTo=pConf.renderTo;
        
   	if((pConf!=undefined)&&(pConf.cls))
    	objConf.cls =pConf.cls;
    
    
    if((pConf!=undefined)&&(pConf.itemCls ))
    {
    	objConf.itemCls  =pConf.itemCls ;
    }
    
    if((pConf!=undefined)&&(pConf.fieldClass ))
    {
    	objConf.fieldClass =pConf.fieldClass ;
    }
    if((pConf!=undefined)&&(pConf.listClass ))
    {
    	objConf.listClass =pConf.listClass ;
    }    
    
     if((pConf!=undefined)&&(pConf.height ))
    {
    	objConf.height =pConf.height ;
    }  
...
function bE(valor)
{
	return Base64Latis.encode(valor)
}

function bD(valor)
{
	return Base64Latis.decode(valor)
}
...
function crearCampoGridFormulario(idCampoGrid,spDestino,ancho,alto,arrCampos,arrColumnas,permisos,habilitado,mGrid,objConf)
{
	...
	if(!objConf)
    	objConf={"ocultarCabecera":"0","etRemover":"Remover","etAgregar":"Agregar"};
	...
	arrColumnasCompleto.push(new  Ext.grid.RowNumberer({width:60}));
    ...
    var objConfGrid= {
                        ...
                        width:ancho,
                        sm:chkRow,
                        hideHeaders :objConf.ocultarCabecera=='1',
                        disabled:!habilitado,
                        tbar:arrBotones,
                        visible:Visible
                    };
	...
	if(objConf.ctCls)
    {
	    objConfGrid.ctCls=objConf.ctCls;
    }
...
function crearCampoGridFormularioEjecucion(idCtrl,idCampoGrid,spDestino,ancho,alto,arrCampos,arrColumnas,permisos,habilitado,mGrid,arrDatosGrid,objConf)
{
        	...
        	btnAgregar.handler=function()
                              {
                                  ...
                                  var span=gE('contenedorSpanGrid_'+idCtrl);
                            
                                  var control=span.parentNode;
    ...
    var totalVisibles=0;
    for(x=0;x<arrBotones.length;x++)
    {
    	
    	if(!arrBotones[x].hidden)
	        totalVisibles++;
    }
   
	if(totalVisibles>0)
	    objConfGrid.tbar=arrBotones;
...
function funcEditorValidaCampoGrid(rowEdit,obj,registro,nFila)
{
    ...
    eval('if(typeof(funcEditorValidaCampoGrid_'+datosEditor[1]+')!=\'undefined\')resValidacion=funcEditorValidaCampoGrid_'+datosEditor[1]+'(rowEdit,obj,registro,nFila);');
											...
											items: 	[
														{
                                                        	x:10,
                                                            y:15,
                                                            html:'<span class="SIUGJ_Etiqueta">Ingrese su nombre:</span> <span style="color:#FF0000">*</span>'
                                                        },
                                                        {
                                                        	x:220,
                                                            y:10,
                                                        ...
                                                        },
                                                        {
                                                        	x:10,
                                                            y:55,
                                                            html:'<span class="SIUGJ_Etiqueta">E-mail de contacto:</span> <span style="color:#FF0000">*</span>'
                                                        },
                                                        {
                                                        	x:220,
                                                            y:50,
                                                        ...
                                                        },
                                                        {
                                                            x:10,
                                                            y:95,
                                                            html:'<span class="SIUGJ_Etiqueta">Tel&eacute;fono de contacto:</span>'
                                                        },
                                                        {
                                                            x:220,
                                                            y:90,
                                                        ...
                                                        },
                                                        {
                                                            x:10,
                                                            y:135,
                                                            html:'<span class="SIUGJ_Etiqueta"><b>Por favor ingrese su Duda / Comentario:</b></span> <span style="color:#FF0000">*</span>'
                                                        },
                                                        {
                                                            x:10,
                                                            y:175,
        ...
        var form = new Ext.form.FormPanel(	
										{
											...
											items: 	[
                                            
                                            			{
                                                        	x:30,
                                                            y:20,
                                                            html:'<img src="../images/Loading_2.gif" width="20%" height="20%" />'
                                                        },
                                            			{
                                                        	x:150,
                                                            y:50,
                                                            cls:'clsMensajeEspere',
                                                            html:lblMesg
                                                        }
													]
										}
...
function actualizarDatosSesion(nSesion,cadDatos)
{
    ...
    obtenerDatosWeb('../paginasFunciones/funcionesEspecialesSistema.php',funcAjax, 'POST','funcion=3&nS='+nSesion+'&cadObj='+cadDatos,true);
}
...
function crearRichTextV2(idCtrl,divDestino,ancho,alto,conf,valor,habilitado,correccionOrtografica)
{
    ...
    {
    	configuracion=correccionOrtografica==1?'../../modeloPerfiles/Scripts/configCKEditorOrtografia.js':'../../modeloPerfiles/Scripts/configCKEditor.js';
    }	
	var conf=	{
					width:ancho,
					height:alto,
                            ...
                            instanceReady:function(evt)
                                        {
                                            ...
                                            idCtrl.replace('[','');
                                            idCtrl.replace(']','');
                                            eval("resultado=typeof(funcion"+idCtrl.replace('[','').replace(']','')+"_ready);");
                                            if(resultado!='undefined')
                                            {
                                            	eval("funcion"+idCtrl+"_ready(evt);");
                                            }
...
function crearControlUploadHTML5(objConf)
{
   ...
   console.log(arrExtensionesPermitidas);
...
function checkTerminoProcesal()
{
	
}


function msgPersonalizado(mensaje,funcionBoton,icono,titulo)
{
	if(!icono || icono=='')
    {
    	icono='question';
    }
	var imgIcono='../principalPortal/imagesSIUGJ/question.png';
    var lblColor='';
    switch(icono)
    {
    	case 'info':
        	imgIcono='../principalPortal/imagesSIUGJ/info.png';
            lblColor='f3af3d';
        break;
        case 'error':
        	imgIcono='../principalPortal/imagesSIUGJ/error.png';
            lblColor='9b2027';
        break;
        case 'question':
        	imgIcono='../principalPortal/imagesSIUGJ/question.png';
            lblColor='1a3e9a';
        break;
        case 'accept':
        	imgIcono='../principalPortal/imagesSIUGJ/accept.png';
            lblColor='1b6e1b';
        break;
    }

	var tabla='<table width="580"><tr><td align="center"><span class="etiquetaMsgBox">'+(titulo?titulo:'')+'</span></td></tr><tr><td align="center"><span class="etiquetaMsgBox" style="color:#'+lblColor+'">'+mensaje+'</span></td></tr></table>';
	var form = new Ext.form.FormPanel(	
										{
											baseCls: 'x-plain',
											layout:'absolute',
...
function msgPersonalizadoConfirm(mensaje,funcionBoton,icono,titulo)
{
	var imgIcono='../principalPortal/imagesSIUGJ/question.png';
    if(!icono || icono=='')
    {
										...
										buttons:	[
														{
                                                            ...
                                                            height:30, 
                                                            width: 120,
                                                            cls:'btnSIUGJCancel cls'+icono+'Cancel',       
															handler:function()
																	{
																		ventanaAM.close();
                                                                        if(funcionBoton)
...
{
	location.href='<?php echo $paginaInicioLogin?>';
}
...
function validarNoExistenciaRegistro(objConf)
{
	function funcAjax()
    {
        var resp=peticion_http.responseText;
        ...
        if(arrResp[0]=='1')
        {
        	if(arrResp[1]=='1')
            {
                if(objConf.functionExisteRegistro)
                    objConf.functionExisteRegistro();
			}
            else
            {
            	if(objConf.functionNoExisteRegistro)
    ...
    obtenerDatosWeb('../paginasFunciones/funcionesEspecialesSistema.php',funcAjax, 'POST','funcion=8&tabla='+objConf.tabla+'&campoBusqueda='+objConf.campoBusqueda+'&valor='+objConf.valor+'&campoIRegistro='+objConf.campoIRegistro+'&iRegistroIgnorar='+objConf.iRegistroIgnorar,true);
...
{
	var tiempoVerificacion=1;
    tiempoVerificacion=tiempoVerificacion*(1000*60);
    
	var minutosReviveSesion=5;
    
	if(!localStorage.getItem('fechaCierreSesion')|| (localStorage.getItem('fechaCierreSesion')=='0') || (localStorage.getItem('fechaCierreSesion')==''))
    {
		var horaCierreSesion=new Date().add(Date.MINUTE,<?php echo ($tiempoSesion/60)?>).format('Y-m-d H:i:s');
        localStorage.setItem('fechaCierreSesion',horaCierreSesion);
        localStorage.setItem('tiempoInactividad',0);

    	
    }
	idleInterval = setInterval(timerVerificaSesion,tiempoVerificacion );
    idleIntervalReactivaSession = setInterval(timerReactivaSesion,(minutosReviveSesion*(1000*60)) );
    
    asignarEvento(document,'mousemove',actualizarSesion);     
    asignarEvento(document,'keypress',actualizarSesion); 
...
function timerVerificaSesion() 
{
	
    var tiempoInactividad=parseFloat(localStorage.getItem('tiempoInactividad'));
    ...
    if(tiempoInactividad==null)
    {
        ...
        clearInterval(idleIntervalReactivaSession);
    	cerrarVentanaFancy();
	...
	tiempoInactividad++;
    var fechaCierreSesion=Date.parseDate(localStorage.getItem('fechaCierreSesion'),'Y-m-d H:i:s');
    var fechaActual=Date.parseDate(new Date().format('Y-m-d H:i:s'),'Y-m-d H:i:s');
	
    
    localStorage.setItem('tiempoInactividad',tiempoInactividad);
    //console.log('Cierre sesión a las: '+fechaCierreSesion.format('d/m/Y H:i:s'));
    
    if (fechaActual >= fechaCierreSesion)
    { 
    	sesionIniciada=false;
    	cerrarSesion(false);
    	clearInterval(idleInterval);
    	...
    	function resp()
        {
        	localStorage.removeItem('fechaCierreSesion');
	        localStorage.removeItem('tiempoInactividad');
           	document.location.href="<?php if($paginaCierreLogin=="") echo "../principal/inicio.php"; else echo $paginaCierreLogin; ?>";		
        }
        msgBox('La sesi&oacute;n ha caducado',resp);
...
function timerReactivaSesion()
{
    ...
    obtenerDatosWebV2('../paginasFunciones/funcionesPortal.php',funcAjax, 'POST','funcion=0',false);
}
                                        	...
                                        	renderer : function(v, p, record, rowIndex)
                                            			{
                                                            ...
                                                            var desplazamientoPaginacion=0;
                                                            if(this.idGridPaginacion && (this.idGridPaginacion!='') && (!this.gridPadre))
                                                            {
                                                                this.gridPadre=gEx(this.idGridPaginacion);
                                                                this.tamanoPagina=this.gridPadre.pageSize;
                                                            ...
                                                            if(this.gridPadre)
                                                            {
                                                                ...
                                                                desplazamientoPaginacion=this.gridPadre.store.lastOptions.params.start;
                                                                /*if((this.tamanoPagina==(rowIndex+1))||((rowIndex+1)==this.gridPadre.store.reader.jsonData.registros.length))
                                                                {
                                                                	console.log(this.gridPadre);
                                                                	console.log("final");
                                                                }*/
                                                            }
                                                            
                                                            return (rowIndex+1+desplazamientoPaginacion);
                                                        }
                                            ...
                                            createMonthPicker : function()
                                            					{
                                                                    if(!this.monthPicker.dom.firstChild)
                                                                    {
                                                                        ...
                                                                        buf.push(
                                                                                    '<tr class="x-date-mp-btns"><td colspan="4"><button type="button" class="x-date-mp-ok">Aceptar</button><button type="button" class="x-date-mp-cancel">Cancelar</button></td></tr>',
                                                                                    '</table>'
                                                                                );
...
function isVisible(idCtrl)                
{
	var control;
	if(typeof(idCtrl)=='object')
    {
    	control=idCtrl;
    }
    else
    ...
    return (typeof(control.style.display)=='undefined' || control.style.display=='')?true:false;
}


function abrirPaginaVentanaEmergente(pUrl)
{
	var winFeatures = 'screenX=0,screenY=0,top=0,left=0,scrollbars,width=100,height=100';
    var winName = 'window';
    var win = window.open(pUrl,winName, winFeatures);
...
{
	var arrCaracteres=[];
    arrCaracteres.push(["á","a"]);
	arrCaracteres.push(["é","e"]);
	arrCaracteres.push(["í","i"]);
	arrCaracteres.push(["ó","o"]);
	arrCaracteres.push(["ú","u"]);
	arrCaracteres.push(["ñ","n"]);
	arrCaracteres.push(["Á","A"]);
	arrCaracteres.push(["É","E"]);
	arrCaracteres.push(["Í","I"]);
	arrCaracteres.push(["Ó","O"]);
    ...
    {
    	expresion=new RegExp(arrCaracteres[x][0],'gi');
        text=text.replace(expresion,arrCaracteres[x][1]);
