<?php session_start();
	...
	$idProceso=bD($_GET["idProceso"]);
	
	$consulta="(SELECT if(f.tipoFormulario=0,f.nombreTabla,(select tablaAsociada from 200_modulosPredefinidosProcesos where idModulo=f.titulo)) as nTablaOriginal,
			f.nombreFormulario,if(f.tipoFormulario=0,'Formulario dinámico','Sistema') as tFormulario,pr.nombre FROM 203_elementosDTD p,900_formularios f,
			4001_procesos pr WHERE pr.idProceso=f.idProceso and f.idFormulario=p.idFormulario 
			AND p.idProceso=".$idProceso.") union (select f.nombreTabla,f.nombreFormulario,'Formulario dinámico' as tFormulario,pr.nombre from 900_formularios f,4001_procesos pr where 
			f.idProceso=".$idProceso." and f.formularioBase=1 and pr.idProceso=f.idProceso) order by nombreFormulario";
	$arrModulosProceso=$con->obtenerFilasArreglo($consulta);
?>
	...
	var tblGrid=	new Ext.grid.EditorGridPanel	(
                                                        {
                                                            ...
                                                            tbar:	[
                                                                        ...
                                                                        },
                                                                        {
                                                                            ...
                                                                            cls:'x-btn-text-icon',
                                                                        	text:'Agregar tabla del proceso',
                                                                            handler:function()
                                                                            		{
                                                                                    	mostrarVentanaAlmacenesProceso();
                                                                                    }
                                                                        }
                                                            		]
                                                        }
																					...
																					click:function()
																						{
                                                                                            ...
                                                                                            function funcAjax()
                                                                                            {
                                                                                            	var idReporte=gE('idConsultaExp').value;
                                                                                                var resp=peticion_http.responseText;
                                                                                                arrResp=resp.split('|');
                                                                                                var cadObj;
                                                                                                if(arrResp[0]=='1')
                                                                                                {
                                                                                                    ...
                                                                                                    if(nodoSel.attributes.tipo=='ad')
                                                                                                    	...
                                                                                                    	tAlmacen=1;
                                                                                                    cadObj='"tipoDataSet":"6","tipoAlmacen":"'+tAlmacen+'","parametros":"'+cadParametros+'","nombreDataSet":"@nombreDS","descripcion":"@descripcion","idReporte":"'+idReporte+'","tabla":"'+objFinal.dTabla.nTablaO+'","camposProy":"'+objFinal.campos+'","operacion":"'+objFinal.operacion+'","tokenSql":['+arrTokens+']';
                                                                                                    cadObj="{"+cadObj+"}";
...
function ingresarParametroAlmacen()
{
    ...
    var arrTipoEntrada=[['1','Campo de tabla'],['7','Consulta auxiliar'],['5','Nuevo par\xE1metro'],['6','Par\xE1metro registrado'],['16','Valor de proceso'],['3','Valor de sesi\xF3n'],['4','Valor de sistema']];
