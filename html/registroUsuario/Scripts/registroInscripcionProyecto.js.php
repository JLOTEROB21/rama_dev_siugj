...
function agregarAutor()
{
	...
	function funcAjax()
    {
        ...
        if(arrResp[0]=='1')
        {
                ...
                var datosAutor='{"tipoInscripcion":"'+gE('tipoInscripcion').value+'","sexo":"'+sexo+'","idProceso":"'+idProceso+'","apPaterno":"'+cv(apPaterno.value)+'","apMaterno":"'+cv(apMaterno.value)+'","nombres":"'+cv(nombre.value)+'","email":"'+cv(mail)+
                			'","fechaNacimiento":"'+fNacimiento.getValue().format("Y-m-d")+'","telCasa":"'+gE('txtTelefono').value+'","telMovil":"'+gE('txtTelefonoMovil').value+'","idProyecto":"'+gE('idProyecto').value+
                ...
                function funcGuardar()
                {
                    ...
                    if(arrResp[0]=='1')
                    {
                    	function respMail()
                        {
                     	...
                     	msgBox('Su cuenta ha sido registrada de manera exitosa, en breve recibir&aacute; un correo electr&oacute;nico con sus datos de acceso.<br><span style="font-size:10px !important"><b>Si no recibe un e-mail de confirmaci&oacute;n, verifique en su buzón de correo no deseado</b></span>',respMail,'btnOk');   
                        return;
                    }
                    else
