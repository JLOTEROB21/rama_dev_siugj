<?php session_start();
	...
	function obtenerResumenActuaciones()
	{
		...
		$consulta="SELECT * FROM _1056_tablaDinamica WHERE id__1056_tablaDinamica=".$idRegistro;
		$fRegistro=$con->obtenerPrimeraFilaAsoc($consulta);
		
		
		$consulta="SELECT * FROM 00013_registrosMacroProceso WHERE idRegistro=".$fRegistro["idRegistroElementoMacroProceso"];
		$fRegistroMacro=$con->obtenerPrimeraFilaAsoc($consulta);
		
		
		$arrRegistros="";
		$consulta="SELECT * FROM 00013_registrosMacroProceso WHERE carpetaAdministrativa='".$cAdministrativa."' AND tipoRegistro IN(4) AND 
					cveElemento IN('".$fRegistroMacro["cveElemento"]."')";
		...
		while($fila=mysql_fetch_assoc($res))
		{
			...
			$o='{"idRegistro":"'.$fila["idRegistro"].'","lblEtiquetaRegistro":"'.cv($fila["lblEtiquetaRegistro"]).'","fechaInicio":"'.$fila["fechaInicio"].
				'","fechaMaximaAtencion":"'.$fila["fechaMaximaAtencion"].'","situacionActual":"'.($fila["situacionActual"]==""?1:$fila["situacionActual"]).
