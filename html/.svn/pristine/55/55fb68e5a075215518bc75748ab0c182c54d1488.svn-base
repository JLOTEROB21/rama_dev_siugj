    	 ...
    	 function resp(btn)
		{
			if(btn=='yes')
			{
				function funcAjax()
				{
					...
					if(arrResp[0]=='1')
					{
						gEx('app-calendar').eventStore.remove(rec);
						gEx('app-calendar').hideEditForm();
					}
					else
					{
						msgBox('No se ha podido llevar cabo la operaci&oacute;n debido al siguiente error: '+' <br />'+arrResp[0]);
					}
				}
				obtenerDatosWeb('../paginasFunciones/funcionesAgenda.php',funcAjax, 'POST','funcion=2&idEvento='+rec.data.EventId,true);
			}
		}
		msgConfirm('Est&aacute; seguro de querer eliminar este evento?',resp)
