...
function llenarConstanciaBase($idDocumento,$idReferencia,$idRegistro,$idFormulario,$idFormularioBase,$idRegistroBase)
{
	...
	switch($idFormularioBase)
	{
		...
		break;
		case 1308:
			$consulta="SELECT fechaCambio FROM 941_bitacoraEtapasFormularios WHERE idFormulario=1308 AND idRegistro=".$idRegistroBase." AND etapaActual=2";
			$fechaRecepcion=$con->obtenerValor($consulta);
		break;
		default:
...
function funcionLlenadoDocumentoBaseGeneralSegundaInstancia($idFormularioBase,$idRegistroBase)
{
	...
	$arrValores["lblProcesoActual"]="";
	if($arrValores["codigoUnicoProcesoTribunalSuperior"]!="")
	{
		$arrValores["lblProcesoActual"]="<B>PROCESO JUDICIAL ACTUAL: </B>".$arrValores["codigoUnicoProcesoTribunalSuperior"];
	}
