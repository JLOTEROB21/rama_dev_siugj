<?php  session_start();
	...
	$contenidoPlatilla=leerContenidoArchivo($baseDir."/modulosEspeciales_SIUGJ/plantillas/acuseRadicacionNulidadLaudo.html");	
	...
	$consulta="SELECT nombreUnidad FROM _17_tablaDinamica d where claveUnidad='".$fRegistro["despachoAsignado"]."'";
	...
	if(file_exists($directorioDestino."/".$nombreDocumento.".pdf"))
	{
		...
		cambiarEtapaFormulario($idFormulario,$idRegistro,4.5,"",-1,"NULL","NULL",0);
