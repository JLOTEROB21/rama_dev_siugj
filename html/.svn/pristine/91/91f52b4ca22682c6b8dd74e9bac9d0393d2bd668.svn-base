<?php
	...
	function esSecretarioSustanciadorApelacionOrdinarioAuto($idRegistro,$actor)
	{
		...
		$consulta="SELECT COUNT(*) FROM 900_formularios WHERE idProceso=330 AND titulo='46'";
		$numReg=$con->obtenerValor($consulta);
		
		if($numReg>0)
		{
			$consulta="SELECT * FROM 7006_repartoManual WHERE idFormulario=944 AND idReferencia=".$idRegistro;
	...
	function permitirFirmarGeneracionAuto($idFormulario,$idRegistro)
	{
		global $con;
		$carpetaAdministrativa=obtenerCarpetaAdministrativaProceso($idFormulario,$idRegistro);
		$consulta="SELECT * FROM 7035_informacionDocumentos WHERE idFormulario=".$idFormulario." AND idReferencia=".$idRegistro;
		$fRegistro=$con->obtenerPrimeraFilaAsoc($consulta);
		$consulta="SELECT * FROM 3000_formatosRegistrados WHERE idFormulario=-2 AND idRegistro=".($fRegistro["idRegistro"]==""?-1:$fRegistro["idRegistro"]);
		$fRegistro=$con->obtenerPrimeraFilaAsoc($consulta);
		if(!$fRegistro || $fRegistro["documentoBloqueado"]==0)
			return 1;
		else
		{
			if($fRegistro["documentoBloqueado"]==2)
			{
				$consulta="SELECT * FROM _04001_firmasColegiadasDocumentos WHERE iFormulario=".$idFormulario." AND iRegistro=".$idRegistro.
						" AND despachoFimante='".$_SESSION["codigoInstitucion"]."' AND firmado=0";	
				$fFirma=$con->obtenerPrimeraFilaAsoc($consulta);
				if($fFirma)
					return 1;
			}
		}
