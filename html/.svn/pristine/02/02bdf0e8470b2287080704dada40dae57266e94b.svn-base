...
<script type="text/javascript" src="../Scripts/cGeneracionDocumentosGrid.js.php"></script>
<script type="text/javascript" src="../visoresGaleriaDocumentos/Scripts/visorDocumentosMedidasCautelares.js.php"></script>
<style>
	.x-item-disabled  
	{
                   ...
                   <tr>
                        ...
                        <?php
							...
							if(strpos($idDocumentoNormalizado,"_")===false)
							{
								...
								$consulta="SELECT idFormulario,idRegistro,idRegistroFormato FROM 3000_formatosRegistrados WHERE idDocumento=".$idDocumentoNormalizado;
								
								$fRegistroDocumentoBase=$con->obtenerPrimeraFila($consulta);	
								
								$consulta="SELECT carpetaAdministrativa,idRegistro FROM 7047_registroDocumentosRelacion WHERE idRegistroFormato=".($fRegistroDocumentoBase[2]==""?-1:$fRegistroDocumentoBase[2]);

								$fDatosRegistroMedidasCautelares=$con->obtenerPrimeraFila($consulta);
								
								$carpetaAdministrativa=$fDatosRegistroMedidasCautelares[0];
							...
							$oActividad=json_decode(bD($oComp));
							$consulta="SELECT fechaVisualizacion FROM 9060_tableroControl_".$oActividad->iTablero." WHERE idRegistro=".$oActividad->iActividad;
							$fechaVisualizacion=$con->obtenerValor($consulta);
							if($fechaVisualizacion=="")
							{
								$consulta="update  9060_tableroControl_".$oActividad->iTablero." set fechaVisualizacion='".date("Y-m-d H:i:s")."' WHERE idRegistro=".$oActividad->iActividad;
								$con->ejecutarConsulta($consulta);
							}
                        ?>
                         <input type="hidden" id="iRegistroMedidaCautelar" value="<?php echo $fDatosRegistroMedidasCautelares[1]?>" />
                        ...
                        <input type="hidden" id="categoriaDocumento" value="<?php echo $categoriaDocumento?>">
