<?php
	...
	function ejecutarAccionComplementariaProvidencias($idFormulario,$idRegistro)
	{
		...
		switch($fRegistro["providenciaAplicar"])
		{
			case 20:
				...
				while($fila=$con->fetchAssoc($resFilas))
				{
					$arrDocumentos=NULL;
					$arrDatos=array();
	...
	{
		global $con;
		$consulta="UPDATE _1004_tablaDinamica SET codigoInstitucion='1002000100010010' WHERE id__1004_tablaDinamica=".$idRegistro;
		return $con->ejecutarConsulta($consulta);
	...
	function seleccionRutaAdmisionTutela($idFormulario,$idRegistro,$idActorProceso)
	{
		global $con;
		$etapaCambio=0;
		...
		cambiarEtapaFormulario($idFormulario,$idRegistro,$etapaCambio,"",-1,"NULL","NULL",$idActorProceso);
	}
	...
	function registrarIncidenciaExcepcional($idFormulario,$idRegistro)
	{
		global $con;
		...
		$consulta="SELECT * FROM _1309_tablaDinamica WHERE idReferencia=".$idRegistro." ORDER BY id__1309_tablaDinamica DESC";
		$fRegistro=$con->obtenerPrimeraFilaAsoc($consulta);
		
		$consulta="SELECT u.idUsuario  FROM 801_adscripcion a,807_usuariosVSRoles uR,800_usuarios u WHERE a.Institucion='".$codigoInstitucion."'
					AND uR.idUsuario=a.idUsuario AND uR.idRol IN(56,96) AND u.idUsuario=uR.idUsuario AND u.cuentaActiva=1";
		$res=$con->obtenerFilas($consulta);
		while($fila=$con->fetchAssoc($res))
		{
		
			$arrValores=array();
			$arrValores["codigoInstitucion"]=$codigoInstitucion;
			$arrValores["usuarioJuez"]=$fila["idUsuario"];
			$arrValores["fechaInicial"]=$fRegistro["fechaInicio"];
			$arrValores["fechaFinal"]=$fRegistro["fechaFin"];
			$arrValores["tipoIncidencia"]="5";
			$arrValores["comentarios"]="Mediante solicitud de compensación por complejidad excepcional. ".$fRegistro["comentariosAdicionales"];
			$arrValores["hInicio"]="00:00:00";
			$arrValores["hFin"]="23:59:59";
			$arrValores["tipoIntervalo"]=1;
			
			
			$arrDocumentosReferencia=array();
			if($fRegistro["documentoAdicional"]!="")
			{
				array_push($arrDocumentosReferencia,$fRegistro["documentoAdicional"]);
			}
				
			$idRegistroRadicacion=crearInstanciaRegistroFormulario(20,$idRegistro,1,$arrValores,$arrDocumentosReferencia,-1,0,"");
