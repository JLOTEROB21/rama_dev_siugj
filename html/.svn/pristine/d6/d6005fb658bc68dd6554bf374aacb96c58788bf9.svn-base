		...
		function obtenerDocentesEsperaContrato()
		{
			...
			{	
				...
				$consulta="SELECT g.idGrupos,g.idMateria,g.nombreGrupo,g.idInstanciaPlanEstudio FROM 4519_asignacionProfesorGrupo m,800_usuarios w ,4520_grupos g 
                    		WHERE  g.idGrupos=m.idGrupo AND g.Plantel='".$_SESSION["codigoInstitucion"]."' AND w.idUsuario=m.idUsuario AND m.fechaAsignacion<=m.fechaBaja
							and m.participacionPrincipal=1 and  m.esperaContrato=1 AND w.idUsuario=".$filaMat[0]."
							and g.idInstanciaPlanEstudio in (".$listPlanesAbiertos.") AND g.idCiclo='".$idCiclo."' AND g.idPeriodo='".$idPeriodo."'";
				...
				$consulta="SELECT g.idGrupos,g.idMateria,g.nombreGrupo,g.idInstanciaPlanEstudio FROM 4519_asignacionProfesorGrupo m,800_usuarios w ,4520_grupos g 
                    		WHERE g.idGrupos=m.idGrupo AND g.Plantel='".$_SESSION["codigoInstitucion"]."' AND w.idUsuario=m.idUsuario AND  m.fechaAsignacion<=m.fechaBaja and m.participacionPrincipal=1 and  m.esperaContrato=1 AND w.idUsuario=".$filaMat[0]."
							and g.idInstanciaPlanEstudio not in (".$listPlanesAbiertos.") AND g.idCiclo='".$idCiclo."' AND g.idPeriodo='".$idPeriodo."'";
				$resMaterias=$con->obtenerFilas($consulta);
				$nMateriasCerradas=0;
				while($filaGpo=mysql_fetch_row($resMaterias))
				{
