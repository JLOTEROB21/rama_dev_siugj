<?php session_start();
	...
	switch($funcion)
	{
		...
		break;
		case 6:
			acutalizarRegistroDespachoProgramnaTrabajo();
		break;
	...
	function obtenerDemandasProgramaTrabajoAccionPublica()
	{
		...
		$resDemandas=$con->obtenerFilas($consulta);
		while($fila=$con->fetchAssoc($resDemandas))
		{
	...
	function obtenerDemandasDisponiblesProgramaTrabajoAccionPublica()
	{
		...
		$arrRegistros="";
		$consulta="SELECT * FROM _1004_tablaDinamica WHERE idEstado=4 and id__1004_tablaDinamica not in(SELECT iRegistro FROM 04000_demandasProgramaTrabajo)";
		$resDemandas=$con->obtenerFilas($consulta);
		while($fDemanda=$con->fetchAssoc($resDemandas))
		{
			
			
			
			$consulta="SELECT fechaCambio FROM 941_bitacoraEtapasFormularios WHERE idFormulario=1004 AND idRegistro=".
					$fDemanda["id__1004_tablaDinamica"]." AND etapaActual=4";
			$fechaRegistro=$con->obtenerValor($consulta);
			$idActividad=$fDemanda["idActividad"];
	...
	function ejecutarRepartoDespachoProgramaTrabajo()
	{
		...
		$consulta="SELECT id__992_tablaDinamica,nombreSala FROM _992_tablaDinamica WHERE tipoSala=3 AND tipoUnidad=29";
	...
	function acutalizarRegistroDespachoProgramnaTrabajo()
	{
		global $con;
		$iP=$_POST["iP"];
		$valor=$_POST["valor"];
		...
		$consulta="UPDATE 04000_demandasProgramaTrabajo SET despachoAsignado=".$valor." WHERE idFormulario=1108 AND idReferencia=".$iP." AND iFormulario=1004 AND iRegistro=".$iR;
		eC($consulta);
