...
Ext.ux.grid.DataDrop = (function(){
    ...
    function onViewRender(){
        ...
        if (v.mainBody) {
			...
			this.textEl.dom.setAttribute('control',this.id);
			asignarEvento(this.textEl.dom,'change',function(el)
													{
														var lineEndRE = /\r\n|\r|\n/,
												        sepRe = /\s*\t\s*/;
														var nv = el.value;
														var almacen=gEx(el.getAttribute('control'));
        //el.blur();
														if (nv !== '') 
														{

															var store = almacen.getStore(), Record = store.recordType;
															el.value = '';
															var rows = nv.split(lineEndRE);
															var cols = almacen.getColumnModel().getColumnsBy(function(c){return !c.hidden;});
																...
																for (var i = 0; i < rows.length; i++) 
																{
																	...
																	if (vals.join('').replace(' ', '') !== '') 
																	{
																		...
																		for (var k = 0; k < vals.length; k++) 
																		{
																			...
																			{
																				if(cols[ct].dataIndex!='')
																				{
																					fldName = cols[ct].dataIndex;
																		...
																		almacen.view.focusRow(idx);
																		Ext.get(almacen.view.getRow(idx)).highlight();
																	}
																}
