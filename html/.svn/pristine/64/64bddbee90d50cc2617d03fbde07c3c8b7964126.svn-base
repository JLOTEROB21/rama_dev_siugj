...
Ext.calendar.EventEditWindow = function(config) {
	...
	var selLugar={
					...
					items:	[
								{
									id:'rdoPlantel',
									fieldLabel:'El lugar de eventos ser&aacute; un instituci&oacute;n',
									value:1	,
									checked:true,
									name:'lugar',
									listeners:	{
													check:function(chk,valor)
															{
																if(valor)
																{
																	gEx('txtLugarOtro').hide();
																	gEx('comboLugar').show();
																	gEx('comboLugar').focus();
																}
															}
												}
								},
								{
									id:'rdoOtro',
									fieldLabel:'El lugar del evento <b>NO</b> ser&aacute; una intituci&oacute;n',
									value:2,
									name:'lugar',
									listeners:	{
													check:function(chk,valor)
															{
																...
																{
																	gEx('comboLugar').hide();
																	gEx('txtLugarOtro').show();
																	gEx('txtLugarOtro').focus();
																}
															}
												}
								}
	...
	formPanelCfg.items.push(selLugar);
	...
	formPanelCfg.items.push(textArea);
	var comboLugar =new Ext.form.ComboBox	(
													{
														id:'comboLugar',
														mode:'local',
	...
	formPanelCfg.items.push(comboLugar);
	...
	formPanelCfg.items.push(lblComentarios);
	var gridUsuarios=crearGridUsuariosInvitados('gridUsuarios');
	formPanelCfg.items.push(gridUsuarios);
    ...
    Ext.calendar.EventEditWindow.superclass.constructor.call(this, Ext.apply({
        ...
        deletingMessage: 'Eliminado evento...',
        fbar: 	[
					/*{
						xtype: 'tbtext',
						text: '<a href="#" id="tblink">Modificar Detalles...</a>'
					},*/
				'->', 
					{
						id:'btnGuardar2',
...
Ext.extend(Ext.calendar.EventEditWindow, Ext.Window, {
    ...
    show: function(o, animateTarget) 
	{
        ...
        if (o.data) 
		{
            ...
            if (this.isAdd) 
			{
				...
				//comboLugar.getStore().remove(comboLugar.getStore().getAt(0));
                rec.markDirty();
                this.setTitle(this.titleTextAdd);
				gEx('lblComentarios').hide();
            }
            else {
			...
			if(o.data.RO=='1')
			{
				gEx('btnGuardar2').hide();
				gEx('delete-btn').hide();
				this.setTitle('Evento [<font color="red">S&oacute;lo lectura</font>]');
			}
			else
			{
				gEx('btnGuardar2').show();
				gEx('delete-btn').show();
			}
            f.loadRecord(rec);
        }
    ...
    onSave: function() 
			{
				var tipoLugar=gEx('tipoLugar').getValues()[0];
				
				if(gEx('title').getValue()=='')
				{
					function resp()
					{
						gEx('title').focus();	
				...
				if(tipoLugar=='1')
				{
					if(gEx('comboLugar').getValue()=='')
					{
						function resp22()
						{
							gEx('comboLugar').focus();	
						}
						msgBox('Debe indicar el lugar del evento',resp22)
						return;	
					}
				}
				else
				{
					if(gEx('txtLugarOtro').getValue()=='')
					{
						function resp21()
						{
							gEx('txtLugarOtro').focus();	
						}
						msgBox('Debe indicar el lugar del evento',resp21)
						return;	
					}
				}
