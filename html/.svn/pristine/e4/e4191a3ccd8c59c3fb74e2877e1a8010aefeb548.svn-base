<?php include("utiles.php");
	...
	class cConexion
	{
		...
		function __construct($host,$login,$passwd,$bd,$nuevaConexion=false)  //constructor de la clase cConexion
		{
		...
		{
			$res = mysql_query ( $consulta, $this->conexion ) or die ( "Imposibe realizar esta acción (Obtener): " . mysql_error () );
		...
		{
			$result = mysql_query ( $consulta,$this->conexion ) or die ( "Imposibe realizar esta acción (Obtener): " . mysql_error () );
		...
		{
			$result = mysql_query ( $consulta, $this->conexion ) or die ( "Imposible realizar esta acción (Llenar): " . mysql_error () );
		...
		//	$re$res=mysql_query($consulta,$this->conexion) or die(mysql_error());
			$res=mysql_query($consulta,$this->conexion);
			//$this->filasAfectadas=$res?mysql_num_rows($res):0;
...
*/			if($modoDebug)
		...
		{
			$result = mysql_query ( $consulta, $this->conexion ) or die ( "Imposibe realizar esta acción (agregar): " . mysql_error () );
			$r = mysql_query ( "SELECT LAST_INSERT_ID()", $this->conexion) or die ( "Imposibe realizar esta acción: " . mysql_error () );
		...
		{
			return $this->obtenerSiguienteFila($res);
		}
		...
		{
			return $this->obtenerSiguienteFilaAsoc($res);
		}
		
		function dataSeek(&$res,$posicion)
		{
			if($posicion==0)
			{
				if(mysql_num_rows($res)>0)
				{
					mysql_data_seek($res,$posicion);
				}	
			}
			else
				if(mysql_num_rows($res)<$posicion)
				{
					mysql_data_seek($res,mysqli_num_rows($res)-1);
				}
		}
		
		function fieldName(&$res,$columna)
		{
			$objColumn=mysql_fetch_field($res,$columna);
		...
		function esCampoLlave($campo,$nTabla)
		{
			$con=$this;

			$consulta="SELECT COLUMN_KEY FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='".$this->bdActual."' AND TABLE_NAME='".$nTabla."' and COLUMN_NAME='".$campo."'";
			$columnKey=$con->obtenerValor($consulta);
			return $columnKey=="PRI"?1:0;
		}
