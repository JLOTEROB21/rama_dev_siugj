                   ...
                   <tr>
                        ...
                        <?php
							...
							function comparar($v1, $v2)
							{
								...
								$arrDatos1=explode("_",$v1);
								$arrDatos2=explode("_",$v2);
								return $arrDatos1[0]<$arrDatos2[0];
...
//,100118
							...
							$consulta="SELECT id__17_tablaDinamica FROM _17_tablaDinamica WHERE cmbCategoria=1 AND id__17_tablaDinamica not in(36,51,53) 
										ORDER BY prioridad";
							$res=$con->obtenerFilas($consulta);
							...
							foreach($arrUGJ as $iUGJ=>$resto)
							{
								...
								$consulta="SELECT clave,(SELECT nombre FROM 800_usuarios WHERE idUsuario=t.usuarioJuez) AS nombreJuez,t.usuarioJuez 
										...
										AND j.idOpcion=1 and t.usuarioJuez<>-1 and t.usuarioJuez is not null and clave<>'000'  order by clave";
								$res=$con->obtenerFilas($consulta);
								while($fila=mysql_fetch_row($res))
								{
							...
							$lblTipoAudiencia=$con->obtenerValor($consulta);
							...
							foreach($arrJueces as $idJuez=>$nombreJuez)
							{
								$consulta="SELECT e.idRegistroEvento,(SELECT COUNT(*) FROM 7007_contenidosCarpetaAdministrativa c,
													...
													and r.idActividad=ca.idActividad and r.idFiguraJuridica=4) as nImputados,
													AVG(TIMESTAMPDIFF(MINUTE, horaInicioReal,horaTerminoReal ))	
													  FROM 7000_eventosAudiencia e,7001_eventoAudienciaJuez j 
													WHERE e.fechaEvento>='".$fechaInicio."' AND fechaEvento<='".$fechaFin."'  AND 
													tipoAudiencia=".$tipoAudiencia.	" AND  j.idRegistroEvento=e.idRegistroEvento 
													and j.idJuez=".$idJuez." and horaInicioReal IS NOT NULL AND 
													(TIMESTAMPDIFF(MINUTE, horaInicioReal,horaTerminoReal )>15) AND  (TIMESTAMPDIFF(MINUTE, horaInicioReal,horaTerminoReal )<400)
													 order by e.duracionAudienciaMAJO desc";
								...
								if($fAudiencia)
								{
									$duracion=$fAudiencia[2];
									
								}
								
								
								$arrAnalisis[$duracion."__".$idJuez]=$nombreJuez;
								
								
								
							}
							uksort($arrAnalisis,"comparar");
							
							
							
							
							$lblEtiquetas="";
							foreach($arrAnalisis as $idJuez=>$resto)
							{
								$arrDatos=explode("_",$idJuez);
								if($lblEtiquetas=="")
									$lblEtiquetas="'".$resto."'";
								else
									$lblEtiquetas.=",'".$resto."'";
							...
							foreach($arrAnalisis as $idJuez=>$resto)
							{
								$arrDatos=explode("_",$idJuez);
								if($dataSet1=="")
									$dataSet1=number_format($arrDatos[0]==""?0:$arrDatos[0],2);
								else
									$dataSet1.=",".number_format($arrDatos[0]==""?0:$arrDatos[0],2);
							}
                        ...
                        ?>
                        	...
                        	<div id="container"  >
                                <canvas id="canvas" style="width: 1000px; height:1400px"></canvas>
                            </div>
                            <script>
							...
							var horizontalBarChartData = {
															...
															datasets: [
																		{
																			label: 'Promedio de duración de audiencias en minutos',
								...
								window.myHorizontalBar = new Chart(ctx, {
																			...
																			options: {
																						...
																						title: {
																									display: true,
																									fontSize:14,
																									text: '<?php echo "Ranking de jueces por audiencia de mayor duración - ".$lblTipoAudiencia ?>'
																								},
																						scales: {
