<?php  
	...
	function crearControl($defControl,$decodeJSON=true,$idForm="-1")
	{
		...
		if($con->ejecutarConsulta($query))
		{
			...
			$numPreguntas=$pregunta==""?0:count($pregunta);
			$query="select idElementoFormularioSig from 903_variablesSistema for update";
	...
	function obtenerSeccionesElementosFormulario($modalidad,$idProceso,$idFormulario,$idRegistro,$idActor,$participante)
	{
		...
		$etapaRegistro=isset($filaReg[0])?$filaReg[0]:"";
		$respRegistro=isset($filaReg[1])?$filaReg[1]:"";
		if(!$filaReg)
		{
			$etapaRegistro=0;
						...
						if($filaAcciones[3]!="")
						{
							...
							if(isset($oAcciones->mostrarFormularioAsociado))
							{
								...
								if($mostrarFormularioAsociado==0)
								{
									...
									while($filaOpcion=$con->fetchRow($resOpciones))
									{
			...
			if($participante==0)
			{
				...
				if($idPerfil!=-1)
				{
					$consulta="select e.idFormulario,nombreFormulario,titulo,e.tipoElemento,nombreTabla,e.obligatorio,e.idElementoDTD,e.funcionExclusion,e.idFuncionVisualizacion,e.idFuncionEdicion,e.tituloElemento,f.frmRepetible,
								f.anchoGrid,f.altoGrid,e.complementario,cadConfiguracion,e.funcionEdicionForzada from 203_elementosDTD e,900_formularios f where e.idFormulario=f.idFormulario and 
		...
		$consulta="select e.idFormulario,nombreFormulario,titulo,e.tipoElemento,nombreTabla,e.obligatorio,e.idElementoDTD,e.funcionExclusion,
					e.idFuncionVisualizacion,e.idFuncionEdicion,e.tituloElemento,f.frmRepetible,f.anchoGrid,f.altoGrid,e.complementario,cadConfiguracion,
					e.funcionEdicionForzada from 203_elementosDTD e,900_formularios f where e.idFormulario=f.idFormulario and 
		...
		if($f)
		{
			...
			if(($f[9]!="0")&&($f[9]!="")&&($f[9]!="-1"))
			{
				...
				$cadObj='{"numEtapa":"'.$etapaRegistro.'","idFormularioEvaluacion":"'.$idFormulario.'","idFormulario":"'.$idFormulario.'","idReferencia":"'.$idRegistro.'","actor":"'.$idActor.
						'","iRegistro":"'.$idRegistro.'","iFormulario":"'.$idFormulario.'"}';
				$obj=json_decode($cadObj);
		...
		while($f=$con->fetchRow($res))
		{
			...
			{
				$cacheCalculo=null;
				$cadObj='{"numEtapa":"'.$etapaRegistro.'","idFormularioEvaluacion":"'.$idFrm.'","idFormulario":"'.$idFormulario.'","idReferencia":"'.$idRegistro.'","actor":"'.
							$idActor.'","iRegistro":"'.$idRegistro.'","iFormulario":"'.$idFormulario.'"}';
			...
			$permiteEdicion=true;
			$existeFuncionEdicion=false;
			if((!$aplicarExcepcion)&&($f[9]!="0")&&($f[9]!="")&&($f[9]!="-1")) //Función de edición
			{
				$existeFuncionEdicion=true;
				$cacheCalculo=null;
				$cadObj='{"numEtapa":"'.$etapaRegistro.'","idFormularioEvaluacion":"'.$idFrm.'","idFormulario":"'.$idFormulario.'","idReferencia":"'.$idRegistro.
						'","actor":"'.$idActor.'","iRegistro":"'.$idRegistro.'","iFormulario":"'.$idFormulario.'"}';
				$obj=json_decode($cadObj);
			...
			$elementoModificable=(($modificaElementos)||($aplicarExcepcion));//(!$existeFuncionEdicion || ($existeFuncionEdicion && $permiteEdicion)) 
			$oSeccion=array();
			...
			switch($f[3])
			{
				...
				case 1:  //modulop Predefinido
					...
					if($nTablaModulo!="")
					{
						...
						$nReg=0;
						if($con->existeCampo("idFormulario",$nTablaModulo))
						{
						...
						else
						{
							...
							if($con->existeCampo("idActividad",$nTablaModulo))
							{
								$tablaBase=obtenerNombreTabla($idFormulario);
								if($con->existeCampo("idActividad",$tablaBase))
								{
									$consulta="SELECT idActividad FROM ".$tablaBase." where id_".$tablaBase."=".$idRegistro;
									$idActividad=$con->obtenerValor($consulta);
									if(($idActividad=="")||($idActividad=="N/E"))
									{
										$idActividad=-1;
									}
									$consulta="select count(*) from ".$nTablaModulo." where idActividad=".$idActividad;
				...
				case 2:
					...
					if($complementario!="")
					{
						...
						{
							$consulta="SELECT actor FROM 950_actorVSProcesoInicio WHERE idProceso=".$idProcesoAux." AND actor IN(".$_SESSION["idRol"].")";
						...
						if($arrComp[1]!=0)
						{
							...
							else
							{
								...
								{
									$filaRegName=$con->fetchAssoc($resName);
		...
		if(isset($arrAcciones[1]))
		{
			...
			foreach($objSometeRevision as $o)
			{
				...
				$considerar=false;
				if(($o->funcionVisualizacion!="")&&($o->funcionVisualizacion!=-1))//&&(someteRevision) Modificado
				{
					$cacheCalculo=null;
		...
		if(isset($arrAcciones[30]))
		{
			$oConfiguracion=json_decode($arrAcciones[30][1]);

			if(isset($oConfiguracion->funcionVisualizacion) &&($oConfiguracion->funcionVisualizacion!="") &&($oConfiguracion->funcionVisualizacion!="-1"))
			{
				$cacheCalculo=null;
				$cadObj='{"numEtapa":"'.$etapaRegistro.'","idFormularioEvaluacion":"'.$idFrm.'","idFormulario":"'.$idFormulario.'","idReferencia":"'.$idRegistro.
						'","actor":"'.$idActor.'","iRegistro":"'.$idRegistro.'","iFormulario":"'.$idFormulario.'"}';
				...
				$resultado=resolverExpresionCalculoPHP($oConfiguracion->funcionVisualizacion,$obj,$cacheCalculo);
				
				
				if(($resultado=="0")||($resultado=="'0'")||($resultado===false))
				{
					unset($arrAcciones[30]);
				}
			}
	...
	function crearInstanciaRegistroFormulario($idFormulario,$idReferencia,$etapaActual,$arrValores,$arrDocumentosReferencia,$idPerfil=-1,$actor=0,$comentariosAdicionales="") //Modificado
	{
		global $con;
		...
		$consulta="INSERT INTO _".$idFormulario."_tablaDinamica(idReferencia,fechaCreacion,responsable,idEstado,codigoUnidad,codigoInstitucion,codigo".$cadenaCampo.")
	...
	function SYS_obtenerRemitenteEnvio($idFormulario,$idRegistro,$idActorProceso)
	{
		...
		{
			$consulta="SELECT * FROM 941_bitacoraEtapasFormularios WHERE idFormulario=".$idFormulario." AND idRegistro=".$idRegistro.
						" and etapaActual=".$fRegistro1["etapaAnterior"]." and etapaAnterior<>etapaActual and etapaAnterior<>".$fRegistro1["etapaActual"]."  ORDER BY idRegistroEstado DESC";
	...
	function SYS_obtenerUsuariosResponsablesRegistro($idFormulario,$idRegistro,$actorDestinatario)
	{
		...
		if($con->filasAfectadas==0)
		{
			$consulta="SELECT DISTINCT responsable,'0' as actorCambio FROM _".$idFormulario."_tablaDinamica WHERE id__".$idFormulario."_tablaDinamica=".$idRegistro;
		...
		while($fDatosDestinatario=$con->fetchRow($resDestinatarios))
		{
			$idUsuarioDestinatario=$fDatosDestinatario[0];
	...
	function obtenerInformacionDataSetControlFormulario($idElemento)
	{
		global $con;
		
		$consulta="SELECT campoConf1,campoConf2,campoConf3 FROM 904_configuracionElemFormulario WHERE idElemFormulario=".$idElemento;
		$fConfiguracion=$con->obtenerPrimeraFilaAsoc($consulta);
		
		$arrInformacion=array();
		$arrTablaCampo=explode(".",$fConfiguracion["campoConf3"]);
		$arrInformacion["campoID"]=$arrTablaCampo[1];
		$arrInformacion["tablaCampoID"]=$arrTablaCampo[0];
		$arrInformacion["campoID_EsLlaveTabla"]=$con->esCampoLlave($arrTablaCampo[1],$arrTablaCampo[0]);
		$arrTablaCampo=explode(".",$fConfiguracion["campoConf2"]);
		$arrInformacion["campoEtiqueta"]=$arrTablaCampo[1];
		$arrInformacion["tablaCampoEtiqueta"]=$arrTablaCampo[0];
		
		$idQuery=-1;
		if(strpos($fConfiguracion["campoConf1"],"[")!==false)
		{
			$idQuery=str_replace("[","",$fConfiguracion["campoConf1"]);
			$idQuery=str_replace("]","",$idQuery);
			
			
		}
		$arrInformacion["idQuery"]=$idQuery;
		$arrInformacion["tablasInvolucradas"]=array();
		if($idQuery==-1)
		{
			array_push($arrInformacion["tablasInvolucradas"],$filaE["campoConf1"]);
			
		}
		else
		{
			$consulta="SELECT nTabla FROM 9014_almacenesDatos WHERE idDataSet=".$idQuery;
			$tablas=$con->obtenerValor($consulta);
			$arrTablas=explode(",",$tablas);
			foreach($arrTablas as $t)
			{
				array_push($arrInformacion["tablasInvolucradas"],$t);
			}
		}
	...
	function obtenerRegistroPadre($idFormulario,$idRegistro)
	{
		global $con;
		
		$consulta="SELECT idProcesoPadre,idReferencia FROM _696_tablaDinamica WHERE id__696_tablaDinamica=".$idRegistro;
		$fRegistro=$con->obtenerPrimeraFilaAsoc($consulta);
		
		$arrDatos=array();
		$arrDatos["idFormulario"]=obtenerFormularioBase($fRegistro["idProcesoPadre"]);
		$arrDatos["idRegistro"]=$fRegistro["idReferencia"];
	...
	function SYS_obtenerUsuarioOriginalRegistro($idFormulario,$idRegistro,$actorDestinatario)
	{
		global $con;
		$arrDestinatarios=array();
		$rolActor=obtenerTituloRol($actorDestinatario);
	...
	function copiarDocumentosProcesoPadre($idFormulario,$idRegistro)
	{
		global $con;
		$consulta="SELECT idProcesoPadre,idReferencia FROM _708_tablaDinamica WHERE id__708_tablaDinamica=".$idRegistro;
		$fRegistro=$con->obtenerPrimeraFilaAsoc($consulta);
		...
		$consulta="INSERT INTO 9074_documentosRegistrosProceso(idFormulario,idRegistro,idDocumento,tipoDocumento)
				SELECT '".$idFormulario."' as idFormulario,'".$idRegistro."' as idRegistro,idDocumento,tipoDocumento 
				FROM 9074_documentosRegistrosProceso WHERE idFormulario=".$iFormulario." AND idRegistro=".$fRegistro["idReferencia"].
				" AND tipoDocumento=2 and idDocumento not in(".$listaDocumentos.")";
