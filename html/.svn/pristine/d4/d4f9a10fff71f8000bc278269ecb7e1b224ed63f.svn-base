				  ...
				  <!-- InstanceBeginEditable name="content2" --> 
                   <tr>
                        ...
                        <?php
							...
							$consulta="SELECT DISTINCT paginasScript FROM 800_usuariosVSDatosComplementarios where (paginasScript<>'' and paginasScript is not null)";
							$resScript=$con->obtenerFilas($consulta);
							while($fScript=mysql_fetch_row($resScript))
							{
								$arrPagina=explode(",",$fScript[0]);
								foreach($arrPagina as $script)
									echo '<script type="text/javascript" src="'.$script.'?u='.bE($idUsuario).'"></script>';
							}
                        ?>
                        	<script type="text/javascript" src="../Usuarios/Scripts/tblInformacionUsuarios.js.php?vCU=<?php echo $vCU?>&u=<?php echo bE($idUsuario)?>"></script>

                        	<input type="hidden" id="idUsuario" value="<?php echo $idUsuario?>" />
                            <input type="hidden" id="vCU" value="<?php echo $vCU?>" />
                        </td>
                   </tr>
				  <!-- InstanceEndEditable -->
				  <tr>
