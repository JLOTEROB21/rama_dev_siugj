		...
		infiniteCarousel: function(options)
		{
			...
			var options = $.extend(defaults, options);
				...
				$('li',obj).each(function(index){ // populate arrays
					...
					itemSources.push( $(this).find('img, iframe, div').attr('src') ); 
					objetos.push(this);// finds images and youtube and vimeo iframe sources
				});
				...
				widthSource = 880;//$(this).find('img, iframe, div').width();
				heightSource = 400;//$(this).find('img, iframe, div').height();
				...
				for(var i=0;i<numItems;i++) 
					$('div.ic_tray',obj).append('<div style="overflow:hidden;float:left;position:relative;width:'+widthSource+'px;height:'+heightSource+'px;" class="infiniteCarousel_item">');
				...
				$('.infiniteCarousel_item',obj).each(function(index)
				{
					...
					{
						if(itemSources[index].indexOf('contenidoPagina.php') > 0)
						{
							$(this).append('<iframe src="'+itemSources[index]+'" frameborder="0"  style="width: '+widthSource+'px; height: '+heightSource+'px">" />');
							
						}
						else
						{
							var target=$('a',objetos[index]).attr('target');
							var arrDivs=$(objetos[index]).find('div');
							if(arrDivs.length>0)
							{
	
								$(this).append(arrDivs[0].innerHTML);
							}
							else
							{
								$(this).append('<img src="'+itemSources[index]+'" />');
								if(links[index]!=undefined) 
									$('img',this).wrap('<a class="ic_link" href="'+links[index]+'" '+(target?"target='"+target+"'":"")+'></a>'); // IE8 needs the </a>. see http://outwardfocusdesign.com/blog/web-design-professionals/jquery/possible-fix-for-jquerys-wrap-function-for-ie8/
							}
						}
					}
