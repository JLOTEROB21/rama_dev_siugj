<?php
	...
	$consulta="SELECT dteFechaAudiencia,horaInicioPropuesta,horaFinalPropuesta FROM _71_tablaDinamica WHERE idReferencia=".$idRef." 
				ORDER BY id__71_tablaDinamica DESC";
	$fechaPropuesta=$con->obtenerPrimeraFila($consulta);
	
?>

var fechaPropuesta='<?php echo $fechaPropuesta[0]?>';
var horaInicioPropuesta='<?php echo $fechaPropuesta[1]?>';
var horaFinPropuesta='<?php echo $fechaPropuesta[2]?>';
...
function inyeccionCodigo()
{
	...
	gE('sp_1406').innerHTML='';
    gE('sp_1432').innerHTML='';
	new Ext.Button (
                      {
                          ...
                          handler:function()
                                  {
                                      mostrarVentanaPropuestaAudiencia();
	              	 	              ...
	              	 	              ;
	fechaPropuesta=Date.parseDate(fechaPropuesta,'Y-m-d')  ; 
  
	    
  	
	gE('sp_1436').innerHTML= fechaPropuesta.format('d')+' de '+arrMeses[(fechaPropuesta.format('m')*1)-1][1]+' de '+fechaPropuesta.format('Y');
    if(Date.parseDate(horaInicioPropuesta,'H:i'))
	    gE('sp_1437').innerHTML='De las '+Date.parseDate(horaInicioPropuesta,'H:i').format('H:i')+' a las ' +Date.parseDate(horaFinPropuesta,'H:i').format('H:i')+'  hrs.';           
 	else
    	gE('sp_1437').innerHTML='De las '+Date.parseDate(horaInicioPropuesta,'H:i:s').format('H:i')+' a las ' +Date.parseDate(horaFinPropuesta,'H:i:s').format('H:i')+'  hrs.';            
