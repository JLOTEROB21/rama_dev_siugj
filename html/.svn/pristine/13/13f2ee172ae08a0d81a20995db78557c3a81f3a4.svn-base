<?php  session_start();
	...
	$consulta="SELECT tituloElemento FROM 908_registrosTablasRetencionDocumental WHERE idRegistro=".($fCarpeta["serie"]==""?"0":$fCarpeta["serie"]);
	$serie=$con->obtenerValor($consulta);
	if($serie=="")
		$serie="----";
	
	
	$consulta="SELECT tituloElemento FROM 908_registrosTablasRetencionDocumental WHERE idRegistro=".($fCarpeta["subserie"]==""?"0":$fCarpeta["subserie"]);
