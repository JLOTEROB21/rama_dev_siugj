<?php
	...
	function enviarMensajeEnvio($idMensaje,$arrParam,$funcionEnvio="",$codificarUtf8=false)
	{
		...
		global $urlSitio;
		$arrOtrosParametros["urlSitio"]=$urlSitio;
		$lParametros="";
		...
		foreach($arrOtrosParametros as $clave=>$valor)
		{
		...
		if($funcionEnvio=="")
		{
			return sendMensajeEnvioWebMailServer($arrDestinatario,$asunto,$cuerpo,$remitente,"",$arrDocumentos,$arrCCO,$arrCC);
		}
		else
		{
			$resultado="";
	...
	function sendMensajeEnvio($arrDestinatario,$asunto,$mensaje,$emisor="",$nombreEmisor="",$arrArchivos=null,$arrCopiaOculta=null,$arrCopia=null)
	{
		try
		{
			...
			$mail->IsSMTP();        
			//$mail->SMTPDebug  = ;                  // set mailer to use SMTP
	...
	function sendMensajeEnvioGmailServer($arrDestinatario,$asunto,$mensaje,$emisor="",$nombreEmisor="",$arrArchivos=null,$arrCopiaOculta=null,$arrCopia=null)
	{
		global $habilitarEnvioCorreo;
		global $mailAdministrador;
		global $nombreEmisorAdministrador;
		...
		$mail->Username = "marco.magana@linktic.co";  // SMTP username
		$mail->Password = "Grup0latis_17";
		...
		$mail->SetFrom ("marco.magana@linktic.co","marco.magana@linktic.co");
