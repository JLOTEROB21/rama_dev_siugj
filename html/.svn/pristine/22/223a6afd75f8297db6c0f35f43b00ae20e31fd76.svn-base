...
require_once $_SERVER["DOCUMENT_ROOT"].'/thotReporter/latis/latis.php';
...
// Private methods
	...
	private function getDataAdapter($args) {
		switch ($args->database) {
			...
			case StiDatabaseType::Latis: $dataAdapter = new StiLatisAdapter(); break;
		}
	...
	private function innerProcess() {
		...
		$idReporte = $result->object->idReporte;
		$dataSource = $result->object->dataSource;
		if ($result->success) {
				...
				case StiEventType::BeginProcessData:
					...
					switch ($request->command) {
						...
						case StiCommand::ExecuteQuery:
							...
							break;
						case StiCommand::RetrieveData:

							$realtable = $dataAdapter->getTableName($queryString);
							
							$result = $dataAdapter->execute($realtable,false,$filters,$idReporte,$dataSource);
							break;
					}
...
class StiHelper {
	public static function createOptions() {
		...
		$options->handler = "../latis/handler_viewer.php";
		$options->timeout = 30;
...
<script type="text/javascript">
	StiHelper.prototype.process = function (args, callback) {
		console.log("Entering protoype method process  in server side:");					
		console.log(args);
		if (args) {
			if (callback)
			...
			if (args.event == 'BeginProcessData') {
				...
				/*if (args.database == 'Latis'){
					console.log("The database is Latis");
					console.log(args);
			...
			//alert(Stimulsoft.System.Convert.fromBase64String(sendText));
	...
	StiHelper.prototype.send = function (json, callback) {
		try {
			console.log("entering send proto");
			console.log(json);
			...
			request.onload = function () {
				if (request.status == 200) {
					...
					try {
						console.log("The response of calling myself is:");
						...
						console.log("data received from myself");
						console.log(args);
						callback(args);
					}
					catch (e) {
