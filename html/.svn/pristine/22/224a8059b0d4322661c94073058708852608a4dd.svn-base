...
Ext.extend(Ext.calendar.DayBodyTemplate, Ext.XTemplate, 
{
    ...
    applyTemplate : function(o){
		...
		var horaFinal=24;
		if(gE('horaFinal')!=null)
			horaFinal=gE('horaFinal').value;
		var incremento=60;
		if(gE('incremento')!=null)
			incremento=gE('incremento').value;
		dt = dt.add(Date.HOUR, horaInicial);
		var nHoras=horaFinal-horaInicial;
		for(i=0; i<nHoras; i++)
		{
			times.push(dt.format('ga'));
			dt = dt.add(Date.MINUTE, incremento);
		}
