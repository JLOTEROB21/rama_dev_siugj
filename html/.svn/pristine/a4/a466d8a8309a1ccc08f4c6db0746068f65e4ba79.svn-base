            ...
            if (f.active) {
                ...
                for (i = 0, len = d.length; i < len; i++) 
				{
                    filters.push({
                        field: f.dataIndex,
                        data: d[i],
						tipoCampo:(typeof(f.tipoCampo)=='undefined')?'0':f.tipoCampo,
						idCampo:(typeof(f.idCampo)=='undefined')?'0':f.idCampo
                    });
                }
            }
        });
    ...
    buildQuery : 	function (filters) 
					{
						...
						if (!this.encode)
						{
							...
							for (i = 0; i < len; i++) 
							{
								...
								if(f.tipoCampo)
									p[[dataPrefix, '[tipoCampo]'].join('')] = f.tipoCampo;
								if(f.idCampo)
									p[[dataPrefix, '[idCampo]'].join('')] = f.idCampo;		
