<?php
	...
	if($idRegistro!=-1)
	{
		$consulta="SELECT asesor FROM _426_tablaDinamica WHERE id__426_tablaDinamica=".$idRegistro;	
...
function inyeccionCodigo()
{
    ...
    {
       asignarEvento(gE('_asesorvch'),'change',buscarDomilicio)
	}
    
    gE('sp_6772').innerHTML='';
...
function participanteAgregado(iParticipante,nombre)
{
    ...
    opt.text=nombre;
	gE('_asesorvch').options[gE('_asesorvch').options.length]=opt;
    gE('_asesorvch').selectedIndex=gE('_asesorvch').options.length-1;
    buscarDomilicio();
}              
