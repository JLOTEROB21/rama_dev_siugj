...
class SolmetraUploader {
  ...
  function SolmetraUploader ($baseurl = false, $uploadurl = false, $config = false) 
  {
	  global $baseDir;
    // load settings
    $configFile = $config === false ? $baseDir.'/media/flashUploader/config.php' : $config;
	...
	//echo  $this->__normalizeDir($this->getSetting('uploaddir'))."<br>";
    //echo $this->__normalizeDir($this->getSetting('tmpdir'));
    // check PHP max size
						...
						)
  {
	...
	if($idDivDibujado==null)
	    $html = '<span id="'.$instance.'">'.$this->br;
    ...
    else 
	{
      ...
      $html .= '<script type="text/javascript">'.$this->br.
               ...
               '<param name="wmode" value="opaque" />'.$this->br.'<param name="allowScriptAccess" value="always" />'.$this->br.
               '<param name="allowFullScreen" value="false" />'.$this->br.
               '<param name="movie" value="'.$flashurl.'?='.$param_str.'" /><param name="quality" value="high" /><param name="bgcolor" value="#ffffff" />	<embed src="'.$flashurl.'?='.$param_str.'" quality="high" bgcolor="#ffffff" width="400" wmode="opaque" height="100" name="solmetraUploader_'.$instance.'" align="middle" allowScriptAccess="always" allowFullScreen="false" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />'.$this->br.
               '</object>'.$this->br.
               '</noscript>'.$this->br;
    }
    ...
    // finish object
    $html .= '  so.write("'.$instance.'");'.$this->br;
  ...
  function handleFlashUpload () 
  {
    ...
    if (!isset($_FILES['SolmetraUploader']) || !is_array($_FILES['SolmetraUploader'])) 
	{
      ...
      return false;
		...
		if ($_FILES['SolmetraUploader']['error'] > 0) 
		{
		 	$errors = array(
  ...
  {
	  global $baseDir;
	$dir = str_replace('%APPDIR%',$baseDir, $dir);
