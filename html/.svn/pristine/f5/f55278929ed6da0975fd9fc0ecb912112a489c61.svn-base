...
$libro=new cExcel("../modulosEspeciales_SGJP/formatos/plantillaAudiencias.xlsx",true,"Excel2007");	
...
$consulta="SELECT DISTINCT e.tipoAudiencia,t.tipoAudiencia FROM 7000_eventosAudiencia e,_4_tablaDinamica t 
		...
		and e.tipoAudiencia=t.id__4_tablaDinamica order by t.tipoAudiencia";
...
while($fDatos=mysql_fetch_row($res))
{
	...
	$consulta="SELECT e.idFormulario,e.idRegistroSolicitud from 7000_eventosAudiencia e WHERE idCentroGestion=15 AND fechaEvento>='".$periodoInicial."' 
			AND fechaEvento<='".$periodoFinal."' AND tipoAudiencia=".$fDatos[0];
	$nReg=0;
...
while($fDatos=mysql_fetch_row($res))
{
	...
	$consulta="SELECT e.idFormulario,e.idRegistroSolicitud from 7000_eventosAudiencia e WHERE idCentroGestion=15 AND fechaEvento>='".$periodoInicial."' 
			AND fechaEvento<='".$periodoFinal."' AND e.idFormulario=185 and  tipoAudiencia=".$fDatos[0];
	$nReg=0;
	$resEventos=$con->obtenerFilas($consulta);
	while($fEventos=mysql_fetch_row($resEventos))
	{
...
$consulta="SELECT COUNT(*) FROM 7006_carpetasAdministrativas WHERE unidadGestion='001' AND fechaCreacion>='".$periodoInicial.
		"' AND fechaCreacion<='".$periodoFinal."'";
...
$consulta="SELECT COUNT(*) FROM 7006_carpetasAdministrativas WHERE unidadGestion='005' AND fechaCreacion>='".$periodoInicial.
		"' AND fechaCreacion<='".$periodoFinal."' and carpetaAdministrativa not like 'TE/%'";
...
$consulta="SELECT COUNT(*) FROM 7006_carpetasAdministrativas WHERE unidadGestion='005' AND fechaCreacion>='".$periodoInicial.
		"' AND fechaCreacion<='".$periodoFinal."' and carpetaAdministrativa like 'TE/%'";
...
$consulta="SELECT COUNT(*) FROM  _96_tablaDinamica p, 7006_carpetasAdministrativas c WHERE p.fechaCreacion>='".$periodoInicial.
		"' AND p.fechaCreacion<='".$periodoFinal."' and p.carpetaAdministrativa=c.carpetaAdministrativa AND
...
for($x=3;$x<=13;$x++)
	$libro->setValor("F".$x,"=sum(D".$x.":E".$x.")");

/*$formula="0";
if(($filaContinuacionIniciales!=0)||($filaContinuacionPromociones!=0))
{
	if(($filaContinuacionIniciales!=0)&&($filaContinuacionPromociones!=0))
	{
...
$libro->setValor("G3",$formula);*/


/*$libro->setValor("G4",$nPromociones);
$libro->setValor("G5",$nPromociones);
$libro->setValor("G6",$nPromociones);
$libro->setValor("G7",$nPromociones);
$libro->setValor("G8",$nPromociones);
$libro->setValor("G9",$nPromociones);
$libro->setValor("G10",$nPromociones);
$libro->setValor("G11",$nPromociones);
$libro->setValor("G12",$nPromociones);
$libro->setValor("G13","=sum(G3:G12)");*/
