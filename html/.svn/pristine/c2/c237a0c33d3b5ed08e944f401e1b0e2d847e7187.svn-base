...
<script type="text/javascript" src="../modulosEspeciales_SGJP/Scripts/tblAgendaEventosJuzgados.js.php"></script>
				  ...
				  <!-- InstanceBeginEditable name="content2" --> 
                   <tr>
                        ...
                        <?php
							...
							if(isset($objParametros->idEvento))
								$idEvento=$objParametros->idEvento;
							
							$consulta="SELECT carpetaAdministrativa,unidadGestion FROM 7006_carpetasAdministrativas WHERE idCarpeta=".$idCarpeta;
							$fCarpeta=$con->obtenerPrimeraFila($consulta);
							
							$consulta="SELECT id__17_tablaDinamica FROM _17_tablaDinamica WHERE claveUnidad='".$fCarpeta[1]."'";	
							$idUnidad=$con->obtenerValor($consulta);
							
							$arrJuecesJuzgado="";
							$consulta="SELECT usuarioJuez FROM _26_tablaDinamica WHERE idReferencia=".$idUnidad;
							$res=$con->obtenerFilas($consulta);
							while($fila=mysql_fetch_row($res))
							{
								$o='{"idUsuario":"'.$fila[0].'","participacion":"Juez","ministerioLey":"0"}';
								if($arrJuecesJuzgado=="")
									$arrJuecesJuzgado=$o;
								else
									$arrJuecesJuzgado.=",".$o;
							...
							$consulta="SELECT tipoDelito FROM _17_gridDelitosAtiende WHERE idReferencia=".$idUnidad;
							$listaDelitos=$con->obtenerListaValores($consulta,"'");
							if($listaDelitos=="")
								$listaDelitos=-1;
							
							
							$consulta="SELECT id__15_tablaDinamica,nombreSala FROM _55_tablaDinamica se,_15_tablaDinamica s WHERE se.idReferencia=".$idUnidad." 
							...
							$consulta="SELECT id__4_tablaDinamica,tipoAudiencia,promedioDuracion FROM _4_tablaDinamica a,_4_tiposUGJ u 
										WHERE u.idPadre=a.id__4_tablaDinamica and u.idOpcion in(".$listaDelitos.") order by tipoAudiencia";
							...
							if($idEvento!=-1)
							{
								...
								$consulta=" select * from (SELECT usuarioJuez AS idUsuario,(SELECT Nombre FROM 800_usuarios WHERE idUsuario=usuarioJuez) AS magistrado 
											 ...
											 7001_eventoAudienciaJuez e WHERE idRegistroEvento=".$idEvento.")) as tmp order by magistrado";
								$resJuez=$con->obtenerFilas($consulta);
								while($fJuez=mysql_fetch_row($resJuez))
								{
									$consulta="SELECT titulo,ministerioLey FROM 7001_eventoAudienciaJuez WHERE idRegistroEvento=".$idEvento." and idJuez=".$fJuez[0];
								...
								$oEventoAudiencia='{"carpetaAdministrativa":"'.$carpetaAdministrativa.'","fechaEvento":"'.$fEvento[0].'","horaInicioEvento":"'.date("H:i",strtotime($fEvento[1])).
                        ...
                        <input type="hidden" id="idSala" value="<?php echo $idSala?>" />
                        <input type="hidden" id="cAdministrativa" value="<?php echo $fCarpeta[0]?>" />
                        <input type="hidden" id="uGestion" value="<?php echo $idUnidad?>" />
                        <input type="hidden" id="arrSalas" value="<?php echo bE($arrSalas) ?>" />
                        <input type="hidden" id="oEventoAudiencia" value="<?php echo bE($oEventoAudiencia)?>" />
                        <input type="hidden" id="arrTipoAudiencia" value="<?php echo bE($arrTipoAudiencia)?>" />
                        <input type="hidden" id="arrJuecesJuzgado" value="<?php echo bE($arrJuecesJuzgado)?>" />
                        </td>
                   </tr>
				  <!-- InstanceEndEditable -->
				  <tr>
