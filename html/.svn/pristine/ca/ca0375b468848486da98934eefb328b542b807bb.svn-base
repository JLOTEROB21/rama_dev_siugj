<?php
	...
	function usersAudiences($stringTokenAccess,$token,$start,$end,$statusAudiences)
	{
		...
		try
		{
			...
			$consulta="SELECT DISTINCT e.idRegistroEvento AS idEvento,fechaEvento,horaInicioEvento AS horaInicial,horaFinEvento AS horaFinal,
						...
						(SELECT COUNT(*) FROM 7000_participantesEventoAudiencia WHERE idRegistroEvento=e.idRegistroEvento) AS totalParticipantes
	...
	function usersGetCourts($stringTokenAccess,$token,$state,$typeCout)
	{
		global $urlSitio;
		global $versionLatis;
		global $con;
		...
		try
		{
			...
			{
				case 0:
					$consulta="select * from(SELECT d.nombreUnidad AS nombreDespacho,d.claveUnidad AS IDDespacho,d.claveRegistro AS claveDespacho,dom.calle AS domicilio,
								(SELECT estado FROM 820_estados WHERE cveEstado=dom.departamentos) AS departamento, 
								(SELECT municipio FROM 821_municipios WHERE cveMunicipio=dom.municipio) AS municipio,dom.latitud,dom.longitud,
								(SELECT GROUP_CONCAT(CONCAT(numero,' Ext. ',extension)) FROM _638_gridTelefono t WHERE t.idReferencia=dom.id__638_tablaDinamica) AS telefonos,
								(SELECT GROUP_CONCAT(email) FROM _638_gridCorreo t WHERE t.idReferencia=dom.id__638_tablaDinamica) AS correos,
								'DESPAHO' AS tipoRegistro
								 FROM _17_tablaDinamica d,_638_tablaDinamica dom WHERE dom.idReferencia=d.id__17_tablaDinamica
								 ".($state!=""?" and dom.municipio='".cv($state)."'":"")."
								 UNION
								 
								 SELECT d.nombreInmueble AS nombreDespacho,d.claveUnidad AS IDDespacho,d.cveInmueble AS claveDespacho,
								 dom.calleSede AS domicilio, 
								(SELECT estado FROM 820_estados WHERE cveEstado=dom.departamento) AS departamento, 
								(SELECT municipio FROM 821_municipios WHERE cveMunicipio=dom.municipio) AS municipio,dom.latitud,dom.longitud,
								(SELECT GROUP_CONCAT(CONCAT(numero,' Ext. ',extension)) FROM _640_telefonoSede t WHERE t.idReferencia=dom.id__640_tablaDinamica) AS telefonos,
								(SELECT GROUP_CONCAT(correoElectronico) FROM _640_email t WHERE t.idReferencia=dom.id__640_tablaDinamica) AS correos,
								'SEDE' AS tipoRegistro
								 FROM _1_tablaDinamica d,_640_tablaDinamica dom WHERE dom.idReferencia=d.id__1_tablaDinamica
								 ".($state!=""?" and dom.municipio='".cv($state)."'":"")."
								 ) as tmp order  by nombreDespacho";
				break;
				case 1://Despachos
					$consulta="SELECT d.nombreUnidad AS nombreDespacho,d.claveUnidad AS IDDespacho,d.claveRegistro AS claveDespacho,dom.calle AS domicilio,
								(SELECT estado FROM 820_estados WHERE cveEstado=dom.departamentos) AS departamento, 
								(SELECT municipio FROM 821_municipios WHERE cveMunicipio=dom.municipio) AS municipio,dom.latitud,dom.longitud,
								(SELECT GROUP_CONCAT(CONCAT(numero,' Ext. ',extension)) FROM _638_gridTelefono t WHERE t.idReferencia=dom.id__638_tablaDinamica) AS telefonos,
								(SELECT GROUP_CONCAT(email) FROM _638_gridCorreo t WHERE t.idReferencia=dom.id__638_tablaDinamica) AS correos,
				...
				break;
				case 2://Sedes
					$consulta=" SELECT d.nombreInmueble AS nombreDespacho,d.claveUnidad AS IDDespacho,d.cveInmueble AS claveDespacho,
								 dom.calleSede AS domicilio, 
								(SELECT estado FROM 820_estados WHERE cveEstado=dom.departamento) AS departamento, 
								(SELECT municipio FROM 821_municipios WHERE cveMunicipio=dom.municipio) AS municipio,dom.latitud,dom.longitud,
								(SELECT GROUP_CONCAT(CONCAT(numero,' Ext. ',extension)) FROM _640_telefonoSede t WHERE t.idReferencia=dom.id__640_tablaDinamica) AS telefonos,
			...
			return '{"result":"1","msgErr":"","numCourts":"'.$con->filasAfectadas.'","courts":'.$arrRegistros.'}';
	...
	$server->register('usersGetCourts',array('stringTokenAccess'=>'xsd:string','token'=>'xsd:string','state'=>'xsd:string','typeCout'=>'xsd:string'),array('return' => 'xsd:string'),$ns,false,'rpc','encoded','');
