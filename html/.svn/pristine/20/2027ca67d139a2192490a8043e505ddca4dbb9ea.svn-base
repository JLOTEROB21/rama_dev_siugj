			    ...
			    <tr>    
				  ...
				  <!-- InstanceBeginEditable name="content2" --> 
                   <tr>
						...
						?>
                        	<script type="text/javascript" src="../nomina/Scripts/modificarPerfilNomina.js.php?iPerfil=<?php echo $idPerfil?>"></script>
                        <?php	
							$nivelAcumulador="0";
							...
							$consulta="SELECT nombrePerfil,idFuncionRecalculo,idFuncionEliminacion FROM 662_perfilesNomina WHERE idPerfilesNomina=".$idPerfil;
							$fPerfil=$con->obtenerPrimeraFila($consulta);
							$nPerfil=$fPerfil[0];
							$idFuncionRecalculo=$fPerfil[1];
							$idFuncionEliminacion=$fPerfil[2];
							$tituloModulo=$fPerfil[0];
							echo formatearTituloPagina($tituloModulo,false,$idPerfil);
							
							$consulta="SELECT idEtiquetaAgrupadora,concat('[',clave,'] ',etiqueta)  FROM 718_etiquetasAgrupadoras WHERE idPerfilNomina=".$idPerfil." and idCategoria=2";
							$arrEtiquetasAgrP=$con->obtenerFilasArreglo($consulta);
							$consulta="SELECT idEtiquetaAgrupadora,concat('[',clave,'] ',etiqueta)  FROM 718_etiquetasAgrupadoras WHERE idPerfilNomina=".$idPerfil." and idCategoria=1";
							$arrEtiquetasAgrD=$con->obtenerFilasArreglo($consulta);
							
						?>
                        <input type="hidden" id="idPerfil" value="<?php echo $idPerfil?>" />
                        <input type="hidden" id="arrAcumuladores" value="<?php echo $arrAcumuladores?>" />
                        <input type="hidden" id="arrEtiquetasAgrP" value="<?php echo bE($arrEtiquetasAgrP)?>" />
                        <input type="hidden" id="arrEtiquetasAgrD" value="<?php echo bE($arrEtiquetasAgrD)?>" />
                        <input type="hidden" id="lblPerfil" value="<?php echo $nPerfil?>" />
                        <input type="hidden" id="idFuncionRecalculo" value="<?php echo $idFuncionRecalculo?>" />
                        <input type="hidden" id="idFuncionEliminacion" value="<?php echo $idFuncionEliminacion?>" />
                        
                            <table>
                            	<tr>
                                	<td><span id="tblCalculos"></span></td>
                                </tr>
                            </table>
                        </td>
                   </tr>
