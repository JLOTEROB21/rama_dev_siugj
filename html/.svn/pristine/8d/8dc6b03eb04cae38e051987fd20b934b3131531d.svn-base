				  ...
				  <!-- InstanceBeginEditable name="content2" --> 
                   <tr>
                        <td align="center">
                        	<?php 
								$idMensajeEnvio=-1;
								if(isset($objParametros->idMensajeEnvio))
									$idMensajeEnvio=$objParametros->idMensajeEnvio;	
									
								$consulta="select * from 2011_mensajesEnvio WHERE idMensajeEnvio=".$idMensajeEnvio;
								$fMensaje=$con->obtenerPrimeraFila($consulta);	
								$arrValoresCuerpo=$fMensaje[7];
								if($arrValoresCuerpo=="")
									$arrValoresCuerpo="[]";
								$arrDestinatarios="";
								if($fMensaje[8]!="")
								{
									$cadObj='{"arrReg":'.$fMensaje[8].'}';
									$obj=json_decode($cadObj);
									if(sizeof($obj->arrReg)>0)
									{
										foreach($obj->arrReg as $r)
										{
											$o="['".$r->idDestinatario."','".$r->destinatario."','".$r->tipoDestinatario."']";
											if($arrDestinatarios=="")
												$arrDestinatarios=$o;
											else
												$arrDestinatarios.=','.$o;
								...
								$arrRemitente="[".$arrRemitente."]";
								
								$arrDocumentosAdj="";
								if($fMensaje[11]!="")
								{
									$cadObj='{"arrReg":'.$fMensaje[11].'}';
									$obj=json_decode($cadObj);
									...
									if(sizeof($obj->arrReg)>0)
									{
										...
										foreach($obj->arrReg as $r)
										{
											...
											if($r->tDocumento==1)
											{
												$consulta="SELECT nombreDocumento,tamano FROM 9048_galeriaDocumentos WHERE idGaleriaDocumentos=".$r->idDocumento;
												$fDocumento=$con->obtenerPrimeraFila($consulta);
												$nDocumento=cv($fDocumento[0]);
												$tamano=$fDocumento[1];
												if($tamano<1024)
												{
											...
											{
												$nDocumento=$r->nombreDocumento;
												$tamano="N/D";
											}
											$o="['".$r->idDocumento."','".$nDocumento."','".$tamano."','".$r->tDocumento."']";
											if($arrDocumentosAdj=="")
												$arrDocumentosAdj=$o;
											else
												$arrDocumentosAdj.=','.$o;
								...
								$arrDocumentosAdj="[".$arrDocumentosAdj."]";
								
								
								$consulta="SELECT cuerpoMensaje FROM 2013_cuerposMensajes WHERE idMensaje=".$idMensajeEnvio;
								$cuerpo=$con->obtenerValor($consulta);
								
								$consulta="SELECT parametro,orden FROM 2012_parametrosMensajeEnvio WHERE idMensaje=".$idMensajeEnvio." ORDER BY orden";
                            ...
                            <script language="javascript" src="../mensajesAccion/Scripts/mensajeEnvio.js.php"></script>
                          	...
                          	<tr>
                            	<td align="left">
                                	...
                                	<tr>
                                        ...
                                        <input type="hidden" value="<?php echo bE($arrRemitente) ?>" id="arrRemitente" />
                                        <input type="hidden" value="<?php echo bE($arrDocumentosAdj) ?>" id="arrDocumentosAdj" />
                                        <input type="hidden" value="<?php echo bE($arrDestinatarios) ?>" id="arrDestinatarios" />
                                        <input type="hidden" value="<?php echo bE($arrCC) ?>" id="arrCC" />
                                        <input type="hidden" value="<?php echo bE($arrCCO) ?>" id="arrCCO" />
                                        <input type="hidden" value="<?php echo bE($arrValoresCuerpo) ?>" id="arrValoresCuerpo" />
                                        <input type="hidden" value="<?php echo $idMensajeEnvio ?>" id="idMensajeEnvio" />
                                        <input type="hidden" value="<?php echo bE($cuerpo) ?>" id="cuerpo" />
                                        <input type="hidden" value="<?php echo bE($arrParam) ?>" id="arrParam" />
                                                	...
                                                	<tr>
                                                        ...
                                                        <td>
                                                            <table width="880">
                                                                <tr height="21">
                                                                    <td width="330">
                                                                        <span class="SIUGJ_Etiqueta">Título mensaje: </span><span class="letraRoja">*</span>
                                                                    </td>
                                                                    <td width="550">
                                                                        <input class="SIUGJ_Control" type="text" value="<?php echo $fMensaje[1]?>" size="60" id="txtTitulo"  />
                                                                    </td>
                                                                </tr>
                                                                <tr height="21" valign="top">
                                                                    <td >
                                                                        <span class="SIUGJ_Etiqueta">Descripción:</span>
                                                                    </td>
                                                                    <td >
                                                                        <textarea class="SIUGJ_Control" id='txtDescripcion' style="width:600px; height:60px !important;" ><?php echo $fMensaje[2]?></textarea>
                                                                    </td>
                                                                </tr>
                                                                <tr height="21">
                                                                    <td >
                                                                        <span class="SIUGJ_Etiqueta">Categoría: </span><span class="letraRoja">*</span>
                                                                    </td>
                                                                    <td >
                                                                       <select id="cmbCategoria"  class="SIUGJ_Control">
                                                                       	<option value="-1">Seleccione</option>
                                                                       <?php
																	   		$consulta="SELECT idCategoria,nombreCategoria FROM 2010_categoriasMensajeEnvio ORDER BY nombreCategoria";
																	   		echo $con->generarOpcionesSelect($consulta,$fMensaje[5]);
																	   ?>
                                                                       </select>
                                                                    </td>
                                                                </tr>
                                                                <tr height="21">
                                                                    <td  valign="top">
                                                                        <span class="SIUGJ_Etiqueta">Parámetros del mensaje:</span>
                                                                    </td>
                                                                    <td >
                                                                      <span id="tblParametros"></span>
                                                                    </td>
                                                                </tr>
                                                            </table>
                                                        </td>
                                        ...
                                        <span id="tblCabecera"  class="x-hide-display">	
                                        <br /><br />
                                        <table >
                                        	<tr>
                                            	<td width="40">
                                                ...
                                                <td>
                                                    <table width="880">
                                                        <tr height="21">
                                                            <td width="220">
                                                                <span class="SIUGJ_Etiqueta">Asunto del mensaje: </span><span class="letraRoja">*</span>
                                                            </td>
                                                            <td width="660">
                                                            	<input class="SIUGJ_Control" type="text" value="<?php echo $fMensaje[6]?>" style="width:600px"  id="txtAsunto" />
                                                            </td>
                                                        </tr>
                                                        <tr height="15">
                                                        	<td>
