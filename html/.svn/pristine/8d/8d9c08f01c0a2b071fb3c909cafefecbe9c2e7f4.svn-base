...
function inicializarCombos()
{
    ...
    singular=gE('singular').value;
    plural=gE('plural').value;
...
function agregarAutor()
{
	if(validar())
	{
        ...
        var nDiv=1;
        var cmbDiv;
        var idParticipacion=obtenerValorSelect('cmbParticipacion');
        ...
        var datosAutor='{"idParticipacion":"'+idParticipacion+'","idFormulario":"'+idForm+'","idRegistro":"'+idReg+'","apPaterno":"'+cv(apPaterno)+'","apMaterno":"'+cv(apMaterno)+'","nombres":"'+cv(nombre)+'","telefonos":"'+cv(telefonos)+'","email":"'+cv(mail)+'","codInstitucion":"'+cv(codInstitucion)+'","codDepto":"'+cv(codDepto)+'"}';
		...
		function funcGuardar()
		{
			...
			if((arrResp[0]=='1')||(arrResp[0]==1))
			{
            	function resp()
                {
                	...
                	Ext.MessageBox.confirm('<?php echo $etj["lblAplicacion"]?>','El '+singular+' ha sido agregado satisfactoriamente, desea continuar registrando '+plural+'?',respAgregar);
                }
				Ext.MessageBox.alert('<?php echo $etj["lblAplicacion"] ?>','<?php echo $etj["lblOperacionOK"] ?>',resp);
			}
			else
				msgBox('<?php echo $etj["errOperacion"].' '?>'+arrResp[0]);
		}
		obtenerDatosWeb("../paginasFunciones/funcionesProyectos.php",funcGuardar,'POST','funcion=29&'+'datosAutor='+datosAutor,true);//33
	}
}
		...
		(
			{
				...
				typeAhead: false,
				minChars:1,
				loadingText: '<?php echo $etj["lblBuscando"]?>...',
				width: 400,
				pageSize:10,
				hideTrigger: true,
				tpl: resultTpl,
...
function nuevaOrgClick()
{
	...
	if(txtBtnAgregar=='<?php echo $etj["lblNuevo"] ?>...')
	{
	...
	else
	{
		...
		gE('btnAgregarOrg').value='<?php echo $etj["lblNuevo"] ?>...';
		gE('idEstado').value='0';
...
function validar()
{
	...
	if(trim(nombre)=='')
	{
		...
		Ext.MessageBox.alert('<?php echo $etj["lblAplicacion"] ?>','El nombre del '+singular+' es obligatorio',funcAceptar);
		return false;
	}
	...
	if(!validarCorreo(mail))
	{
		...
		Ext.MessageBox.alert('<?php echo $etj["lblAplicacion"] ?>','<?php echo $etj["lblErrMailOb"] ?>',funcAceptar2);
		return false;
	}
														...
														handler:function ()
															{
																...
																var objParam='{"codigoUPadre":"","nombre":"'+cv(txtInstitucion.getValue())+'","descripcion":"","institucion":"1","objInst":'+objIns+'}';
