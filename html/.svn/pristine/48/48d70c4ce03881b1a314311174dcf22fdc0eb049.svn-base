...
<?php
	...
	if(existeRol("'93_0'"))
	{
		echo '<link rel="stylesheet" type="text/css" href="../Scripts/ext/resources/css/xtheme-grayCensida.css"/>';
	}
?>
				  ...
				  <!-- InstanceBeginEditable name="content2" --> 
                   <tr>
                        ...
                        <?php
							...
							$ocultarBarraBotones=0;
							if(isset($objParametros->ocultarBarraBotones))
								$ocultarBarraBotones=$objParametros->ocultarBarraBotones;
							$sL=0;
							if(isset($objParametros->sL))
								$sL=$objParametros->sL;			
							?>
                            <script type="text/javascript" src="../modeloProyectos/Scripts/visorRegistroProcesosV2.js.php?l=<?php echo bE($sL)?>&tS=<?php echo $tipoSeccionProceso?>&iR=<?php echo bE($idReferencia)?>&iF=<?php echo bE($idFormulario)?>&idUsuario=<?php echo bE($idUsuario)?>&actor=<?php echo bE($actor)?>"></script>
							<?php
							...
							if($paginasScripts!="")
							{
								...
								{
									if(strpos($p,".css")===false)
										echo '<script type="text/javascript" src="'.$p.'?l='.bE($sL).'&tS='.$tipoSeccionProceso.'&iR='.bE($idReferencia).'&iF='.bE($idFormulario).'&idUsuario='.bE($idUsuario).'&actor='.bE($actor).'"></script>';
									else
										echo '<link rel="stylesheet" type="text/css" href="'.$p.'?l='.bE($sL).'&tS='.$tipoSeccionProceso.'&iR='.bE($idReferencia).'&iF='.bE($idFormulario).'&idUsuario='.bE($idUsuario).'&actor='.bE($actor).'" media="screen" />';
								}
							}
							$tipoVista=2;
							...
							$pantallaCompleta=0;
							if(isset($objParametros->pantallaCompleta))
								$pantallaCompleta=$objParametros->pantallaCompleta;
								
							$contentIframe=0;
							if(isset($objParametros->contentIframe))
								$contentIframe=$objParametros->contentIframe;
                        ...
                        <input type="hidden" id="pantallaCompleta" value="<?php echo $pantallaCompleta?>" />
                        <input type="hidden" id="contentIframe" value="<?php echo $contentIframe?>" />
                        <input type="hidden" id="soloContenido" name="soloContenido" value="<?php echo ($soloContenido?'1':'0')?>" />
                        <input type="hidden" id="parametrosProceso" name="parametrosProceso" value="<?php echo $parametrosProceso?>" />
                        <input type="hidden" id="tituloModulo" value="<?php echo bE($tituloModulo)?>" />
                        <input type="hidden" id="ocultarBarraBotones" value="<?php echo $ocultarBarraBotones ?>" />
