...
<link rel="stylesheet" type="text/css" href="../Scripts/ux/treeGrid/treegrid.css" media="screen" />
...
<script type="text/javascript" src="../Scripts/ux/treeGrid/EditorGrid.js"></script>
			  ...
			  <table width="100%" border="0" cellpadding="2" cellspacing="2" bgcolor="#FFFFFF">
			    ...
			    <tr>    
				  ...
				  <!-- InstanceBeginEditable name="content2" --> 
                  	<?php
							$accion=$objParametros->hAccion;
							$id=$objParametros->hId;
							$idDocumento=$objParametros->idProceso;
							$idProceso=$objParametros->idProceso;
    				?>
                  	<script type="text/javascript" src="Scripts/cfgDTD.js.php?iD=<?php echo bE($idDocumento)?>"></script>
                    <script type="text/javascript" src="Scripts/cfgDTD2.js.php?iD=<?php echo bE($idDocumento)?>"></script>
                  <?php
				  	$consulta="select nombre,idTipoProceso,situacion,repetible,valoresDefault,complementario,hashDiferenciaTiempo from 4001_procesos where idProceso=".$idDocumento;
					$fProceso=$con->obtenerPrimeraFila($consulta);
					$tProceso=$fProceso[1];
					$situacion=$fProceso[2];
					$repetible=$fProceso[3];
					$cadObjConf=$fProceso[4];
					$hashDiferenciaTiempo=$fProceso[6];
					$objConf;
					if($cadObjConf!="")
					{
					...
					$formularioBase=obtenerFormularioBase($idProceso);
					if($formularioBase=="")
						$formularioBase="-1";
					$consulta="SELECT idGrupoElemento,nombreCampo FROM 901_elementosFormulario WHERE idFormulario=".$formularioBase." AND tipoElemento IN (2,3,4,14,15,16)";
					$arrCampos=$con->obtenerFilasArreglo($consulta);
					$tituloModulo.=$fProceso[0]	;
					
					$consulta="SELECT numEtapa,concat(numEtapa,'.- ',nombreEtapa) FROM 4037_etapas WHERE idProceso=".$idProceso." ORDER BY numEtapa";
					...
					<table width="900" border="0" >
                        ...
                        <tr>
                            <td align="left" width="400" colspan="4">
	                            <br /><br />
    	                        <span id="divEscenario" class="x-hide-display"></span>
                            
        	                </td>
                    	</tr>
                        <input type="hidden" id="hAccion" name="hAccion" value="<?php echo $accion ?>">
                        <input type="hidden" id="hTArticulo" name="hTArticulo" value="1">
                        <input type="hidden" id="idDocumento" name="idDocumento" value="<?php echo $idDocumento?>" />
                        <input type="hidden" id='cmbSituacionCache' value="<?php echo $situacion ?>" />
                        <input type="hidden" id="tProceso" value="<?php echo $tProceso?>"/>
                        <input type="hidden" id="idProceso" value="<?php echo $idProceso?>" />
                        <input type="hidden" id="formularioBase" value="<?php echo $formularioBase?>" />
                        <input type="hidden" id="arrCampos" value="<?php echo bE($arrCampos)?>" />
                        <input type="hidden" id="hashDiferenciaTiempo" value="<?php echo $hashDiferenciaTiempo?>" />
