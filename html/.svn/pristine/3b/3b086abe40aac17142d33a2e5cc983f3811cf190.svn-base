...
include_once("cConectoresGestorContenido/cOneDrive.php");	
	...
	try
	{
		...
		if($fFormato[3]==1)
		{
			echo '{"resultado":"3","mensaje":"","documentoBloqueado":"1"}';
			return;
		}
		...
		else
		{
			if($fFormato[2]!="")  //DOcumento adjunto
			{
				...
				else
				{
					...
					if(($fFormato[3]==2)&&(!$modoColegiadoNativo))
					{
						$comando="gs -q -dNOPAUSE -dBATCH -sDEVICE=pdfwrite -sOutputFile=".$archivoDestino." -c 3000000 setvmthreshold -f ".$archivoTemporalDestino;
						$arrResultado=array();
						$valResultado=0;
			...
			else
			{
				...
				$resultado=generarDocumentoPDFFormatoFirmaElectronicaConsejo($obj->idRegistroFormato,true,$documentosAnexos,$documentoBloqueado);
				echo $resultado;
				return;
			}
		}
...
function generarDocumentoPDFFormatoFirmaElectronicaConsejo($idRegistroFormato,$descomponerDocumentoMarcadores=true,$documentosAnexos="",$bloquearDocumento=1)
{
	global $con;
	global $baseDir;
