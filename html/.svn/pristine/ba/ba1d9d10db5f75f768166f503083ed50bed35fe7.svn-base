...
function obtenerTitularSecretaria($idFormulario,$idRegistro,$actorDestinatario)
{
	...
	{
		case 699:
			$campoCarpeta="carpetaAdministrativaActuacionesIntervinientes";
		break;
		case 706:
...
function obtenerTitularPuestoDespachoPendienteFirma($idFormulario,$idRegistro,$actorDestinatario)
{
	...
	$consulta="SELECT despachoFimante FROM _04001_firmasColegiadasDocumentos WHERE iFormulario=".$idFormulario." AND iRegistro=".$idRegistro.
				" AND firmado=0 ORDER BY orden asc";
...
function obtenerTitularPuestoSiSustanciadorNOAsignado($idFormulario,$idRegistro,$actorDestinatario)
	{
		global $con;
		global $tipoMateria;
		...
		$arrRegistros=obtenerSustanciadorAsignado($idFormulario,$idRegistro,$actorDestinatario);
	...
	function esProvidenciaFijacionLista($idFormulario,$idRegistro)
	{
		global $con;
		
		$arrAutos["730"]=1;
		$consulta="SELECT *FROM _1251_tablaDinamica WHERE id__1251_tablaDinamica=".$idRegistro;
		$fDatosRegistro=$con->obtenerPrimeraFilaAsoc($consulta);
		
		if($fDatosRegistro)
		{
			if(isset($arrAutos[$fDatosRegistro["idTipoDocumentoPrincipal"]]))
				return 1;
		}
		return 0;
	...
	function esProvidenciaDistintoFijacionLista($idFormulario,$idRegistro)
	{
		global $con;
