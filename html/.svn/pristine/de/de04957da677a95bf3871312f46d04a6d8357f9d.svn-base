<?php include_once("conexionBD.php");
	...
	class cMicrosoftGraph
	{
		...
		function cMicrosoftGraph($idConfiguracionConexion) 
		{
			...
			$this->scope = "offline_access user.read User.Read.All User.ReadWrite.All, Directory.Read.All, Directory.ReadWrite.All, calendars.read calendars.readwrite MailboxSettings.Read files.read.all files.readwrite.all OnlineMeetings.ReadWrite Chat.Read.All";
		...
		function callAPI($method, $url, $data, $token,$isJSON = false, $timeZone = false, $setHeaders = false)
		{
			...
			try
			{
				$this->cabecerasUltimaEjecucion=array();
				$curl = curl_init();
