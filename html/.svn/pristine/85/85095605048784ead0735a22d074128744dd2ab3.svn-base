				  ...
				  <!-- InstanceBeginEditable name="content2" --> 
                   <tr>
						...
						if(isset($objParametros->sL))
							$sL=$objParametros->sL;	
					  ?>
					  	<script type="text/javascript" src="../modulosEspeciales_SIUGJ/Scripts/tblRegistroParticipantes.js.php?iF=<?php echo bE($idFormulario)?>&iR=<?php echo bE($idRegistro)?>"></script>
					  <?php
					  	$nombreTabla=obtenerNombreTabla($idFormulario);
					  
					  	$consulta="SELECT idActividad FROM ".$nombreTabla." WHERE id_".$nombreTabla."=".$idRegistro;
						...
						if($idFormulario==1116)
						{
							...
							$procesoJudicial=$con->obtenerValor($consulta);
							
							$consulta="SELECT idActividad FROM 7006_carpetasAdministrativas WHERE carpetaAdministrativa='".$procesoJudicial."'";
							$idActividadReferencia=$con->obtenerValor($consulta);
							$consulta="SELECT COUNT(*) FROM 7005_relacionFigurasJuridicasSolicitud WHERE idActividad=".$idActividad;
							
							$numParticipantes=$con->obtenerValor($consulta);

							if($numParticipantes==0)
							{
								$consulta="INSERT INTO 7005_relacionFigurasJuridicasSolicitud(idActividad,idParticipante,idFiguraJuridica,situacion,idCuentaAcceso,etapaProcesal,situacionProcesal,
											detalleSituacion,cuentaAccesoGenerica)
											SELECT ".$idActividad.",idParticipante,idFiguraJuridica,situacion,idCuentaAcceso,etapaProcesal,situacionProcesal,detalleSituacion,cuentaAccesoGenerica 
											FROM 7005_relacionFigurasJuridicasSolicitud
											WHERE idActividad=".$idActividadReferencia;
