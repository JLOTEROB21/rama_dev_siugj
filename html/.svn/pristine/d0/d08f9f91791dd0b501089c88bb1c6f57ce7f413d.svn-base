...
<script type="text/javascript" src="../liquidadorJudicial/Scripts/perfilLiquidacion.js.php"></script>
<style>
.x-grid3-cell-inner 
{
    min-height: 21px;
				  ...
				  <!-- InstanceBeginEditable name="content2" --> 
                   <tr>
                        ...
                        <?php
							...
							{
								$idPerfil=	$objParametros->idRegistro;
							}
							
							$consulta="SELECT cvePerfil,nombrePerfil,descripcion,situacionActual,ambitoGlobal FROM 20010_perfilesLiquidacion WHERE idRegistro=".$idPerfil;
							$arrRegistro=utf8_encode($con->obtenerFilasJSON($consulta));
							
							$consulta="SELECT nombreVariable FROM 20010_variablesGlobales WHERE idPerfilLiquidacion=".$idPerfil." ORDER BY nombreVariable";
							$arrVariables=utf8_encode($con->obtenerFilasArreglo($consulta));
							
							$arrParametros="";
							$consulta="SELECT etiquetaParametro,nombreParametro,tipoEntrada,fuenteDatos,funcionSistema	,
											descripcion,opcionesFuncion FROM 20010_parametrosLiquidacion
										 WHERE idPerfilLiquidacion=".$idPerfil." ORDER BY idRegistro";
							
							$res=$con->obtenerFilas($consulta);
							while($fila=mysql_fetch_assoc($res))
							{
								$lblFuncionSistema="";
								if($fila["funcionSistema"]==-1)
								{
								...
								$objParams='{"etiquetaParametro":"'.cv($fila["etiquetaParametro"]).'","nombreParametro":"'.cv($fila["nombreParametro"]).
								'","tipoEntrada":"'.cv($fila["tipoEntrada"]).'","fuenteDatos":"'.cv($fila["fuenteDatos"]==-1?"":$fila["fuenteDatos"]).
								'","funcionSistema":"'.($fila["funcionSistema"]==-1?"":$fila["funcionSistema"]).
								'","lblFuncionSistema":"'.cv($lblFuncionSistema).'","descripcion":"'.cv($fila["descripcion"]).'","opcionesFuncion":'.
								cv($fila["opcionesFuncion"]).'}';
								
								
								$o="['".$fila["nombreParametro"]."','".$fila["etiquetaParametro"]."','".$objParams."']";
								if($arrParametros=="")
								{
									$arrParametros=$o;
								}
                        ...
                        ?>
                            ...
                            <input type="hidden" id="registroBase" value="<?php echo bE($arrRegistro)?>" />
                            <input type="hidden" id="arrVariables" value="<?php echo bE($arrVariables)?>" />
