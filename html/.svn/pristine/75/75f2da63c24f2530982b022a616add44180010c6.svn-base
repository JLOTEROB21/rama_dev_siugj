				...
				if($r1 && $r2)
				{
					if(copy($archivoTemporalDestino,$archivoOrigen)&&copy($archivoTemporalDestino.".pkcs7",$archivoOrigen.".pkcs7"))
					{
						...
						$consulta="INSERT INTO 7071_promocionesRegistradasUsuario(expediente,idExpediente,fechaRegistro,idUsuarioRegistro,asunto,
									...
									VALUES('".$obj->expediente."',".$obj->idExpediente.",'".date("Y-m-d H:i:s")."',".$_SESSION["idUsr"].",'".cv(urldecode($obj->asunto)).
									"',1,".$idDocumento.",'".$obj->cveUnidadGestion."','".$obj->cveMateria."')";
						$con->ejecutarConsulta($consulta);
						echo '{"resultado":"1","mensaje":"","idPromocion":"'.$con->obtenerUltimoID().'"}';
