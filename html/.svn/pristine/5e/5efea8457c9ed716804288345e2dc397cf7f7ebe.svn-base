<?php
	...
	if($idRegistro!=-1)
	{
		$consulta="SELECT victima FROM _425_tablaDinamica WHERE id__425_tablaDinamica=".$idRegistro;	
...
function inyeccionCodigo()
{
    ...
    {
       asignarEvento(gE('_victimavch'),'change',buscarDomilicio)
	}
    
    gE('sp_6762').innerHTML='';
...
function participanteAgregado(iParticipante,nombre)
{
    ...
    opt.text=nombre;
	gE('_victimavch').options[gE('_victimavch').options.length]=opt;
    gE('_victimavch').selectedIndex=gE('_victimavch').options.length-1;
    buscarDomilicio();
}
