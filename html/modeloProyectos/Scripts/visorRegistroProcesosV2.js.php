<?php
	...
	if($tipoProceso!=1)
	{
		...
		$formularioBase=$con->obtenerValor($consulta);
		if($formularioBase==0)
		{
			$tipoProceso=0;
		}
	...
	$consulta="select f.idConfGrid,campoAgrupacion,btnAgregarLabel,btnModificarLabel,btnEliminarLabel,btnVerLabel,mostrarVistaPrevia,
				botonesOcultar,botonesPersonales,configuracionExtra,configuracionEventos,funcionVistaRegistro from 909_configuracionTablaFormularios f where f.idFormulario=".$idFormulario;
	...
	$funcionVistaRegistro=$filaConfFrm[11];
	if($funcionVistaRegistro=="")
	{
		$funcionVistaRegistro="generarVistaDefault";
	}
	...
	$consulta="	select cg.titulo,tamanoColumna as 'tamColumna',case cg.alineacionValores when 1 then 'left' when 2 then 'right' when 3 then 'center' 
				...
				idGrupoElemento=cg.idElementoFormulario) is null,
				 case cg.idElementoFormulario
					 ...
					 when '-28' then 'idRegistro'
					 end
				   ,(select distinct(nombreCampo) 
				from 901_elementosFormulario where idGrupoElemento=cg.idElementoFormulario)) as nombreCampo,idFuncionRenderer,visible
...
//	new  Ext.grid.RowNumberer({width:35})
								...
								renderer:function(val,meta,registro)
										{
											if(registro.data.permisos)
											{
												...
												if(permisos.indexOf('F')!=-1)
												{
													tEliminacion=0;
												}
												
												if((registro.data.permisos.indexOf('E')!=-1)||(registro.data.permisos.indexOf('F')!=-1))
													return '<a href=\"javascript:removerRegistro(\''+bE(registro.get('idRegistro'))+'\',\''+bE(tEliminacion)+'\')\"><img src=\"../principalPortal/imagesSIUGJ/delete.png\" alt=\"".cv($btnEliminarLabel)."\" title=\"".cv($btnEliminarLabel)."\"></a>'
											}
	...
	while($filas=mysql_fetch_row($res))
	{
		...
		if($filas[6]!="")
		{
			...
			$consulta="SELECT COUNT(*) FROM 9033_rolesFuncionesScripts WHERE idFuncion=".$filas[6];
			$numElementos=$con->obtenerValor($consulta);
			if($numElementos>0)
			{
				$consulta="SELECT COUNT(*) FROM 9033_rolesFuncionesScripts WHERE idFuncion=".$filas[6]." AND rol IN(".$_SESSION["idRol"].")";
				$numElementos=$con->obtenerValor($consulta);
				if($numElementos==0)
				{
					$funcRenderer="";
		...
		switch($filas[4])
		{
			...
			case 15:
				...
				$consulta="SELECT * FROM 904_configuracionElemFormulario WHERE idElemFormulario=".$filas[3];
				$fComp=$con->obtenerPrimeraFila($consulta);
	...
	if(($tipoProceso!=1)&&($tipoProceso!=0))
	...
	$consulta="select idEtapa,idProceso,anchoGrid,altoGrid from 900_formularios where idFormulario=".$idFormulario;
	...
	while($fConfiguracion=mysql_fetch_row($rConfiguracion))
	{
		...
		switch($fConfiguracion[3])
		{
			case 1://
				...
				$control='crearComboExt("op1_'.$fConfiguracion[2].'",arrCondicionales,0,0,60,{ctCls:"comboWrapSIUGJ",cls:"comboSIUGJ",listClass:"listComboSIUGJControl",valor:"'.$operador1.'"}),{xtype:"tbspacer",width:5},new Ext.form.NumberField({id:"filtro1_'.$fConfiguracion[2].'",width:"40",allowDecimals:true,allowNegative:true,value:"'.$valor1.'", enableKeyEvents:true,cls:"controlSIUGJ"}),'.
						'{xtype:"tbspacer",width:5},{id:"btnClean_'.$fConfiguracion[2].'_1",icon:"../images/find_remove.png",border:true,width:40,cls:"x-btn-icon btnSiugjSmall",handler:function(btn){limpiarFiltro(btn)}},{xtype:"tbspacer",width:5},{"xtype":"label",html:"<div class=\'letraNombreTablero\'>y</div>"},{xtype:"tbspacer",width:15},crearComboExt("op2_'.$fConfiguracion[2].'",arrCondicionales,0,0,60,{ctCls:"comboWrapSIUGJ",cls:"comboSIUGJ",listClass:"listComboSIUGJControl",valor:"'.$operador2.'"}),{xtype:"tbspacer",width:5},new Ext.form.NumberField({id:"filtro2_'.
							$fConfiguracion[2].'",width:"40",allowDecimals:true,allowNegative:true,value:"'.$valor2.'", enableKeyEvents:true,cls:"controlSIUGJ"}),{xtype:"tbspacer",width:5},{id:"btnClean_'.$fConfiguracion[2].'_2",icon:"../images/find_remove.png",border:true,width:40,cls:"x-btn-icon btnSiugjSmall",handler:function(btn){limpiarFiltro(btn)}}';
			...
			case 4://
				...
				$totalValores=0;
				$aValoresAnalisis=explode("],[",str_replace("]]","]",str_replace("[[","[",$arrValores)));
				$deshabilitarLimpiezaFiltro=false;
				if((sizeof($aValoresAnalisis)==1)&&($valor1!="") && ($valor1!=-1))
					$deshabilitarLimpiezaFiltro=true;

				$control='crearComboExt("filtro1_'.$fConfiguracion[2].'",arrValores_'.$fConfiguracion[2].',0,0,'.$fConfiguracion[4].',{ctCls:"comboWrapSIUGJ",cls:"comboSIUGJ",listClass:"listComboSIUGJControl",valor:"'.$valor1.'"'.$compMulti.'}),{xtype:"tbspacer",width:5},{id:"btnClean_'.$fConfiguracion[2].'_1",hidden:'.($deshabilitarLimpiezaFiltro?"true":"false").',icon:"../images/find_remove.png",border:true,width:40,cls:"x-btn-icon btnSiugjSmall",handler:function(btn){limpiarFiltro(btn)}}';
				$tipoControl="";
				$campo="";
				if($idConfiguracionCtrl<0)
				{
			...
			case 5:
				...
				$control='crearComboExt("op1_'.$fConfiguracion[2].'",arrCondicionalesTexto,0,0,120,{ctCls:"comboWrapSIUGJ",cls:"comboSIUGJ",listClass:"listComboSIUGJControl",valor:'.$operador1.'}),{xtype:"tbspacer",width:5},new Ext.form.TextField({"id":"filtro1_'.$fConfiguracion[2].'",width:"'.$fConfiguracion[4].'",value:"'.
						$valor1.'", enableKeyEvents:true,cls:"controlSIUGJ"}),{xtype:"tbspacer",width:5},{id:"btnClean_'.$fConfiguracion[2].'_1",icon:"../images/find_remove.png",border:true,width:40,cls:"x-btn-icon btnSiugjSmall",handler:function(btn){limpiarFiltro(btn)}}';
				$tipoControl="";
				$campo="";
				if($idConfiguracionCtrl<0)
				{
			...
			case 6://
				...
				$control='crearComboExt("op1_'.$fConfiguracion[2].'",arrCondicionales,0,0,60,{ctCls:"comboWrapSIUGJ",cls:"comboSIUGJ",listClass:"listComboSIUGJControl",valor:"'.$operador1.'"}),{xtype:"tbspacer",width:5},new Ext.form.DateField({"id":"filtro1_'.$fConfiguracion[2].'",value:"'.$valor1.'","ctCls":"campoFechaSIUGJ"})'.
						',{xtype:"tbspacer",width:5},{id:"btnClean_'.$fConfiguracion[2].'_1",icon:"../images/find_remove.png",border:true,width:40,cls:"x-btn-icon btnSiugjSmall",handler:function(btn){limpiarFiltro(btn)}},{xtype:"tbspacer",width:5},{"xtype":"label",html:"<div class=\'letraNombreTablero\'>y</div>"},
						{xtype:"tbspacer",width:15},crearComboExt("op2_'.$fConfiguracion[2].'",arrCondicionales,0,0,60,{ctCls:"comboWrapSIUGJ",cls:"comboSIUGJ",listClass:"listComboSIUGJControl",valor:"'.
						$operador2.'"}),{xtype:"tbspacer",width:5},new Ext.form.DateField({"id":"filtro2_'.
						$fConfiguracion[2].'",value:"'.$valor2.'","ctCls":"campoFechaSIUGJ"}),{xtype:"tbspacer",width:5},{id:"btnClean_'.$fConfiguracion[2].
						'_2",icon:"../images/find_remove.png",border:true,width:40,cls:"x-btn-icon btnSiugjSmall",handler:function(btn){limpiarFiltro(btn)}}';
				$tipoControl="";
				$campo="";
				if($idConfiguracionCtrl<0)
				{
...
var visorExpedienteProcesos='<?php echo $visorExpedienteProcesos?>';
...
function crearGridRegistros()
{
    ...
    if((gE('tipoSeccionProceso').value=='1')||(gE('tipoSeccionProceso').value=='2'))
    	arrActoresDisp=[];
        
	var cmbActor=crearComboExt('cmbActor',arrActoresDisp,0,0,200,{listClass:"listComboSIUGJ","ctCls":"comboSIUGJGridHeader"});
	...
	function cargarDatos(proxy,parametros)
    {
        ...
        if((tipoProceso!='1') &&(gE('tipoSeccionProceso').value=='0'))
        {
            ...
            if((window.parent)&&(window.parent.gEx)&&(window.parent.gEx('btnDeleteRecordProceso'))&&(window.parent.gEx('btnDeleteRecordProceso').disable))
            {
            	window.parent.gEx('btnDeleteRecordProceso').disable();
            }
            ...
            if((obj.idActorAgregar!=undefined)&&(obj.idActorAgregar!='-1')&&(obj.idActorAgregar!='')&&('<?php echo $ocultarAgregar?>'=='false'))
            {
                ...
                if((window.parent)&&(window.parent.gEx)&&(window.parent.gEx('btnAddRecordProceso'))&&(gE('sL').value=='0'))
                {
                    window.parent.gEx('btnAddRecordProceso').enable();
                }
                else
                {
                	if((window.parent)&&(window.parent.gEx)&&(window.parent.gEx('btnAddRecordProceso'))&&(gE('sL').value=='1'))
                    {
                        window.parent.gEx('btnAddRecordProceso').disable();
                    }
                }
                
                if((btnNuevo)&&(gE('sL').value=='0'))
                {
                	btnNuevo.show();
            ...
            else
            {
                ...
                if((window.parent)&&(window.parent.gEx)&&(window.parent.gEx('btnAddRecordProceso')))
                {
                    window.parent.gEx('btnAddRecordProceso').disable();
                }
        ...
        for(x=0;x<arrIdFiltros.length;x++)                                     
        {
            ...
            if(filtro.getValue()!='')
            {
            	...
            	condicion='in('+cv(filtro.getValue(),true,true)+')';
                ...
                if(filtro.getXType()=='checkboxcombo')
                {
                    ...
                    {
                    	if(valoresCondicion=='')
                        	valoresCondicion="'"+cv(arrValores[aux],true,true)+"'";
                        else
                        	valoresCondicion+=",'"+cv(arrValores[aux],true,true)+"'";
                    }
                ...
                if(operando)
                {
                	if(f[2]=='10')
                    {
                    	...
                    	condicion=bE('{"idFuncionBusqueda":"'+filtro.idFuncionBusqueda+'","condicion":"'+operando.getValue()+
                        			'","valorBusqueda":"'+cv(filtro.getValue())+'"}');
                    }
                    else
                    {
        ...
        proxy.baseParams.parametrosProceso=gE('parametrosProceso').value;
...
;   
	...
	var vConfig=	<?php echo $funcionVistaRegistro."(".$habilitarGrouping.");" ?>;
    ...
    if(gE('ocultarBarraBotones').value=='0')
    {                        
        oConfiguracion.tbar=	[
                                    <?php
                                    if($sL==0)
                                    ...
                                    ?>
                                    
                                               {
                                                  ...
                                                  html:'&nbsp;&nbsp<span class="etiquetaBarraGrid">Vista:</span>&nbsp;&nbsp;&nbsp;&nbsp;'
                                              },
                                               {
                                                  id:'btn',
                                                  text:'',
                                              ...
                                              cmbActor,
                                             {
                                              ...
                                              <?php
                                                ...
                                                if($tipoSeccionProceso!=1)
                                                {
                                                      ...
                                                      {
                                                          id:'btnAbrir',
                                                          icon:'../principalPortal/imagesSIUGJ/open.png',
                                                          cls:'x-btn-text-icon',
                                                          disabled:true,
	...
	var mostrarTitulo=(gE('idProcesoPadre').value=='-1')&&(gE('tipoSeccionProceso').value=='0')&&(gE('pantallaCompleta').value=='0');
                                ...
                                items: 	[
                                			{
                                                ...
                                                region:'center',
                                                id:'pGridRegistros',
                                                cls:'panelSiugj <?php echo $arrFiltros!="{xtype:'tbspacer',width:20}"?"panelSiugjBorder":""?>',
                                                border:<?php echo $arrFiltros!="{xtype:'tbspacer',width:20}"?"true":"false"?>,
                                                layout:'border',
                                                <?php
                                                ...
                                                items:	[
                                                            {
                                                                ...
                                                                border:false,
                                                                cls:'panelSiugj',
                                                                title: mostrarTitulo?bD(gE('tituloModulo').value):false,
                                                                layout:'border',                                                                    
                                                                items:	[
    ...
    gridRegistros=	gEx('pGridRegistros');                                      
	...
	gEx('gridRegistros').on('rowclick',function(sm,nFila,r)
    													{
                                                        	var registro=gEx('gridRegistros').getStore().getAt(nFila);
                                                            
                                                            <?php
																
																switch($tipoSeccionProceso)
																{
																	case 0:
															?>
                                                                        var frameContenidoGrid=gEx('frameContenidoGrid');
                                                                        ...
                                                                        if((window.parent)&&(window.parent.gEx)&&(window.parent.gEx('btnWatchRecordProceso')))
                                                                        {
                                                                            window.parent.gEx('btnWatchRecordProceso').enable();
                                                                        }
                                                                        
                                                                        
                                                                        if((window.parent)&&(window.parent.gEx)&&(window.parent.gEx('btnWatchRecordProceso')))
                                                                        {
                                                                            window.parent.gEx('btnDeleteRecordProceso').enable();
                                                                        }
                                                                        ...
                                                                        if(((permisos.indexOf('E')==-1)&&(permisos.indexOf('F')==-1))||(gE('sL').value=='1'))
                                                                        {
                                                                            if(gEx('btnEliminar'))
                                                                            {
															...
															?>
                                                                        ...
                                                                        var cadObjConf=bD(gE('objConfAux').value);
                                                                        var oConf=eval('['+cadObjConf+']')[0];
                                                                        var cadObj='{"alto":"<?php echo $filaResp[3]+110?>","ancho":"<?php echo $filaResp[2]+80?>","idFormularioReferencia":"'+gE('idFormulario').value+
                                                                                    '","idReferencia":"'+gE('idReferencia').value+'","idRegistroReferencia":"'+registro.data.idRegistro+'","modalidad":"'+oConf.modalidad+'","soloLectura":"'+gE('sL').value+'","titulo":"'+cv(oConf.titulo,false,true)+'"}';
                                                                                    
                                                                        var oConf=eval('['+cadObj+']')[0];             
                                                                        window.parent.crearTabPaginaRegistroSeccion(oConf,true);
	...
	if((tipoProceso=='1')||(tipoProceso=='0'))
 	{
        ...
        if((cAccion.indexOf('M')!=-1)&&(gE('tipoSeccionProceso').value!='3'))
        {
...
function agregarNuevoRegistro()
{
	...
	?>
				...
				if((tipoProceso!='1') &&(gE('tipoSeccionProceso').value!='3'))
				{
					var pag='../modeloPerfiles/'+visorExpedienteProcesos;
					var arrDatos=[['parametrosProceso',parametrosProceso],['idRegistro','-1'],['idFormulario',gE('idFormulario').value],['dComp','YWdyZWdhcg=='],['actor',actor],['idProcesoP',gE('idProcesoPadre').value],['idReferencia',gE('idReferencia').value],['idUsuario','<?php echo $_SESSION["idUsr"]?>']];    
				   
					if(objParametrosProceso)
					{
						for(campo in objParametrosProceso)
						{
							arrDatos.push([campo,objParametrosProceso[campo]]);
						}
					}
					var objConf={};
					objConf.params=arrDatos;
					objConf.ancho='100%';
					objConf.alto='100%';
					objConf.url=pag;
					objConf.modal=true;
					objConf.openEffect='elastic';
					objConf.closeEffect='elastic';
					if(gE('contentIframe').value=='0')
						window.parent.abrirVentanaFancy(objConf);
					else
                    	if(gE('contentIframe').value=='100')
                        	window.parent.parent.abrirVentanaFancy(objConf);
                        else
							abrirVentanaFancySuperior(objConf);
				}
				else
				{
					
					var idReferencia=gE('idReferencia');
					var arrDatos=[['parametrosProceso',parametrosProceso],['idRegistro','-1'],['idFormulario',gE('idFormulario').value],['idReferencia',idReferencia.value],['dComp','<?php echo base64_encode("agregar")?>'],['formularioNormal','1']];
					if(objParametrosProceso)
					{
						for(campo in objParametrosProceso)
						{
	...
	?>
				var cadObjConf=bD(gE('objConfAux').value);
				var oConf=eval('['+cadObjConf+']')[0];
				var cadObj='{"alto":"<?php echo $filaResp[3]+100?>","ancho":"<?php echo $filaResp[2]+80?>","idFormularioReferencia":"'+gE('idFormulario').value+

							'","idReferencia":"'+gE('idReferencia').value+'","idRegistroReferencia":"-1","modalidad":"'+oConf.modalidad+'","soloLectura":"0","titulo":"'+cv(oConf.titulo,false,true)+'"}';
...
function removerRegistro(iR,tE)
{
	if(gE('ocultarBarraBotones').value=='1')
    {
    	msgConfirm=window.parent.msgConfirm;
        msgBox=window.parent.msgBox;
    }
	...
	function resp(btn)
    {
        if(btn=='yes')
        {
            ...
            obtenerDatosWeb('../paginasFunciones/funcionesFormulario.php',funcResp, 'POST','funcion=39&tE='+bD(tE)+'&idRegistro='+bD(iR)+'&idFormulario=<?php echo $idFormulario?>',true);
        }
    }
...
function generarVistaDefault(habilitarGrouping)
{
	return new Ext.grid.GroupingView(	{
                                              enableNoGroups :true,
                                              enableGrouping : habilitarGrouping,
                                              enableGroupingMenu : true,
                                              forceFit:false,
                                              hideGroupedColumn : habilitarGrouping,
                                              groupTextTpl: '{text}, (Total: {values.rs.length})',//(<font color="#000066">Total: </font> <font color="#FF0000">{values.rs.length}</font>)
...
function verInformacion()
{
    ...
    if((tipoProceso!='1') &&(gE('tipoSeccionProceso').value!='3'))
    {
          ...
          var pag='../modeloPerfiles/'+visorExpedienteProcesos;
          var arrDatos=[['idRegistro',fila.get('idRegistro')],['idFormulario',gE('idFormulario').value],['dComp','YXV0bw=='],['actor',idActorRegistro],['idProcesoP',gE('idProcesoPadre').value],['idReferencia',gE('idReferencia').value]];    
          
          if(tipoActor=='4')
          {
              pag='../modeloPerfiles/vistaRevisor.php';  
              arrDatos.push(['idProceso',gE('idProceso').value]);  
    ...
    else
    {
          ...
          if(gE('tipoSeccionProceso').value=='3')
          {
              var arreglo=new Array();
              arreglo[0]='pM';
              arreglo[1]=(gE('sL').value=='0')?'1':'0';
              arrDatos.push(arreglo);
              var arreglo=new Array();
              arreglo[0]='pE';
              arreglo[1]=(gE('sL').value=='0')?'0':'0';
              arrDatos.push(arreglo);
              var arreglo=new Array();
              arreglo[0]='pC';
              arreglo[1]='1';
          ...
          enviarFormularioDatos('../modeloPerfiles/verFichaFormularioV2.php',arrDatos);
    }
}

function removerRegistroBtn()
{
    ...
    var tEliminacion=1;
    var permisos=fila.get('permisos');
    if(permisos.indexOf('F')!=-1)
    {
        tEliminacion=0;
    }
    ...
    function resp(btn)
    {
        if(btn=='yes')
        {
            ...
            obtenerDatosWeb('../paginasFunciones/funcionesFormulario.php',funcResp, 'POST','funcion=39&tE='+tEliminacion+'&idRegistro='+fila.get('idRegistro')+'&idFormulario=<?php echo $idFormulario?>',true);
        }
    }
    msgConfirm('Est&aacute; seguro de querer eliminar el registro seleccionado?',resp);
}