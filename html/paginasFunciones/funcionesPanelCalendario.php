<?php session_start();
	...
	function obtenerJuecesGuardias()
	{
		global $con;
		$uG=$_POST["uG"];
		$fechaInicial=$_POST["start"];
		$fechaFinal=$_POST["end"];
		...
		$consulta="SELECT id__13_tablaDinamica,fechaInicio,fechaFinalizacion,'' as clave,u.nombre,ug. claveUnidad,'' as claveFolioCarpetas  FROM _13_tablaDinamica g,800_usuarios u,
					...
					u.idUsuario=g.usuarioJuez AND a.idUsuario=u.idUsuario and ".$comp." and ug.claveUnidad=a.Institucion order by fechaInicio";
		
		$resJueces=$con->obtenerFilas($consulta);
		
		while($fila=$con->fetchRow($resJueces))
		{
			$texto=$fila[4].'__['.date("d/m/Y - H:i ",strtotime($fila[1])).' - '.date("d/m/Y H:i",strtotime($fila[2])).' hrs.]';
			
			$fila[1]=date("Y-m-d H:i:s",strtotime($fila[1]));
			$fila[2]=date("Y-m-d H:i:s",strtotime($fila[2]));
			
			

			$titulo="<img onclick=javascript:removerAsignacion(\\\"".bE($fila[0])."\\\") src=\\\"../images/delete.png\\\" title=\\\"Remover asignaci&oacute;n\\\" alt=\\\"Remover asignaci&oacute;n\\\"> ".$texto;
			
			$o='{"id":"e_'.$fila[0].'","allDay":false,"editable":false,"title":"'.($titulo).'","start":"'.$fila[1].'","end":"'.$fila[2].'","color":"#F1F7FF"}';	
	...
	function registrarEventosJuez()
	{
		...
		switch($obj->tipoEvento)
		{
			...
			case 4:
				...
				if($obj->idArchivo!=-1)
				{
					$idDocumento=registrarDocumentoServidorRepositorio($obj->idArchivo,$obj->nombreArchivo,0);
					$consulta="INSERT INTO 9074_documentosRegistrosProceso(idFormulario,idRegistro,idDocumento,tipoDocumento)VALUES(20,".$idRegistro.",".$idDocumento.",1)";
	...
	function obtenerIncienciasJuez()
	{
		...
		$consulta="SELECT id__20_tablaDinamica,concat(fechaInicial,' ',g.hInicio) as fechaInicial,concat(fechaFinal,' ',g.hFin) as fechaFinal,'' as clave,u.nombre,ug.claveUnidad,g.tipoIncidencia,comentarios,'' as claveFolioCarpetas,
					g.hInicio,g.hFin,g.tipoIntervalo  FROM _20_tablaDinamica g,800_usuarios u,
					_17_tablaDinamica ug,801_adscripcion ads WHERE g.idEstado=1 and g.tipoIncidencia<>3 and ug.id__17_tablaDinamica in(".$uG.") 
					 AND u.idUsuario=g.usuarioJuez AND ads.Institucion=ug.claveUnidad AND ads.idUsuario=u.idUsuario
					AND u.idUsuario=g.usuarioJuez and ".$comp."  order by fechaInicial";
		...
		while($fila=$con->fetchRow($resJueces))
		{
			...
			$texto.='__[Del '.date("d/m/Y H:i",strtotime($fila[9])).' al '.date("d/m/Y H:i",strtotime($fila[10])).' hrs - '.
			...
			$consulta="SELECT idDocumento FROM 9074_documentosRegistrosProceso WHERE idFormulario=20 AND idRegistro=".$fila[0];
			$documentoAdjunto=$con->obtenerValor($consulta);

			$titulo="<img onclick=javascript:removerAsignacion(\\\"".bE($fila[0])."\\\") src=\\\"../images/delete.png\\\" title=\\\"Remover asignaci&oacute;n\\\" alt=\\\"Remover asignaci&oacute;n\\\"> ".$texto;
			if($documentoAdjunto!="")
			{
				$consulta="SELECT nomArchivoOriginal FROM 908_archivos WHERE idArchivo=".$documentoAdjunto;
				$nomArchivoOriginal=$con->obtenerValor($consulta);
				$arrArchivo=explode(".",$nomArchivoOriginal);
				$extension=$arrArchivo[count($arrArchivo)-1];
				$btn="<img src=\\\"../images/Icono_txt.gif\\\"  onclick=javascript:visualizarDocumentoAdjuntoB64(\\\"".bE($documentoAdjunto)."\\\",\\\"".bE($extension)."\\\") title=\\\"Visualizar documento adjunto\\\" alt=\\\"Visualizar documento adjunto\\\">";
				$titulo=$btn.=" ".$titulo;
			}
				
			$o='{"id":"e_'.$fila[0].'","allDay":false,"editable":false,"title":"'.($titulo).'","start":"'.$fila[1].'","end":"'.$fila[2].
				'","color":"#F1F7FF","tipoIncidencia":"'.cv($fila[6]).'","intervalo":"'.$fila[11].'","comentarios":"'.cv($fila[7]).
	...
	function obtenerVacacionesJuez()
	{
		...
		while($fila=$con->fetchRow($resJueces))
		{
			...
			$texto.='__[Del '.date("d/m/Y H:i",strtotime($fila[1])).' al '.date("d/m/Y H:i",strtotime($fila[2])).' hrs]';
			...
			$o='{"id":"e_'.$fila[0].'","allDay":false,"editable":false,"title":"'.($titulo).'","start":"'.$fila[1].'","end":"'.$fila[2].
				'","color":"#F1F7FF","tipoIncidencia":"'.cv($fila[6]).'","comentarios":"'.cv($fila[7]).
				'","documentoAdjunto":"'.$documentoAdjunto.'"}';	
	...
	function obtenerRegistroDiasNOHabiles()
	{
		...
		while($fila=$con->fetchRow($resDiasInhabiles))
		{
