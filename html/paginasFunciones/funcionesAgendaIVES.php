	...
	function obtenerDisponibilidadHorarioProfesor()
	{
		...
		if(($nReg==0)&&($idCiclo<=10))
		{
			if(!esPeriodoBase($idPeriodo))
			{
				...
				//$consulta="SELECT idPeriodo,idCiclo FROM 4544_fechasPeriodo WHERE '".$fechasPeriodo[0]."'>=fechaInicial AND '".$fechasPeriodo[0]."'<=fechaFinal AND idPeriodo IN (".obtenerPeriodoBase().") and idInstanciaPlanEstudio=".$filaFechas[4]." ORDER BY fechaFinal DESC";
		...
		$consulta="SELECT idDiaSemana,horaInicio,horaFin FROM 4065_disponibilidadHorario d
                    WHERE d.ciclo=".$idCiclo." AND d.idPeriodo=".$idPeriodo." AND  tipo=1 AND idUsuario=".$idUsuario."  ORDER BY idDiaSemana,horaInicio";
