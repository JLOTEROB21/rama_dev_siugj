<?php session_start();
	...
	function guardaIdentifica($con,$accion)
	{
		...
		$FExpedicion=isset($_POST['FExpedicion'])?$_POST['FExpedicion']:'';
		...
		$grupoEtnico=!isset($_POST["grupoEtnico"])?"NULL":$_POST["grupoEtnico"];
		$discapacidad=!isset($_POST["discapacidad"])?"NULL":$_POST["discapacidad"];
		$validado=isset($_POST["datosValidados"])?$_POST["datosValidados"]:"0";
		...
		if($con->ejecutarConsulta($query))
		{
			...
			if($accion=="Nuevo")
			{
				...
				if($cadTelefono!="")
				{
					...
					{
						$datosTel=explode('_',$arrTelefonos[$y]);
						$consulta[$x]="insert into 804_telefonos(codArea,Numero,Extension,Tipo,Tipo2,idUsuario) values('".$datosTel[0]."','".$datosTel[1]."','".$datosTel[2]."',0,".$datosTel[3].",".$Id.")";
						$x++;
					}
				}
			...
			else
			{
				...
				$query="SELECT idParticipante FROM 7005_relacionFigurasJuridicasSolicitud WHERE idCuentaAcceso=".$Id;
				...
				if($idParticipante!="")
				{
					$query="SELECT idRegistro FROM 7025_datosContactoParticipante WHERE idParticipante=".$idParticipante;
					...
					$consulta[$x]=" INSERT INTO 7025_datosContactoParticipanteRespaldos(idFormulario,idReferencia,idParticipante,fechaCreacion,calle,noExt,noInterior,
								...
								SELECT idFormulario,idReferencia,idParticipante,fechaCreacion,calle,noExt,noInterior,
								 ...
								 7025_datosContactoParticipante WHERE idParticipante=".$idParticipante;
					$x++;
					$consulta[$x]="set @iRegistroRespaldo:=(select last_insert_id())";
					$x++;
					
					$consulta[$x]="INSERT INTO 7025_correosElectronicoRespaldos(idReferencia,correo)
								SELECT  @iRegistroRespaldo,correo FROM 7025_correosElectronico WHERE idReferencia=".$idRegistro;
					$x++;
					$consulta[$x]="INSERT INTO 7025_telefonosRespaldos( idReferencia,tipoTelefono,lada,numero,extension,idPais)
								SELECT  @iRegistroRespaldo,tipoTelefono,lada,numero,extension,idPais FROM 7025_telefonos WHERE idReferencia=".$idRegistro;
					$x++;						
									//,localidad='".cv($o->localidad)."', //noExt='".cv($Numero)."',noInterior='".cv($o->noInt)."',
					$consulta[$x]="update 7025_datosContactoParticipante set fechaCreacion='".date("Y-m-d H:i:s")."',calle='".cv($Calle)."',
								colonia='".cv($Colonia)."',codigoPostal='".cv($CP)."',entidadFederativa='".cv($Estado)."',municipio='".cv($Ciudad)."'
					...
					$consulta[$x]="delete from 7025_correosElectronico where idReferencia=@idRegistro";
					$x++;
					...
					if($cadCorreos!="")
					{
						...
						{
							$datosMail=explode('/',$arrCorreos[$y]);
							$consulta[$x]="INSERT INTO 7025_correosElectronico(idReferencia,correo) VALUES(@idRegistro,'".cv($datosMail[0])."')";
							$x++;
						}
					}
					
					$consulta[$x]="delete from 7025_telefonos where idReferencia=@idRegistro";
					$x++;
					...
					if($cadTelefono!="")
					{
						...
						for($y=0;$y<$ct;$y++)
						{
							...
							$consulta[$x]="INSERT INTO 7025_telefonos(idReferencia,tipoTelefono,lada,numero,extension,idPais) VALUES(@idRegistro,'".$datosTel[3].
									"','','".$datosTel[1]."',".($datosTel[2]==""?"NULL":$datosTel[2]).",".$datosTel[0].")";
			...
			{
				$consulta[$x]="INSERT INTO _47_respadoDatosIdentificacion(idPersonaRegistro,fechaCambio,responsableCambio,
							...
							grupoEtnico,discapacidad,aceptaNotificacionMail)		
							SELECT '".$idParticipante."','".date("Y-m-d H:i:s")."',".$_SESSION["idUsr"].",tipoPersona,esMexicano,
				...
				$x++;
				$consulta[$x]="set @idRegistroPersona:=(select last_insert_id())";
				$x++;
				$consulta[$x]="INSERT INTO _47_respaldoAlias(idRegistroRespaldo,nombre,apPaterno,apMaterno)
							SELECT @idRegistroPersona,nombre,apPaterno,apMaterno FROM _47_gAlias WHERE idReferencia=".$idParticipante;
				$x++;
				
				$consulta[$x]="UPDATE _47_tablaDinamica SET nacionalidad='".cv($Nacionalidad)."',rfcEmpresa='".cv($RFC).
							"',curp='".cv($CURP)."',apellidoPaterno='".cv(trim($Apat))."',apellidoMaterno='".cv(trim($Amat))."',nombre='".cv(trim($Nombres)).
							"',genero=".$cmbGenero.",fechaNacimiento=".($FNacimiento==""?"NULL":"".$FNacimiento."").
							",tipoIdentificacion='".($tipoIdentificacion==""?"NULL":$tipoIdentificacion).
							"',folioIdentificacion='".cv($noIdentificacion)."' WHERE id__47_tablaDinamica=".$idParticipante;
			...
			?>	
					...
					<script>
							if(typeof(window.parent.regresarGuardar)!='undefined')
							{	
								 window.parent.regresarGuardar('<?php echo $bandera?>');
							}
        			</script>
